...
class Manager:
    ...
    def downscale_folder_seg(self,input_folder,output_folder,voxel_size):
        files = os.listdir(input_folder)
        files.sort()
        if not os.path.isdir(output_folder):
            os.makedirs(output_folder)
        for file in files:
            ...
            if file.endswith(('.mha', '.nii', '.mha.gz', '.nii.gz', '.inr', '.inr.gz')):
                ...
                if not os.path.isfile(img_t):
                    print("     -> " + str(img_path))
                    os.system("conda run -n astec applyTrsf -interpolation nearest -iso " + str(voxel_size) + " " + img_path + " " + img_t)
    ...
    def plot_shifts(self,cells_shifts,embryo_name):
        ...
        for t in cells_shifts:
            ...
            plt.ylabel("Distribution of contact surface shits")
    ...
    def plot_surfaces(self, half_surfaces_by_t, full_surfaces_by_t,embryo_name, means_by_t, pvalues_by_t):
        ...
        for t in half_surfaces_by_t:
            ...
            plt.xlabel("Time point")
            plt.legend(["Mean shift = "+str(mean_shift),"pvalue spearman = "+str(pvalue)])
    ...
    def plot_means(self,means_by_t , std_by_t,embryo_name):
        ...
        plt.title("Mean of shits between surfaces contact for : " + str(embryo_name))
        plt.ylabel("Mean ")
        plt.xlabel("Time point")
        plt.xticks(rotation = 45)
        ...
        plt.clf()
    def compare_surfaces_downscale(self,embryo_name,input_folder_prefix,input_folder_suffix,times):
        ...
        self.downscale_folder_seg(input_folder,downscaled_folder,0.6)
        ...
        if not os.path.isfile(lineage_file_down):
            self.generate_surface("INT_FUSE_01_V2", "CORR_INT_POST_V2_06", times[0], times[1], lineage_file_down, embryo_name, "01")
    ...
    def compute_all_surfaces_downscale(self):
        ...
        for i in range(0,len(names)):
            self.compare_surfaces_downscale(names[i],input_folder_prefix,input_folder_suffix,times[i])
