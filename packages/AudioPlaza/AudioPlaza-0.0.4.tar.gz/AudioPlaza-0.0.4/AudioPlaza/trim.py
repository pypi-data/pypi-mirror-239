...
class trimmer:
    ...
    def __pad_audio(self, audio, startpoint, endpoint, padding_sec=0.3):
        base_size = int(padding_sec * self.sample_rate) *2 + (endpoint - startpoint)
        padded = torch.zeros(base_size, dtype=torch.float32)
        padded_start = int(padding_sec * self.sample_rate)
        padded_end = int(padding_sec * self.sample_rate) + (endpoint - startpoint)

        if padded_end > base_size:
            padded_end = base_size
            padded_start = base_size - (endpoint - startpoint)
        
        if endpoint > audio.shape[0]:
            endpoint = audio.shape[0]
            padded_end = padded_start + (endpoint - startpoint)    
        
        padded[padded_start:padded_end] = audio[startpoint:endpoint]
    ...
    def trim(self, audio_path, output_path, padding_sec=0.3):
        ...
        padded_audio = self.__pad_audio(audio, startpoint, endpoint, padding_sec=padding_sec)
