...
class AbstractItem(ABC, metaclass=type):
    ...
    def _materialize(self, grid, cell):
        dummy = GridItem(grid, self.name, cell)
        dummy_dict = {}
        for k, v in dummy.__dict__.copy().items():
            if k.startswith('_'):
                dummy_dict[k.replace('_', '', 1)] = v
            else:
                dummy_dict[k] = v
        self.__dict__.update(dummy_dict)
        del dummy
        return self
