...
class BaseCharacter(BaseActor):
    ...
    def _see_item(self, item):
        if self._is_in_sight(item):
            direction = get_direction(Vec2d(item.position[0], item.position[1]) ,Vec2d(self.position[0], self.position[1]))
        
            if self._is_in_pickup_range(item):
                self._nearby_items[item.name] = item
                print(f'You see {item} at your feet.')
            else:
                print(f'You see {item} to the {direction}')
        elif self._is_in_distant_view(item):
            print(f'You see something away to the {direction}')
