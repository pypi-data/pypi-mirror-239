"""
This file defines the Cosmo class.
It's the main file. `import cosmo` will import an instance of this class.
"""
...
class Cosmo:
            ...
            if not self.local:
                ...
                if check_for_update():
                    display_markdown_message(
                        "> **A new version of Open Cosmo is available.**\n>Please run: `pip install --upgrade open-cosmo`\n\n---"
                    )
        except:
            # Doesn't matter
            pass
    ...
    def _streaming_chat(self, message=None, display=True):
        ...
        # Display mode actually runs cosmo.chat(display=False, stream=True) from within the terminal_interface.
        ...
        if message or message == "":
            ...
            return
        raise Exception(
            "`cosmo.chat()` requires a display. Set `display=True` or pass a message into `cosmo.chat(message)`."
        )
    ...
    def reset(self):
        for code_cosmo in self._code_cosmos.values():
            code_cosmo.terminate()
        self._code_cosmos = {}
