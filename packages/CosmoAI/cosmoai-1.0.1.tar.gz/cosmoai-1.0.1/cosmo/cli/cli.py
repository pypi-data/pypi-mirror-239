...
def cli(cosmo):
    parser = argparse.ArgumentParser(description="Open Cosmo")
    ...
    parser.add_argument(
        ...
        dest="fast",
        action="store_true",
        help="run `cosmo --model gpt-3.5-turbo`",
    )
    parser.add_argument(
        "--version",
        dest="version",
        action="store_true",
        help="get Open Cosmo's version number",
    )

    args = parser.parse_args()

    # This should be pushed into an open_config.py util
    # If --config is used, open the config.yaml file in the Open Cosmo folder of the user's config dir
    if args.config:
        if args.config_file:
    ...
    if args.local:
        ...
        cosmo.system_message = "You are an AI."
        cosmo.model = (
            "openai/" + cosmo.model
        )  # This tells LiteLLM it's an OpenAI compatible server
        cosmo.api_base = "http://localhost:1234/v1"
        cosmo.max_tokens = 600
        cosmo.context_window = 3000
        cosmo.api_key = "0"

        display_markdown_message(
            """
> Open Cosmo's local mode is powered by **`LM Studio`**.
...
> Don't expect `gpt-3.5` / `gpt-4-1106-preview` level quality, speed, or reliability yet!

"""
        )

    # Set attributes on cosmo
    for attr_name, attr_value in vars(args).items():
        # Ignore things that aren't possible attributes on cosmo
        if attr_value is not None and hasattr(cosmo, attr_name):
            # If the user has provided a config file, load it and extend cosmo's configuration
            if attr_name == "config_file":
                user_config = get_config_path(attr_value)
                cosmo.config_file = user_config
                cosmo.extend_config(config_path=user_config)
            else:
                setattr(cosmo, attr_name, attr_value)

    # if safe_mode and auto_run are enabled, safe_mode disables auto_run
    if cosmo.auto_run and (
        cosmo.safe_mode == "ask" or cosmo.safe_mode == "auto"
    ):
        setattr(cosmo, "auto_run", False)

    # If --conversations is used, run conversation_navigator
    if args.conversations:
        conversation_navigator(cosmo)
        return

    if args.version:
        version = pkg_resources.get_distribution("open-cosmo").version
        print(f"Open Cosmo {version}")
        return
