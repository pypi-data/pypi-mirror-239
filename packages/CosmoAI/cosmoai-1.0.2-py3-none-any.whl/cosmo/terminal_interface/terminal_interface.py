...
examples = [
    ...
    "Check all the links on `opencosmo.com`.",
]
...
def terminal_interface(cosmo, message):
    ...
    if not cosmo.auto_run and not cosmo.local:
        cosmo_intro_message = [
            "**Open Cosmo** will require approval before running code."
        ]

        if cosmo.safe_mode == "ask" or cosmo.safe_mode == "auto":
            if not check_for_package("semgrep"):
                cosmo_intro_message.append(
                    f"**Safe Mode**: {cosmo.safe_mode}\n\n>Note: **Safe Mode** requires `semgrep` (`pip install semgrep`)"
                )
        else:
            cosmo_intro_message.append("Use `cosmo -y` to bypass this.")

        cosmo_intro_message.append("Press `CTRL-C` to exit.")

        display_markdown_message("\n\n".join(cosmo_intro_message) + "\n")
    ...
    while True:
        ...
        if message.startswith("%") and interactive:
            handle_magic_command(cosmo, message)
            continue

        # Many users do this
        if message.strip() == "cosmo --local":
            print("Please press CTRL-C then run `cosmo --local`.")
            continue
            ...
            for chunk in cosmo.chat(message, display=False, stream=True):
                ...
                if "executing" in chunk:
                    if not cosmo.auto_run:
                        ...
                        if not cosmo.safe_mode == "off":
                            if cosmo.safe_mode == "auto":
                                should_scan_code = True
                            elif cosmo.safe_mode == "ask":
                                response = input(
                ...
                if "output" in chunk:
                    ...
                    active_block.output = truncate_output(
                        active_block.output, cosmo.max_output
                    )
