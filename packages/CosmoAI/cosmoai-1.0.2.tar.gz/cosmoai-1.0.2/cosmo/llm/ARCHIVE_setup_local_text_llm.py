...
def setup_local_text_llm(cosmo):
    """
    ...
    repo_id = cosmo.model.replace("huggingface/", "")
    ...
    if repo_id != "TheBloke/Mistral-7B-Instruct-v0.1-GGUF":
        ...
        display_markdown_message(
            f"**Open Cosmo** will use `{repo_id}` for local execution."
        )

    if "gguf" in repo_id.lower() and cosmo.gguf_quality == None:
        ...
        cosmo.gguf_quality = gguf_quality_choices[answers["gguf_quality"]]

    path = ooba.download(f"https://huggingface.co/{repo_id}")

    ooba_llm = ooba.llm(path, verbose=cosmo.debug_mode)
    print("\nReady.\n")
    ...
    def local_text_llm(messages):
        ...
        if cosmo.context_window:
            context_window = cosmo.context_window
        else:
            context_window = DEFAULT_CONTEXT_WINDOW

        if cosmo.max_tokens:
            max_tokens = cosmo.max_tokens
        else:
        ...
        prompt = messages_to_prompt(messages, cosmo.model)
        """
        ...
        if "mistral" in repo_id.lower():
            ...
            messages[0][
                "content"
            ] = "You are Open Cosmo. You almost always run code to complete user requests. Outside code, use markdown."
            messages[0][
                "content"
        ...
        for token in ooba_llm.chat(messages):
            ...
            # instead of symbols in their code when used with Open Cosmo.
