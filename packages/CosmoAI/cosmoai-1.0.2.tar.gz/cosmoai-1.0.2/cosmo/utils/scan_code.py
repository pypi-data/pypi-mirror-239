...
from ..code_cosmos.language_map import language_map
...
def scan_code(code, language, cosmo):
    """
    Scan code with semgrep
    """

    temp_file = create_temporary_file(
        code, get_language_file_extension(language), verbose=cosmo.debug_mode
    )
    ...
    try:
        ...
        if scan.returncode == 0:
            ...
            print(
                f"  {'Code Scaner: ' if cosmo.safe_mode == 'auto' else ''}No issues were found in this {language_name} code."
            )
            print("")
    ...
    cleanup_temporary_file(temp_file, verbose=cosmo.debug_mode)
