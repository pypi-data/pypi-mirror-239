...
def get_manufactures_container() -> dict[os.DirEntry, dict[os.DirEntry, dict[os.DirEntry, int]]]:
    ret = dict()
    with os.scandir(__collection_path) as ms:
        for manufacture in ms:
            ret[manufacture] = dict()
            with os.scandir(manufacture) as ts:
                for type_ in ts:
                    if type_.is_dir():
                        ret[manufacture][type_] = list()
                        with os.scandir(type_) as vs:
                            for ver in vs:
                                ret[manufacture][type_].append(ver)
    return ret
