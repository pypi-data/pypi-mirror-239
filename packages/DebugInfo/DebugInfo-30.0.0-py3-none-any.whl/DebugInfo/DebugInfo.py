...
class 交互接口类:
    ...
    __输入提示修饰方法: _Callable[[str], str] or None = 蓝字
    ...
    def __init__(self):
        ...
        self.__输入提示修饰方法: _Callable[[str], str] or None = 交互接口类.__输入提示修饰方法
    ...
    def 添加选项(self,
        ...
        if isinstance(选项, 交互选项类):
            ...
            # 统一把 选项,代号, 备注 识为字符串
            if 选项:
                待添加选项列表.append(交互选项类(代号=str(代号 if 代号 else ''), 选项=str(选项), 说明=str(备注 if 备注 else '')))
    ...
    def 发起文本交互(cls,
        ...
        if isinstance(限定范围, list):
            if 限定范围:
                限定范围 = [str(限定项).strip() for 限定项 in 限定范围]
            if 限定范围:
                限定范围 = [限定项 for 限定项 in 限定范围 if 限定项]
            ...
            while True:
                ...
                if not 输入文本:
                    ...
                    if not 多选:
                        ...
                        输入文本切割 = list(set([有效文本 for 有效文本 in [文本.strip() for 文本 in 输入文本切割] if 有效文本]))
                        if 输入文本切割:
                            存在无效值: bool = False
                            for 文本 in 输入文本切割:
                                if 文本 not in 选项代号表:
    ...
    @classmethod
    def 指定路径(cls, 输入提示: str = None, 画板: 打印模板 = None) -> str:
        """
        ...
        路径: str
        while True:
            路径 = (cls.发起文本交互(输入提示=输入提示, 画板=画板)).strip()
            if '0' == 路径:
                break
            else:
                路径 = cls.路径格式化(路径)
    ...
    @classmethod
    def 指定文档(cls, 输入提示: str = None, 画板: 打印模板 = None) -> str:
        """
        向用户发起交互,以获取用户指定的文档
        :param 输入提示: 提示用户输入时的提示内容
        :param 画板: 提供用户交互输出的渠道
        ...
        """
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(cls, cls.指定文档)

        输入提示 = (输入提示 if 输入提示 else '').strip()
        输入提示 = 输入提示 if 输入提示 else '请指定文档: '

        文档: str
        while True:
            文档 = (cls.发起文本交互(输入提示=输入提示, 画板=画板)).strip()
            if '0' == 文档:
                break
            else:
                文档 = cls.路径格式化(文档)
    ...
    def 选择文档(初始路径: str = None,
        ...
        if not (参考路径 and _os.path.isdir(参考路径)):
            # 如果参考路径无效,则引导用户指定参考路径
            画板.消息(f"当前路径为: {交互接口类.路径格式化('.')}")
            参考路径 = 交互接口类.指定路径(输入提示='请指定文档路径(0: 退出操作): ', 画板=画板)
            if '0' == 参考路径:
                # 如果用户选择了退出,则退出
                选择结果.append('0')
                return 选择结果
