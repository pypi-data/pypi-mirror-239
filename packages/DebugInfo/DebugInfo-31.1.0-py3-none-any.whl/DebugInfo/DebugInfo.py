...
class 交互接口类:
    ...
    class _文档信息结构:
        def __init__(self,
            ...
            self.路径: str = 路径
            self.体积: str = 体积
            self.修改时间: _datetime = _datetime.fromtimestamp(修改时间)

    class _everything搜索结果类:
        """
        整理everyThing的搜索结果
        """
        ...
        def __init__(self):
            self.总数: int = 0
            self.状态码: int = 0
            self.结果列表: list[交互接口类._文档信息结构] = []
            self.错误消息: str = ''

    # 提供输入提示消息的修饰,以期提高交互体验, 默认输入提示文本为白底黑色样式,以便醒目
    __输入提示修饰方法: _Callable[[str], str] or None = 白底黑字
    __EverythingHostname: str = '127.0.0.1'
    __EverythingHostPort: int = 0
    __Everything服务存在: bool = True
    ...
    # endregion
    @classmethod
    def __everything可用(cls) -> bool:
        if cls.__Everything服务存在:
            if cls.__EverythingHostname and 1 <= cls.__EverythingHostPort <= 65535:
                return True
        return False

    @classmethod
    def _everything搜索(cls,
                        搜索关键字: str,
                        搜文档: bool = True,
                        搜路径: bool = False,
                        画板: 打印模板 = None) -> _everything搜索结果类:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(cls, cls._everything搜索)

        搜索结果: cls._everything搜索结果类 = cls._everything搜索结果类()
        搜索关键字 = str(搜索关键字 if isinstance(搜索关键字, str) else '').strip()
        画板.调试消息(f'搜索关键字为: {搜索关键字}')

        # 默认搜文档
        if not 搜文档 and not 搜路径:
            搜文档 = True

        if 搜索关键字 and cls.__everything可用:
            搜索url: str = f'http://{cls.__EverythingHostname}:{cls.__EverythingHostPort}/?search={搜索关键字}' \
                           f'&json=1&path_column=1&size_column=1&date_modified_column=1' \
                           f'&sort=name&ascending=1'
            ...
            try:
                ...
                画板.消息(f'推荐您参考文档 https://ynrx7b5i1u.feishu.cn/docx/CKO8d2NlTo62yVxkUKmcMSWBnDf '
                          f'设置并启用 everything 服务, 并绑定 http 服务端口为: {交互接口类.__EverythingHostPort}')
            else:
                            ...
                            if everything搜索结果['totalResults'] > 0:
                                for result in everything搜索结果['results']:
                                    ...
                                    if 'size' in result.keys():
                                        文档.体积 = result['size']
                                    if 'date_modified' in result.keys() and str(result['date_modified']).isnumeric():
                                        文档.修改时间 = int(result['date_modified'])

                                    if 文档.名称 and '$RECYCLE.BIN' not in 文档.名称:
                                        # 如果文档名有效且这不是个拉圾站中的文档
                                        文档命中: bool = False
                                        if 搜文档 and 'file' == 文档.类型:
                                            if not 文档命中:
                                                搜索结果.结果列表.append(文档)
                                                文档命中 = True
                                        if 搜路径 and 'folder' == 文档.类型:
                                            if not 文档命中:
                                                搜索结果.结果列表.append(文档)
                                                文档命中 = True
    ...
    @classmethod
    def 设置everything服务地址(cls, 地址: str = '127.0.0.1'):
        cls.__EverythingHostname = str(地址 if isinstance(地址, str) else '').strip()

        # 复位 everything 服务状态
        if cls.__EverythingHostname and 1 <= cls.__EverythingHostPort <= 65535:
            cls.__Everything服务存在 = True

    @classmethod
    def 设置everything服务端口(cls, 端口: int = 22):
        端口号: int = 0
        if isinstance(端口, int):
            端口号 = 端口
        elif isinstance(端口, str):
            端口 = 端口.strip()
            if 端口.isnumeric():
                端口号 = int(端口)

        if 1 <= 端口号 <= 65535:
            cls.__EverythingHostPort = 端口号
        else:
            cls.__EverythingHostPort = 0
            ...
            if 选项数量 > 0:
                ...
                for 选项索引 in range(选项数量):
                    if isinstance(选项[选项索引], 交互选项类):
                        ...
                        待添加选项列表.append(交互选项类(代号=代号值, 选项=选项名称, 说明=说明))
        else:
            # 统一把 选项,代号, 备注 识为字符串
            if 选项:
                待添加选项列表.append(
                    交互选项类(代号=str(代号 if 代号 else ''), 选项=str(选项), 说明=str(备注 if 备注 else '')))

        添加选项数量: int = 0
        if 待添加选项列表:
            for 项 in 待添加选项列表:
                if 项.有效 and 项.选项 not in [选项.选项 for 选项 in self._选项列表]:
                    self._选项列表.append(项)
                    添加选项数量 += 1
    ...
    @classmethod
    def 指定选择路径(cls, 输入提示: str = None, 画板: 打印模板 = None) -> str:
        """
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                路径 = cls.路径格式化(用户输入)
                if not _os.path.isdir(用户输入):
                    if cls.__everything可用():
                        # 尝试通过 everything 列出符合条件的路径供用户选择
                        可用路径列表 = cls._everything搜索(搜索关键字=用户输入.replace('/', '\\').rstrip('\\'),
                                                   搜文档=False,
                                                   搜路径=True,
                                                   画板=画板.副本)
                        if 可用路径列表:
                            if 可用路径列表.总数 > 0:
                                交互端 = 交互接口类()
                                路径列表: list[str] = []
                                for 文档 in 可用路径列表.结果列表:
                                    路径列表.append(f'{文档.路径}\\{文档.名称}')
                                if len(路径列表) <= 12:
                                ...
                                选择结果 = 交互端.发起选项交互(输入提示='请选择路径代号',
                                                  操作说明=f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的路径供您选择',
                                                  兴趣字=用户输入,
                                                  画板=画板.副本)
                                if '0' in 选择结果.代号:
                                    break
                                ...
                                画板.提示错误(f'everything搜索反馈错误:{可用路径列表.状态码}')
                                if 可用路径列表.错误消息:
                                    画板.消息(可用路径列表.错误消息)
                if not _os.path.isdir(路径):
                    # 如果依然没有找到可用的路径,则返回用户重新输入
                    画板.消息(f'所指定的路径不存在: {路径}')
                    continue
                else:
                    break
        return 路径
    ...
    @classmethod
    def 指定选择文档(cls, 输入提示: str = None, 画板: 打印模板 = None) -> str:
        """
        向用户发起交互,以获取用户指定的文档
        :param 输入提示: 提示用户输入时的提示内容
        :param 画板: 提供用户交互输出的渠道
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                文档 = cls.路径格式化(用户输入)
                if not _os.path.isfile(文档):
                    # 尝试通过 everything 列出符合条件的文档供用户选择
                    if cls.__everything可用():
                        # everything可用,尝试通过everything搜索匹配的路径,以供用户选择
                        可用文档列表 = cls._everything搜索(搜索关键字=用户输入.replace('/', '\\').rstrip('\\'),
                                                   搜文档=True,
                                                   搜路径=False,
                                                   画板=画板.副本)
                        if 可用文档列表:
                            if 可用文档列表.总数 > 0:
                                交互端 = 交互接口类()
                                路径列表: list[str] = []
                                for 结果 in 可用文档列表.结果列表:
                                    路径列表.append(f'{结果.路径}\\{结果.名称}')
                                ...
                                选择结果 = 交互端.发起选项交互(输入提示='请选择文档代号',
                                                  操作说明=f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的文档供您选择',
                                                  兴趣字=用户输入,
                                                  画板=画板.副本)
                                if '0' in 选择结果.代号:
                                    break
                                ...
                                画板.提示错误(f'everything搜索反馈错误:{可用文档列表.状态码}')
                                if 可用文档列表.错误消息:
                                    画板.消息(可用文档列表.错误消息)
                if not _os.path.isfile(文档):
                    画板.消息(f'所指定的文档不存在: {文档}')
                    continue
                else:
                    break
        return 文档
    ...
    def 选择文档(初始路径: str = None,
        ...
        # 提供用户选择文档的交互
        while True:
            if not (参考路径 and _os.path.isdir(参考路径)):
                画板.消息(f"当前路径为: {交互接口类.路径格式化('.')}")
                参考路径 = 交互接口类.指定选择路径(输入提示='请指定文档路径或者关键字(0: 退出操作): ', 画板=画板)
                if '0' == 参考路径:
                    # 如果用户选择了退出,则退出
                    选择结果.append('0')
                    return 选择结果
            ...
            with _os.scandir(参考路径) as 文档集:
                for 文档 in 文档集:
                    if 文档.name and not str(文档.name).startswith('~$'):
                        if _os.path.isfile(f'{参考路径}/{文档.name}'):
                            文档列表.append(文档.name)

            if not 文档列表:
                画板.消息(f'当前路径下没有可用的文档: {参考路径}')
                # 引导用户重新指定操作路径
                参考路径 = 交互接口类.指定选择路径(输入提示='请指定文档路径(0: 退出操作): ', 画板=画板)
                if '0' == 参考路径:
                    # 如果用户选择了退出,则退出
                    选择结果.append('0')
                    return 选择结果
            ...
            交互端: 交互接口类 = 交互接口类()
            if len(文档列表) <= 12:
                交互端.添加选项(选项=sorted(文档列表))
            else:
                交互端.添加选项(选项=sorted(文档列表, reverse=True))
            ...
            if 'c' in 用户选择.代号 or 'C' in 用户选择.代号:
                ...
                for 文档 in 用户选择.选项:
                    if 文档:
                        ...
                        # 确保用户所选择的文档是存在的
                        if _os.path.isfile(文档全名):
                            临时选择结果.append(文档全名)
                        else:
                            画板.消息(f'所选择的文档无效: {文档全名}')

                            # 用户选择了无效的文档,需要引导用户重新选择
                            continue
