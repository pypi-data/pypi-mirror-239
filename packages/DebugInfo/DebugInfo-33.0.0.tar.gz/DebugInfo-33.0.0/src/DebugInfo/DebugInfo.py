...
class 搜索接口基类:
    def __init__(self,
                 ...
                 hostport: int = 0,
                 hostuser: str = None,
                 hostpassword: str = None):
        self._hostname: str = hostname
        self._hostport: int = hostport
        self._hostuser: str = hostuser
        self._hostpassword: str = hostpassword

        # 表示当前 everything 服务是否在线
        self.服务在线: bool = True
    ...
    def hostname(self, hostname: str):
        hostname = (hostname if hostname else '').strip()
        if hostname:
    ...
    @hostport.setter
    def hostport(self, port: int):
        if isinstance(port, str) and port.isnumeric():
            port = int(port)
    ...
    @property
    def hostuser(self) -> str:
        return self._hostuser

    @hostuser.setter
    def hostuser(self, hostuser: str):
        hostuser = (hostuser if hostuser else '').strip()
        if hostuser:
            self._hostuser = hostuser

        self.服务在线 = self.可用

    @property
    def hostpassword(self) -> str:
        return self._hostpassword

    @hostpassword.setter
    def hostpassword(self, hostpassword: str):
        hostpassword = (hostpassword if hostpassword else '').strip()
        if hostpassword:
            self._hostpassword = hostpassword

        self.服务在线 = self.可用

    # endregion

    # 提供一个判定搜索接口是否可用的统一方法,子类需要生写该方法
    def 可用性判定(self) -> bool:
        return False
...
class everything搜索接口类(搜索接口基类):
    def __init__(self,
                 hostname: str = None,
                 hostport: int = 0):
        super(everything搜索接口类, self).__init__(hostname=hostname,
                                              hostport=hostport)

    def 可用性判定(self) -> bool:
        # 提供搜索接口可用性判定逻辑
        if self.服务在线:
            if self._hostname and 1 <= self._hostport <= 65535:
                return True
        return False
    ...
    def 搜索(self,
        ...
        if self.可用 and 搜索关键字:
            搜索url: str = f'http://{self._hostname}:{self._hostport}/?search={搜索关键字}' \
            ...
            try:
                ...
                画板.消息(f'推荐您参考文档 https://ynrx7b5i1u.feishu.cn/docx/CKO8d2NlTo62yVxkUKmcMSWBnDf '
                      f'设置并启用 everything 服务, 并绑定 http 服务端口为: {self._hostport}')
            else:
...
class sshFind搜索接口类(搜索接口基类):
    def __init__(self,
                 ...
                 hostuser: str = None,
                 hostpassword: str = None):
        super(sshFind搜索接口类, self).__init__(hostname=hostname,
                                           hostport=hostport,
                                           hostuser=hostuser,
    ...
    def 可用性判定(self) -> bool:
        ...
        if self.服务在线:
            if self._hostname and 1 <= self._hostport <= 65535:
                if self._hostuser and self._hostpassword:
                    return True
        return False
        ...
        if self.可用 and 搜索关键字:
            ...
            if 搜索关键字:
                if 指定find参数:
                    ...
                    find指令 = f'find {参考路径} -iname "{搜索关键字}" -type f'
                elif 搜路径:
                    find指令 = f'find {参考路径} -iname "{搜索关键字}" -type d'
                    ...
                    if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用:
                        ...
                        if 可用路径列表:
                            if 可用路径列表.总数 > 0:
                                ...
                                操作说明: list = [f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的路径供您选择']
                                if 可用路径列表.截断:
                    ...
                    if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用:
                        ...
                        if 可用文档列表:
                            if 可用文档列表.总数 > 0:
                                ...
                                操作说明: list = [f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的文档供您选择']
                                if 可用文档列表.截断:
                                    操作说明.append(红字(f'搜索结果未完全呈现'))
