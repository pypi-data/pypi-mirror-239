...
class Univariate(ptf_analysis):
    ...
    def difference(self,average_group:ndarray) -> ndarray:
        ...
        '''
        if self._holding_period > 1 :
            r, c = np.shape(average_group)
            temp_average_group = np.zeros((r, c-self._holding_period))

            for j in range(c-self._holding_period):
                temp_average_group[:, j] = np.prod(1 + average_group[:, j : self._holding_period + j], axis=1) - 1 

            average_group = temp_average_group
    ...
    def summary_statistics(self, variables:DataFrame =None, periodic:bool =False):
        ...
        if periodic == True:
            ...
            for i in range(len(groups_time)):
                ...
                stats_skew = sts.skew(group[:, 1].astype(np.float64))
                stats_kurt = sts.kurtosis(group[:, 1].astype(np.float64))
            ...
            if type(variables).__name__ == 'DataFrame' or type(variables).__name__ == 'Series':
                ...
                for j in range(c):
                    ...
                    for i in range(len(groups_time)):
                        ...
                        stats_skew = sts.skew(group[:, -(c-j)].astype(np.float64))
                        stats_kurt = sts.kurtosis(group[:, -(c-j)].astype(np.float64))
            ...
            stats_skew = sts.skew(self.sample[:, 1].astype(np.float64))
            stats_kurt = sts.kurtosis(self.sample[:, 1].astype(np.float64))
            ...
            if type(variables).__name__ == 'DataFrame' or type(variables).__name__ == 'Series':
                ...
                if c > 1: 
                    stats_skew = sts.skew(variables.iloc[:, -c:].astype(np.float64), axis=0)
                elif c == 1:
                    stats_skew = [sts.skew(variables.astype(np.float64))] 
                stats_kurt = sts.kurtosis(temp_sample[:, -c:].astype(np.float64), axis=0)
                ...
                stats_std = np.std(temp_sample[:, -c:], axis=0, dtype=float)
                if c > 1:    
                    stats_skew = sts.skew(variables[:, -c:].astype(np.float64), axis=0)
                elif c == 1:
                    stats_skew = [sts.skew(variables.astype(np.float64))]
                stats_kurt = sts.kurtosis(temp_sample[:, -c:].astype(np.float64), axis=0)
                stats_min = np.min(temp_sample[:, -c:], axis=0)
