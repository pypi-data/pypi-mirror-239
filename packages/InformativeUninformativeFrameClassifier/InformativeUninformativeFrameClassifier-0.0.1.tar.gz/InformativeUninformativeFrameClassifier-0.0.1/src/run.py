...
This has been adapted from the original code of https://github.com/luiscarlosgph/vitcifar10
"""
import argparse
import glob
import random
...
def help(option):
    ...
    """
    help_msg = {
        "--jsdatasets": "Path to the json file containing the dataset ",
        ...
        "--pretrained": "Initialise model with pretrained weights " + "from ImageNet.",
...
def parse_cmdline_params():
    ...
    desc = "Run many training cycles of the ViT on input datasets."
    args = argparse.ArgumentParser(description=desc)
    args.add_argument(
        "--jsdatasets", required=True, default=None, type=str, help=help("--jsdatasets")
    )
    args.add_argument("--niter", required=True, type=int, help=help("--niter"))
    ...
    for train_iter in range(0, args.niter):
        ...
        if not only_model_best(args_copy.cpdir):
            ...
            if path_to_last_checkpoint is None:
                ...
                model_functions.train.main(args_copy, train_dl, valid_dl)
            else:
            ...
            checkpoints = [
                f for f in os.listdir(args_copy.cpdir) if f.startswith("epoch_")
            ]
