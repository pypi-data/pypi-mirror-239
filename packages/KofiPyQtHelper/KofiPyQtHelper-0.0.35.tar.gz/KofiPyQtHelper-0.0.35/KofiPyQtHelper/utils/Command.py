    ...
    def processExcelFile(self, loadFile, variateTemplate, childrenCount):
        ...
        if self.category:
            path = CommonHelper.getAbsFilePath(
                f"./config/values/{self.category}/{variateTemplate}"
            )
        else:
            path = CommonHelper.getAbsFilePath(f"./config/values/{variateTemplate}")
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            for config in configList:
                if "isChildren" in config and config["isChildren"]:
                    ...
                    item[config["name"]] = sheet.cell(row, config["index"]).value
                else:
                    data[config["name"]] = sheet.cell(row, config["index"]).value
