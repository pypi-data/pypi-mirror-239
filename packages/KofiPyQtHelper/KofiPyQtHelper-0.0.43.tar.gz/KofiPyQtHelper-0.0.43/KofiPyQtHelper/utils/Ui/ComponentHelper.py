...
class ComponentHelper(TableHelper, TreeHelper, ListviewHelper, ValueHelper):
    ...
    def initCombobox(self, parent: QWidget, info):
        objInput = QComboBox()
        if "itemCommand" in info:
            params = info["itemParams"] if "itemParams" in info else None
            info["items"] = self.commands[info["itemCommand"]](params)
        ...
        self.variates[info["name"]] = self.setComboValue(
            objInput,
            info["valueType"],
            info["items"],
            info["value"] if "value" in info else "",
        )
