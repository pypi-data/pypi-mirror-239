...
def generate_read_enum(ark_json_schema, enum, current_package):
    ...
    if object_namespace == current_package:
        return f"{name}(value=read_{enum_class}(input_stream, TRIM_SIZE=TRIM_SIZE))"
    else:
        # TODO add an import for this object_namespace here
        object_namespace = object_namespace.replace("::", ".")
        return f"{object_namespace}.{name}(value=read_{enum_class}(input_stream, TRIM_SIZE=TRIM_SIZE))"


def get_read_statement(ark_json_schema, field, current_package):
    if field["type"] in primitive_types:
        if field["type"] in numerical_types:
            return f"read_{field['type']}(input_stream, TRIM_SIZE=TRIM_SIZE)"
        elif field["type"] == "bool":
            return "read_bool(input_stream, TRIM_SIZE=TRIM_SIZE)"
        elif field["type"] == "guid":
            return "read_guid(input_stream, TRIM_SIZE=TRIM_SIZE)"
        elif field["type"] == "string":
            return "read_string(input_stream, TRIM_SIZE=TRIM_SIZE)"
    if field["type"] == "byte_buffer":
        return "read_byte_buffer(input_stream, TRIM_SIZE=TRIM_SIZE)"
    if field["type"] == "arraylist":
        return f"read_arraylist(input_stream, field={field}, TRIM_SIZE=TRIM_SIZE)"
    if field["type"] == "array":
        return f"read_array(input_stream, field={field}, length={field['array_size']}, TRIM_SIZE=TRIM_SIZE)"
    if field["type"] == "dictionary":
        return f"read_dictionary(input_stream, field={field}, TRIM_SIZE=TRIM_SIZE)"
    if field["type"] == "enum":
    ...
    if field["type"] == "object":
        object_namespace, name = field["object_type"].rsplit("::", 1)
        if object_namespace == current_package:
            return f"{name}().deserialize(input_stream, TRIM_SIZE=TRIM_SIZE)"
        else:
            object_namespace = object_namespace.replace("::", ".")
            return f"{object_namespace}.{name}().deserialize(input_stream, TRIM_SIZE=TRIM_SIZE)"
...
def converter(ark_json_schema, type_identifier, recursive_call=False):
    ...
    class_def += """\tdef deserialize(self, input_stream, TRIM_SIZE):\n"""

    if not is_final:
        class_def += "\t\thas_more_sections = read_bitstream_header(input_stream, TRIM_SIZE=TRIM_SIZE)\n"
    for field in schema["fields"]:
        ...
        if (
            ...
            class_def += (
                f"\t\tread_{field['type']}(input_stream, TRIM_SIZE=TRIM_SIZE)\n"
            )
            continue
        ...
        if (
            ...
            class_def += f"\t\tif read_bool(input_stream, TRIM_SIZE=TRIM_SIZE):\n\t\t\tself.{field['name']} = read_{field['type']}(input_stream, TRIM_SIZE=TRIM_SIZE)\n"
            continue
    ...
    if not is_final:
        ...
        class_def += "\t\t\thas_more_sections, (group_number, group_data_length) = read_optional_group_header(input_stream, TRIM_SIZE=TRIM_SIZE)\n"
        for group in schema["groups"]:
            ...
            for field in group["fields"]:
                ...
                if (
                    ...
                    class_def += f"\t\t\t\tread_{field['type']}(input_stream, TRIM_SIZE=TRIM_SIZE)\n"
                    continue
                # for optional fields we read a bool and if its true we read the value
                if (
                    "attributes" in field
                ...
                class_def += f"\t\t\t\tself.{field['name']} = {read_statement}\n"
            class_def += "\t\t\t\tcontinue\n"
        class_def += "\n\t\t\tconsume(input_stream, group_data_length)\n"
