...
class RosMessageParserVisitor(ParseTreeVisitor):
    ...
    def visitRos_type(self, ctx: RosMessageParser.Ros_typeContext):
        ...
        return f"read_object(type='{class_type}', stream=stream, package=self.package,TRIM_SIZE=TRIM_SIZE)"
...
class RosMessageParserVisitorForJsonSchema(ParseTreeVisitor):
    ...
    def visitRos_type(self, ctx: RosMessageParser.Ros_typeContext):
        ...
        if class_type == "Header":
            ...
            class_type = "std_msgs/Header"
        # return class_type
        if class_type not in self.class_registry:
            # print(f"adding {class_type} to registry")
            self.class_registry[class_type] = {}
