...
def dict_to_model(data, model_name='DynamicModel'):
    ...
    for changes in watch(*paths):
        ...
        try:
            if file.endswith('env') or file.startswith('env.'):  # todo: 可增加其他条件
                ...
                os.environ['TOML_CONFIG'] = str(toml.load(open(file)))
            elif file.endswith(('.yaml', '.yml')):
                os.environ['YAML_CONFIG'] = str(yaml.safe_load(open(file)))
            elif file.endswith('.json'):
                os.environ['JSON_CONFIG'] = str(json.load(open(file)))
