...
class NeffSolver(FiberSolver):
    ...
    def __call__(self, wavelength: float, mode: Mode, delta: float, lowbound: float):
        ...
        if lowbound is None or isnan(lowbound):
            ...
            if pm:
                lowbound = self.fiber.get_effective_index(pm, wavelength, delta)
                if isnan(lowbound):
                    return lowbound
            else:
                layers_maximum_index = [
                    layer.get_maximum_index(wavelength) for layer in self.fiber.layers
                ]
            ...
            if mode.family is ModeFamily.LP and mode.nu > 0:
                ...
                lb = self.fiber.get_effective_index(pm, wavelength, delta)
                if isnan(lb):
                    return lb
    ...
    def _lpfield(self, wavelength: float, nu: int, neff: float, radius: float) -> tuple:
        N = len(self.fiber)
        C = numpy.array((1, 0))
