...
class NessusReport(BaseModel):
    ...
    def process_nessus_files(regscale_ssp_id: int, folder_path: Path) -> None:
        ...
        def move_files() -> None:
            ...
            for file in file_collection:
                ...
                processed_dir = Path(file).parent / "processed"
                ...
                try:
                    ...
                    logger.debug("File %s already exists in %s", file, processed_dir)
        ...
        for iss in issues:
            if iss.id:
                existing_links = Link.fetch_links_by_parent(
                    app=app, regscale_id=iss.id, regscale_module="issues"
                )
                if iss.pluginId not in {li.title for li in existing_links}:
                    # Create Link
                    link = Link(
    ...
    def create_issues(
        ...
        for item in filter_severity(list_of_vulns, minimum_severity):
            ...
            kev, kev_due_date = lookup_kev(item.cve)
            poam = severity.lower() in ["moderate", "high", "critical"]
            iss = Issue(
                isPoam=poam,
                title=title,
                description=description,
                ...
                cve=item.cve,
                kevList=kev["cveID"] if kev else None
            ...
            iss.sourceReport = item.extra_data["report_uuid"]
