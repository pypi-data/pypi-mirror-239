...
class SDTcloud():
    ...
    def getDataFromMeasurement(self, bucket, measurement, startTime="-1h", stopTime="now()"):
        if self.influxdb_client is None:
            raise AttributeError('Please initInfluxDB function run!!')
    ...
    def getTablesFromTimeScaleDBConn(self):
        ...
        try:
            ...
            result = self.timescaledb_conn.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public';")
    ...
    def showColumsFromTable(self, database, table):
        if self.timescaledb_conn is None:
            raise AttributeError('Please initTimeScaleDB function init!!')
    
        try:
            # cur = self.timescaledb_conn.cursor()
            results = self.timescaledb_conn.execute(f'SELECT * FROM information_schema.columns where table_catalog={database} and table_name={table}')
            column_list = [row.column_name for row in results]
            print(f'Database : {database},\nCollection : {table},\nColumn List : {column_list} \n')
        except Exception as e:
            raise Exception(e)

    def getDataFromTimeScaleDB(self, database, table):
        if self.timescaledb_conn is None:
            raise AttributeError('Please initTimeScaleDB function init!!')
        ...
        try:
            ...
            cur.execute(f'SELECT * FROM {database}.public.{table};')
        except Exception as e:
            raise Exception(e)
        finally:
            cur.close()
    ...
    def showColumnsFromCollection(self, database, collection):
        if self.mongodb_client is None:
            raise AttributeError('Please initMongoDB function run!!')
    ...
    def getDataFromMongoDBClient(self, database, collection):
        if self.mongodb_client is None:
            raise AttributeError('Please initMongoDB function run!!')
    ...
    def showColumnsFromTable(self, keyspace, table):
