...
class SDTcloud():
    ...
    def getDataFromMeasurement(self, bucket, measurement, query_condition):
        ...
        start_time = query_condition.get('start_time')
        stop_time = query_condition.get('stop_time')

        limit_count = query_condition.get('limit_count')
        field_list = query_condition.get('field_list')

        # range query 조건
        range_query = '|> range(start: {}, stop: {})'
        if start_time and stop_time:
            range_query = range_query.format(start_time, stop_time)
        else:
