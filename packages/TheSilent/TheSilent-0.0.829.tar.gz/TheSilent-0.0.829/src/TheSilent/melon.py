...
def melon(host,delay=0,crawl=1):
    clear()
    hits = []
    ...
    hosts = kitten.kitten_crawler(host,delay,crawl)

    for _ in hosts:
        _.rsplit("/")
        try:
            forms = re.findall("<form.+form>",puppy.text(host).replace("\n",""))

        except:
            forms = []

        # check for command injection
        print(CYAN + f"checking for command injection on: {_}")
        for mal in mal_command:
            time.sleep(delay)
            try:
                start = time.time()
                data = puppy.text(host + "/" + mal)
                end = time.time()
                if end - start >= 45:
        ...
        print(CYAN + f"checking for python injection on: {_}")
        ...
        # check for xss
        print(CYAN + f"checking for xss on: {_}")
        for mal in mal_xss:
            time.sleep(delay)
            try:
                data = puppy.text(host + "/" + mal)
    ...
    if len(hits) > 0:
        ...
        print(f"we didn't find anything interesting on {host}")
