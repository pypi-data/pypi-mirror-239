...
class GptManager:
    ...
    def initialize_keys(self):
        ...
        input(self.instruction_pre_keys)
...
##response hanling
    def update_text_with_responses(self):
        output_keys = []
        self.last_content = get_any_value(self.current_output,'query_response')
        ...
        if self.last_api_response: 
            ...
            self.window_mgr.update_value('-RESPONSE-',f"#TITLE#{self.get_new_line(1)}{self.last_title}{self.get_new_line(2)}#USER QUERY#{self.get_new_line(1)}{self.last_request}{self.get_new_line(2)}#{self.last_model} RESPONSE#{self.get_new_line(2)}{self.last_api_response}{self.get_new_line(2)}")
        else:
