...
class Classification(FileUtils):
    ...
    def train(self,base_path,model_name='resnet50', model_id=0, cuda=True):
        ...
        logging.info('pipeline evaluation is done! return result in json format...')
        ...
        self.delete_id(base_path, model_id)
    ...
    def delete_id(self, base_path, model_id):
        '''
        notes: delete related dataset, log, checkpoint after evaluation
        '''
        import shutil        
        os.makedirs('modelfile', exist_ok=True)
        os.makedirs(os.path.join(base_path, f'history_{self.model_id}'), exist_ok=True)
        ...
        shutil.copyfile(os.path.join(base_path, f'checkpoint/{model_id}/best_epoch_weights.pth'), 
                        f'modelfile/{self.model_id}.pth')
        
        shutil.rmtree(os.path.join(base_path, f'dataset/{model_id}'))
        shutil.rmtree(os.path.join(base_path, f'log/{model_id}'))
