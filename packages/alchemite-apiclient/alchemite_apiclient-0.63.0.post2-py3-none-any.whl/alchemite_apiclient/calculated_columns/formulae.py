...
def weighted_sum(
    ...
    if constant is None:
        ...
        return sum_([column(i) * literal(j) for i, j in zip(cols, weights)]) + literal(
            constant
        )
...
def weighted_mean(cols: List[str], weights: List[float]) -> _CCE:
    ...
    Returns: CalculatedColumnExpression
    """
    return sum_([column(i) * literal(j) for i, j in zip(cols, weights)]) / literal(sum(weights))
...
def weighted_col_sum(summand_cols: List[str], weight_cols: List[str]) -> _CCE:
    ...
    Returns: CalculatedColumnExpression
    """
    return sum_([column(i) * column(j) for i, j in zip(summand_cols, weight_cols)])


def weighted_inverse_col_sum(summand_cols: List[str], weight_cols: List[str]) -> _CCE:
    """
...
def weighted_col_mean(summand_cols: List[str], weight_cols: List[str]) -> _CCE:
    ...
    Returns: CalculatedColumnExpression
    """
    return sum_([column(i) * column(j) for i, j in zip(summand_cols, weight_cols)]) / sum_([column(j) for j in weight_cols])
