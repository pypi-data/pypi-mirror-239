...
def assert_if_op(res: CCE, test, val1, val2):
    ...
    assert [x.to_dict() for x in res.get_arguments()] == [test.to_dict(), val1.to_dict(), val2.to_dict()]
    assert res.to_dict() == {"if": test.to_dict(), "then": val1.to_dict(), "else": val2.to_dict()}
...
@pytest.mark.parametrize("input_dict,error", [
    pytest.param({"ln": [{"lit": [2.0]}]}, None, id="valid_float"),
    pytest.param({"ln": [{"lit": [np.float32(2.0)]}]}, None, id="valid_exotic"),
    ...
    pytest.param({"ln": [{"lit": [2.0, 3.0]}]}, ValueError, id="multi_float"),
    pytest.param({"ln": [{"lit": [2.0]}], "sum": [{"lit": [2.0]}]}, ValueError, id="multiple_keys"),
    pytest.param({"if": {"lit": [True]}, "then": {"lit": [2.0]}, "else": {"ref": ["a"]}}, None, id="valid_if"),
    pytest.param({"if": {"lit": [True]}, "then": {"lit": [2.0]}}, ValueError, id="partial_if"),
    pytest.param({"then": {"lit": [2.0]}}, ValueError, id="just_then"),
    pytest.param({"if": {"lit": [True]}, "than": {"lit": [2.0]}, "else": {"ref": ["a"]}}, ValueError, id="if_typo"),
    pytest.param({"if": {"lit": [True]}, "then": {"lit": [2.0]}, "alse": {"ref": ["a"]}}, ValueError, id="if_typo2"),
    pytest.param({"if": [{"lit": [True]}], "then": [{"lit": [2.0]}], "else": [{"ref": ["a"]}]}, ValueError,
                 id="listed_if"),
])
...
@pytest.mark.parametrize("val1,val2,flipped", [
...
@pytest.mark.parametrize("args,error", [
    ...
    pytest.param([cc.column("a"), {"lit": [2.0]}], TypeError, id="raw_dict"),
