...
class APIManagementLocal( GenericCRUD):
    ...
    def cheack_limit (api_type_id:int,time:int)->int:
        logger.start(object={'api_type_id':str(api_type_id),'time':str(time)})
    ...
    def try_to_call_api(api_type_id:int, endpoint:str,outgoing_body:str,outgoing_header:str,time:int)->str:
        ...
        cheack=APIManagementLocal.cheack_limit(api_type_id,time)
        connection = Connector.connect("api_call")
        cursor = connection.cursor()
        try:
                ...
                if cheack==1:
                      ...
                      outgoing_body_significant_fields_hash = APIManagementLocal._get_json_with_only_sagnificant_fields_by_api_type_id(json1=json.dumps(data), api_type_id=str(api_type_id))
    ...
    def  delete_api(api_type_id:int,time:int,data:str):
        logger.start(object={'api_type_id':str(api_type_id),'time':str(time)})
        try:
            cheack_limit=APIManagementLocal.cheack_limit(api_type_id,time)
    ...
    def  get_api(api_type_id:int,time:int,data:str):
        logger.start(object={'api_type_id':str(api_type_id),'time':str(time)})
        try:
            cheack_limit=APIManagementLocal.cheack_limit(api_type_id,time)
            data_j=json.loads(data)    
    ...
    def  put_api(api_type_id:int,time:int,data:str):
        logger.start(object={'api_type_id':str(api_type_id),'time':str(time)})
        try:
            cheack_limit=APIManagementLocal.cheack_limit(api_type_id,time)
            data_j=json.loads(data)    
