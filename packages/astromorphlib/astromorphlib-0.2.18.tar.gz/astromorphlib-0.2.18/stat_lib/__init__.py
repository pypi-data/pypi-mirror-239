...
def phot_mod (gal, tab_gal, conn, size_image_phy=50, snr_g = 2.0,
    ...
    #     print (Fore.RED+ '\nThe folder "tables" was created')
    #
    # path = 'tables/{}_{}_fwhm.tab'.format(Field, band)
    # if os.path.exists(path):
    #     tab_fwhm = Table.read(path, format='ascii')
    #     print (f'\nThe table {path} already exist')
    # else:
    #     query_txt1 = (f"SELECT det.ID, det.ID_RA, det.ID_DEC, det.A_{band}, det.B_{band}, " +
    #                   f"det.Theta_{band}, det.KRON_RADIUS_{band}, det.{band}_auto, " +
    #                   f"det.FIELD, det.FWHM_{band},  " +
    #                    "claf.CLASS, claf.model_flag, claf.PROB_GAL, " +
    #                    "claf.PROB_QSO, claf.PROB_STAR " +
    #                   f"FROM idr4_single.idr4_single_{band} as det join " +
    #                    "idr4_vacs.idr4_star_galaxy_quasar as claf on " +
    #                    "claf.ID = det.ID WHERE det.FIELD = '" +  str(Field) +
    #                    "' and claf.model_flag='0' and claf.CLASS=1 and "
    #                   f"claf.PROB_STAR > 0.8 and det.{band}_auto >13 and det.{band}_auto<17 ")
    #
    #     print ("\nDoing the following query ...\n", query_txt1)
    #     tab_fwhm=conn.query(query_txt1)
    #
    #     tab_fwhm.write(f'tables/{Field}_{band}_fwhm.tab', format='ascii')

    print(Fore.BLACK + '\n\nObserved Seeing of the Field')
    print('--------------------------------')
    ...
    # psf_g= np.round(np.nanmedian(tab_fwhm['FWHM_'+splus_filters[band]])*3600,1)
    # print ('\nNumber of star 13<R_auto<17 : {}'.format(len(tab_fwhm['Field'])))
    ...
    plt.savefig('images/fig_{}_{}_{}kpc_sky_mask.png'.format(gal, band,
                size_image_phy), bbox_inches='tight')
    if show_plot=='yes' and run_auto=='no':
        plt.show()
    else:
        plt.close()
    print(('Saveving images/fig_{}' +
          '_{}_{}kpc_sky_mask.png').format(gal, band, size_image_phy))
    ...
    plt.savefig('images/fig_{}_{}_{}kpc_sky.png'.format(gal, band,
                size_image_phy), bbox_inches='tight')
    if show_plot=='yes' and run_auto=='no':
        plt.show()
    else:
        plt.close()
    print (('Saving images/fig_{}' +
           '_{}_{}kpc_sky.png').format(gal, band, size_image_phy))
    ...
    if user_order is None:
        ...
        for i, gali in enumerate(sorta[::-1]):
           ...
           plot_stat(morph, gal+listl[i], imag, imagm, median_sky_g, area,t, zp,
                     ...
                     petro_extent_cas=petro_extent_cas, w=w,
                     tab_gal=tab_gal, idg=idg)
    else:
       if user_claf is None:
...
def plot_stat(morph, gal, imag, imagm, median_sky_g, area, t, zp, psfg, psf_g,
              ...
              petro_extent_cas=1.5, w=None, tab_gal=None, idg=0):
...
def ex_phot_mod (gal, tab_gal, conn, size_image_phy=50, snr_g = 2.0, areagal=10,
    ...
    if run_auto=='no':
        plot_simbad(gal, tab_gal, conn, size_image_phy=radsimbad,
                    ...
                    print_scale='no', usesimbad=usesimbad, band=band)
    ...
    if run_auto!='yes':
        ...
        part1 = f""" snr_g = {snr_g}, sky_box = {sky_box},\n deblend ="{deblend}", """
        part2 = f"""user_order=[1,2], user_list=["A","B"], band='{band}', """
        part3 = f"""segmap='{segmap}', mask_stars='{mask_stars}', """
        part4 = f"""petro_extent_cas='{petro_extent_cas}', eta='{eta}', """
        part5 = f"""outputfile='{outputfile}')"""
        print(part0 + part1 + part2 + part3 + part4 + part5)
        ...
        part3 = f"""segmap='{segmap}', mask_stars='{mask_stars}', """
        part4 = f"""petro_extent_cas='{petro_extent_cas}', eta='{eta}', """
        part5 = f"""outputfile='{outputfile}', show_plot='{show_plot}', """
        part6 = f"""run_auto='yes')"""
        print(part0 + part1 + part2 + part3 + part4 + part5 + part6)
...
def simbad_thumbnail(GAL, tab_gal='simbad', size_image_phy=100,  width = 300,
...
def ex_phot_mod_decals (gal, tab_gal='simbad', size_image_phy=50, snr_g = 2.0,
                        ...
                        outputfile='properties.dat', dr=10, petro_extent_cas=1.5,
    ...
    print (Fore.BLUE +'image size in pix: {:.1f}'.format(size_image))
    #area_min=(((np.square(areagal)*np.pi)/arc_kpc**2)/0.55**2)
    area_min=areagal/arc_kpc**2/sc_ima**2
    area_min_deblend=areagal_deblend/arc_kpc**2/sc_ima**2
    ...
    if run_auto == 'no':
        ...
        part5 = f"""eta={eta}, mask_stars='{mask_stars}', """
        part6 = f"""simbad_cone={simbad_cone}) """
        print(part0 + part1 + part2 + part3 + part4 + part5 + part6)
    else:
        if segmap!=None:
            segmc = fits.open(segmap)[0].data
        ...
        part4 = f"""segmap='{segmap}', petro_extent_cas={petro_extent_cas}, """
        part5 = f"""eta={eta}, mask_stars='{mask_stars}', run_auto='yes', """
        part6 = f"""simbad_cone={simbad_cone}) """
        print(part0 + part1 + part2 + part3 + part4 + part5 + part6)

        phot_mod_decals(gal, tab_gal, size_image_phy=size_image_phy,
                        snr_g = snr_g, band=band, sky_box = sky_box,
                        deblend =deblend, user_order=[maxarea],
                        user_list=[band], show_plot=show_plot,
                        outputfile=outputfile,
                        petro_extent_cas=petro_extent_cas, eta=eta,
                        segmap=segmap, mask_stars = mask_stars,
                        run_auto='yes', simbad_cone=simbad_cone)


def phot_mod_decals (gal, tab_gal, size_image_phy=50, snr_g = 2.0,
                     band = 'r',
                     ...
                     petro_extent_cas=1.5, segmap=None, mask_stars='no',
                     run_auto='no', simbad_cone=2/3600.):
    ...
    patht = 'fits_images/' + gal +'_{}_{}kpc.fits'.format(
            band, size_image_phy)
    imagn = fits.open(patht)
    imag = imagn[0].data
    try:
        w =  wcs.WCS(imag.header)
    except:
        w = None
