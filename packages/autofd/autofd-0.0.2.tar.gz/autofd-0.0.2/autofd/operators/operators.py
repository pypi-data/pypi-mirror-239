...
def def_operator(prim, impl, spec, multiple_results=False):
  ...
  def _abstract_eval(*args, **kwargs):
    s = spec(*args, **kwargs)
    return (
      tuple(map(GeneralArray, s)) if multiple_results else GeneralArray(s)
    )
...
def _normalize_support(f):
  arg_spec = SpecTree.from_args(f)
  ret = []
  for spec, supp in zip(arg_spec, f.support):
    if supp is None:
      ret.append(None)
    else:
      ret.append(
        tree_map(lambda su, sp: su.reshape((-1, *sp.shape)), supp, spec)
      )
  return tuple(ret)
...
def permute_args_spec(f, *, perm):
  return (f.ret, *(f.arg[i] for i in perm))
...
def split_spec(f):
  ...
  return tuple((ret, *f.arg) for ret in map(Ret, f.ret.spec))


split_p = core.Primitive("split")
def_operator(split_p, split_impl, split_spec, multiple_results=True)
...
def linear_transpose_impl(f, *, argnums, unpack):
  ...
  def transposed_f(*args):
    ...
    partial_f = partial_impl(f, *bound_args, argnums=bound_argnums)
...
def linear_transpose_spec(f, *, argnums, unpack):
  ...
  if unpack:
    assert isinstance(f.ret.spec, tuple)
    return (ret, *map(f.arg.__getitem__, c_argnums), *map(Arg, f.ret.spec))
  else:
    return (ret, *map(f.arg.__getitem__, c_argnums), f.ret)
...
def integrate(f, *, argnums=0, dummy=False):
  ...
  """
  return integrate_p.bind(f, argnums=argnums, dummy=dummy)


def _integrate_impl(f, *, argnums, dummy):
  argnums = (argnums,) if isinstance(argnums, int) else argnums
  c_argnums = complement_argnums(argnums, f.num_args)
  if len(c_argnums) == 0 and dummy:
    return tree_map(lambda s: jnp.zeros(s.shape, s.dtype), f.ret.spec)
  if num_args(f) == 0:
    return f()
  else:
    raise ValueError(f"Cannot integrate {f} over argnums={argnums}.")
...
def integrate_p_abstract_eval(f, *, argnums, dummy):
  cargnums = complement_argnums(argnums, f.num_args)
  rest_spec = tuple(map(lambda idx: f.arg_spec[idx], cargnums))
  ...
  if len(rest_spec) == 0:
    ...
    return GeneralArray((Ret(SpecTree.from_ret(f)), *map(Arg, rest_spec)))


def _integrate_transpose_rule(t, f, *, argnums, dummy):
  assert ad.is_undefined_primal(f)
  if is_function(t):
    return broadcast_arg_to(t, SpecTree.from_args(f), argnums)
  else:
...
def compose_spec(f, *gs, share_inputs):
  ...
  if share_inputs:
    ...
    return (f.ret, *gs[0].arg)
  return (f.ret, *(a for g in gs for a in g.arg))
...
def _compose_jvp_rule(primals, tangents, *, share_inputs):
  ...
  tangents_out.append(
    compose_p.bind(
      ...
      *split_p.bind(zip_functions(*gs, share_inputs=share_inputs)),
      *split_p.bind(zip_functions(*nonzero_gs_dot, share_inputs=share_inputs)),
      share_inputs=True,
    )
  )
...
def nabla_spec(f, *, argnums, method):
  ...
  return (Ret(jspec), *f.arg)


nabla_p = core.Primitive("nabla")
def_operator(nabla_p, nabla_impl, nabla_spec)
...
def linearize_impl(f, argnums):
  ...
  def linearized_f(*args):
    ...
    bound_argnums = complement_argnums(argnums, f.num_args)
    unbound_args = map(primals.__getitem__, argnums)
    bound_args = map(primals.__getitem__, bound_argnums)
    partial_f = partial_impl(f, *bound_args, argnums=bound_argnums)
...
def linearize_spec(f, argnums):
  return (f.ret, *f.arg, *map(f.arg.__getitem__, argnums))


linearize_p = core.Primitive("linearize")
def_operator(linearize_p, linearize_impl, linearize_spec)
...
def identity(*, arg_spec):
  return identity_p.bind(arg_spec=arg_spec)


def _identity_impl(*, arg_spec):
  ret_spec = arg_spec[0] if len(arg_spec) == 1 else arg_spec

  @with_spec(arg_spec, ret_spec)
  def _identity(*args):
    return args[0] if len(arg_spec) == 1 else args

  return _identity


identity_p = core.Primitive("identity")
identity_p.def_impl(_identity_impl)


@identity_p.def_abstract_eval
def _identity_abstract_eval(*, arg_spec):
  ret_spec = arg_spec[0] if len(arg_spec) == 1 else arg_spec
  return GeneralArray((Ret(ret_spec), *map(Arg, arg_spec)))


def identity_like(value):
  return identity(arg_spec=SpecTree.from_value(value))


def constant(value, *, arg_spec=None):
  return constant_p.bind(value, arg_spec=arg_spec)
...
def _constant_impl(
  ...
  *,
  ...
  ret_spec = SpecTree.from_value(value)
...
constant_p.def_impl(_constant_impl)


@constant_p.def_abstract_eval
def _constant_abstract_eval(
  value,
  *,
  arg_spec=None,
):
...
def partial(f, *args, argnums=0):
  argnums = (argnums,) if isinstance(argnums, int) else argnums
  return partial_p.bind(f, *args, argnums=argnums)


def partial_spec(f, *args, argnums):
  assert len(argnums) == len(args)
  c_argnums = complement_argnums(argnums, f.num_args)
  return (f.ret, *(f.arg[i] for i in c_argnums))
...
def _partial_transpose_rule(t, f, *args, argnums):
  r"""Suppose we have a function $f(x, y)$, now we want to
  transpose $\hat{B}(f, x)$ wrt $x_1$. Where $x_1$ is a part of $x$.
  .. math::
    T_{x_1}(\hat{B}):
    h \mapsto \hat{C}(\hat{T}_{x_1}(f), \mathrm{Const}(x_2), I, h)
  """
  argnums_tuple = (argnums,) if isinstance(argnums, int) else argnums
  cargnums = complement_argnums(argnums, num_args(f))

  if ad.is_undefined_primal(f):
    wrt_f = broadcast_arg_to(t, SpecTree.from_args(f), cargnums)
    support = [None] * num_args(f)
    for idx, arg in zip(argnums, args):
      support[idx] = arg
      wrt_f.support = tuple(support)
  else:
    wrt_f = None

  t_args, t_index_in_f, t_index_in_args = [], [], []  # transposed args
  unt_args, unt_index_in_f = [], []  # untransposed args
  ret = [wrt_f] + [None] * len(args)
  for j, (i, a) in enumerate(zip(argnums_tuple, args)):
    if ad.is_undefined_primal(a):
      t_index_in_f.append(i)
      t_index_in_args.append(j)
      t_args.append(a)
    else:
      unt_index_in_f.append(i)
      unt_args.append(a)
  # argnums of x_1
  argnums_t = tuple(t_index_in_f)
  unt_args, unt_index = tuple(unt_args), tuple(unt_index_in_f)
  perm = np.argsort(unt_index + cargnums)
  perm = (*perm, len(perm))
  t_arg_spec = SpecTree.from_args(t)
  rest_args = identity(arg_spec=t_arg_spec)
  rest_args = (rest_args,) if len(t_arg_spec) == 1 else split_p.bind(rest_args)

  perm_tf = permute_args_p.bind(
    linear_transpose(f, argnums=argnums_t), perm=perm
  )
  # this call mixes partial and compose, unt_args are values,
  # rest_args and t are functions.
  wrt_t_args = perm_tf(
    *unt_args,
    *rest_args,
    t,
  ...
  wrt_t_args = tuple(
    integrate(wrt_t_arg, argnums=tuple(range(num_args(wrt_t_arg))))
    for wrt_t_arg in split_p.bind(wrt_t_args)
  )
  for i, a in zip(t_index_in_args, wrt_t_args):
    ret[1 + i] = a
  return tuple(ret)


def _partial_jvp_rule(primals, tangents, *, argnums):
  f, args = primals[0], primals[1:]
  nargs = num_args(f)
  f_dot, args_dot = tangents[0], tangents[1:]
  jvps = []
  if not isinstance(f_dot, ad_util.Zero):
    jvp1 = partial(f_dot, *args, argnums=argnums)
    jvps.append(jvp1)
  argnums_tuple = (argnums,) if isinstance(argnums, int) else argnums
  nonzero_args_dot, nonzero_argnums = [], []
  for i, arg_dot in zip(argnums_tuple, args_dot):
    if not isinstance(arg_dot, ad_util.Zero):
      nonzero_args_dot.append(arg_dot)
      nonzero_argnums.append(i)
  argnums_jvp = (*argnums_tuple, *range(nargs, nargs + len(nonzero_argnums)))
  lf = linearize_p.bind(f, argnums=nonzero_argnums)
  jvp2 = partial(
    lf,
    *args,
    *nonzero_args_dot,
  ...
  jvps.append(jvp2)
  return partial(f, *args, argnums=argnums), sum(jvps)
...
def invoke(f, *args):
  ...
  return integrate(partial(f, *args, argnums=tuple(range(len(args)))))
...
def binary_op(op, f1, f2, **params):
  ...
  if not is_function(f1) and not is_function(f2):
    ...
    if isinstance(f2, (tuple, list)):
      return compose(lambda x: tree_map(op, x, f2), f1)
    else:
      return compose(lambda x: tree_map(lambda y: op(y, f2), x), f1)
  elif is_function(f2) and not is_function(f1):
    if isinstance(f1, (tuple, list)):
      return compose(lambda x: tree_map(op, f1, x), f2)
    else:
      return compose(lambda x: tree_map(lambda y: op(f1, y), x), f2)
  else:
    ...
    if is_pytree(ret_f1) and not is_pytree(ret_f2):
      return compose(
        lambda tree, y: tree_map(lambda x: op(x, y), tree),
        f1,
        f2,
        share_inputs=True,
      )
...
def unary_op(op, f, **params):
  ...
  return compose(lambda x: tree_map(op, x), f)
...
def broadcast_arg_to(f, target_spec, argnums):
  """Broadcast a function with arguments

  .. code-block:: python

    def broadcasted_f(*args):
      return f(*args[argnums])

  Args:
    f: function to be broadcasted.
    target_spec: the target spec to be broadcasted to.
    argnums: the argument index of f's arguments in the target spec.
  Returns:
    A function that takes the same arguments as specified by target_spec.
  """
  arg_spec = SpecTree.from_args(f)
  rest_index = complement_argnums(argnums, num_args(f))
  selected_spec = map(target_spec.__getitem__, argnums)
  rest_spec = map(target_spec.__getitem__, rest_index)

  if arg_spec != selected_spec:
    raise ValueError(
      f"Cannot broadcast function {f} with spec {arg_spec}, "
      f"to target spec {target_spec} with argnums {argnums}."
    )

  @with_spec(rest_spec, Spec((), "f"))
  def zero(*_):
    return 0.

  @with_spec((SpecTree.from_ret(f), Spec((), "f")), SpecTree.from_ret(f))
  def ignore_second(arg1, _):
    return arg1

  perm = np.argsort(argnums + rest_index).tolist()
  return permute_args_p.bind(
    compose(ignore_second, f, zero, share_inputs=False), perm=perm
  )
...
def call(obj, *args, **params):
  ...
  # then compose with the function arguments
    ...
    f = partial(obj, *non_function_args, argnums=tuple(non_function_index))
