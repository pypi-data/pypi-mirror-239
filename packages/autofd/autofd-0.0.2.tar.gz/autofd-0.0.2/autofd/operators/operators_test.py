...
class _TestConcatSplit(absltest.TestCase):
  ...
  def test_split_nabla(self):
    ...
    def F1(f):
      ...
      return integrate(fa1 * fb1, dummy=True, argnums=(0, 1))

    def F2(f):
      fa2, fb2 = nabla(f, argnums=0), nabla(f, argnums=1)
      return integrate(fa2 * fb2, dummy=True, argnums=(0, 1))
...
class _TestCompose(absltest.TestCase):
  ...
  def test_higher_order_el(self):
    ...
    def F(f):
      ...
      return integrate(If, dummy=True, argnums=0)
  ...
  def test_euler_lagrange(self):
    ...
    def I(f):
      return integrate(f**2 + nabla(f)**2, dummy=True)
...
class _TestNabla(parameterized.TestCase):
  ...
  def test_nabla_nabla(self):
    ...
    def F(f):
      ...
      return integrate(dfdf, dummy=True)

    def FF(f):
      ddfddf = compose(lambda x: jnp.sum(x), nabla(nabla(f))**2)
      return integrate(ddfddf, dummy=True)
...
class _TestPartial(absltest.TestCase):
  ...
  def test_partial(self):
    ...
    tb.support = (b,)
    ta = jax.linear_transpose(
      lambda a: partial(f, a, c, argnums=(0, 2)),
      a,
    )(tb)[0]
  ...
  def test_partial_gradient(self):
    ...
    def func1(a, b, c):
      fab = partial(f, a, b, c, argnums=(0, 1, 2))

      @with_spec((fab.ret_spec,), Spec((), "f"))
      def reduce_sum(x):
        return jnp.sum(x)

      return integrate(compose(reduce_sum, fab))
