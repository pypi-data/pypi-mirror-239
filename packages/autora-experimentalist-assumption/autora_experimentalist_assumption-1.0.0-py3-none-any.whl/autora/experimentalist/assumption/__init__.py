...
def sample(X, y, model, num_samples, loss=True, theorist=None, confirmation_bias=False):
    """
    Assumption experimentalist challenges assumptions made by the theorist.
    ...
    Because it samples with respect to a Theorist, this experimentalist cannot be used on the first
    cycle
    ...
    if y is not None:
        if loss:
            ...
            idx = np.flip(loss_delta.argsort()[:num_samples])
    else:
        raise TypeError(
...
assumption_sample = sample

assumption_sampler = deprecated_alias(assumption_sample, "assumption_sampler")
