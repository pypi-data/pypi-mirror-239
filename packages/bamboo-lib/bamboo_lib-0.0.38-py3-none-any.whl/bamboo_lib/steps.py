...
class IngestClickhouseStep(PipelineStep):
    """
    Loads data to a ClickHouse server.
    
    Usage:
    ```
    load_step = IngestClickhouseStep(
        "table_name",
        ...
        pk=['year', 'dim_0', 'dim_1', 'geoid'],
        nullable_list=['mea', 'moe'],
    )
    ```
    """
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        """
        IngestClickhouseStep constructor.
    
        Args:
            `table_name` : str
                The name of the table where the data will be stored.
            `connector` : clickhouse_connect.driver.Client
                An instance of Client class, or one of its subclasses.
                Is the main method of connection with the ClickHouse server.

        Keyword Args:
            `if_exists` : Literal["drop", "append"]
                Directive to follow if the table intended for insertion already 
                exists.
            `dtypes` : dict[str, str]
                A mapping of the name of the columns to set in the table as key,
                and its ClickHouse Data Type as value.
                Refer to https://clickhouse.com/docs/en/sql-reference/data-types
            `pk` : list[str]
                List of Primary Keys to use in the ingested table.
                Specified values will only be considered if they appear as keys 
                in the `dtype` argument. The value must be a list, even if only
                one key is being specified.
            `nullable_list` : list[str]
                List of the column names whose Data Types are intended to be set
                as Nullable. Specified values will only be considered if they 
                appear as keys in the `dtype` argument.
                Avoid using Nullable types in ClickHouse, as they impact performance.
                Refer to https://clickhouse.com/docs/en/sql-reference/data-types/nullable
        
        """

        if if_exists not in ("drop", "append"):
            raise ValueError("Must specify valid `if_exists` directive: `drop` or `append`")

        if isinstance(pk, str):
            raise TypeError("Value for `pk` must be a list/tuple/set of strings")

        self.table_name = table_name
        self.connector = connector
        self.if_exists = if_exists
        self.dtypes = {
            column: ("Nullable({})" if column in nullable_list else "{}").format(dtype)
            for column, dtype in dtypes.items()
        }
        self.pk = tuple(column for column in pk if column in dtypes)

        super().__init__(**kwargs)


    def run_step(self, prev, params):
        logger.debug("Running IngestClickhouseStep")

        client = self.connector

        if self.if_exists == 'drop':
            logger.debug("- Dropping table '%s' if exists", self.table_name)
            client.command('DROP TABLE IF EXISTS {}'.format(self.table_name))

        sql_ingest = "CREATE TABLE "
        if self.if_exists == "append":
            sql_ingest += "IF NOT EXISTS "
        sql_ingest += '{table_name} '.format(table_name=self.table_name)
        sql_ingest += '({columns}) '.format(columns=", ".join(
            f"{key} {value}" 
            for key, value in self.dtypes.items()
        ))
        sql_ingest += 'ENGINE=MergeTree() '
        sql_ingest += 'PRIMARY KEY ({0}) ORDER BY ({0})'.format(', '.join(self.pk))

        logger.debug("- Ingesting %d rows to table '%s'", prev.shape[0], self.table_name)
        client.command(sql_ingest)
        client.insert(self.table_name, prev[list(self.dtypes.keys())])

        logger.debug("IngestClickhouseStep complete")
        return prev
