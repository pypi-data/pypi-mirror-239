...
class Pod:
    ...
    def __init__(
        ...
        if self.pod_db_connector is not None:
            for ds in self.base_datasources.values():
                ...
                if not isinstance(ds, DatabaseSource):
                    # self.base_datasources has only base sources, so it's safe to cast
                    self._update_pod_db(
                        self.pod_db_connector, ds.name, cast(BaseSource, ds.datasource)
                    )
    ...
    def _update_pod_db(
        self, connector: PodDbConnector, datasource_name: str, datasource: BaseSource
    ) -> None:
