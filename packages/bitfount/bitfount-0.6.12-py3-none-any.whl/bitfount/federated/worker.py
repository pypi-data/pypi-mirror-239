...
from bitfount.utils.pod_db_connector import PodDbConnector
...
class SaveResultsToDatabase(BaseProtocolHook):
    ...
    def on_run_end(
        ...
        if context == TaskContext.WORKER:
            ...
            connector: PodDbConnector = kwargs["connector"]
...
class _Worker:
    ...
    async def run(self) -> None:
        ...
        if self._pod_db_connector is not None and self.project_id is not None:
            project_db_con = self._pod_db_connector.get_project_db_connection(
                self.project_id
            )
        ...
        try:
            await worker_protocol.run(
                ...
                hook_kwargs={
                    ...
                    "table": table if self._pod_db_connector is not None else None,
                },
            )
