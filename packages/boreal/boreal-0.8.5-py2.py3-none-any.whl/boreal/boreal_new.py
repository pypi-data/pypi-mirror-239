...
def init_visualization(audio, srate):
    ...
    signal_plot = figure(width=400, height=200,
...
def update_visual_data(bokeh_pane, info_pane, audio_pane, mpos_source, duration_secs):
    k = 0
    ctime = 0.0; 
    block_duration = 0.05
    while(1):
        t0 = time.perf_counter()
        ctime  = float(audio_pane.param.time)
        info_pane.object = str(ctime)+str(type(ctime))
...
def time_domain_waveform(audio, srate):
    ...
    # player = pn.widgets.Player(name='Player', start=0, end=100, value=0, loop_policy='once')
    info_pane = pn.pane.Markdown(object='INIT')
    # player.interval = int(30.0 * 1000.0 / 100.0)


    # start visualization thread
    visualThread = Thread(target=update_visual_data, args=[bokeh_pane, info_pane, audio_pane, mpos_source, duration_secs])
    ...
    #    for event in events:
    #        if (event.name == 'value'):
    #            info_pane.object = str(audio_pane.time)
    #            if (np.fabs(event.new - event.old) > 2):
    #                new_time = (event.new / 100.0) * 30.0
    #                audio_pane.time = new_time 
    #                info_pane.object = "JUMP:" + str(new_time)
    #            pn.io.push_notebook(info_pane)
    #            ctime = audio_pane.time
    #            mpos_source.data = dict(xs = [[ctime,ctime,ctime]], ys=[[0.0, 1.0, -1.0]])
    #            pn.io.push_notebook(bokeh_pane)                
    ...
    #watcher = player.param.watch(partial(callback,audio_pane,audio_play), ['value','direction'], onlychanged=False)
    visualThread.start()
    display(info_pane)
    display(audio_pane)
    display(bokeh_pane)
...
def time_domain_waveform_holoviews(audio, srate):
    hv.notebook_extension("bokeh",logo=False)
    pn.extension()
    audio_pane = pn.pane.Audio(audio, sample_rate=srate, throttle = 20)
    display(audio_pane)
    
    def waveform_data(audio,srate): 
        def process(signal, hopSize, block_process):
            winSize = hopSize
            offsets = np.arange(0, len(signal), hopSize)
            amp_track = np.zeros(len(offsets))
        ...
        max_envelope = process(audio, 4096, lambda block: np.max(np.abs(block)))
        times = np.linspace(0, len(audio) / srate, num=len(max_envelope))    
        return (times,max_envelope)
  
    def interactive_play(x,y,t,*args):
        ...
        if (x is not None):
            ...
            t = x 
            audio_pane.time = x
            #label = hv.Points([], label='x: %0.3f, y: %0.3f, t: %0.3f' % (lx,ly,lt))
        return hv.VLine(t).opts(line_width=1,color='blue')


    tap_stream  = hv.streams.Tap(transient=True) # was single
    time_stream = Params(parameters=[audio_pane.param.time], rename={'time': 't'})
    time_slider = pn.widgets.FloatSlider(name='Time', value=0.0, start=0.0, end=30.0)

    dmap_time = hv.DynamicMap(interactive_play, streams=[tap_stream,time_stream])


    (times, max_envelope) = waveform_data(audio,srate)
    time = hv.Dimension('time', label='Time', unit='sec')
    pos_amplitude = hv.Dimension('pos_amplitude', range=(0.0,1.0),  label='Amplitude')
    neg_amplitude = hv.Dimension('neg_amplitude', range=(-1.0, 0.0), label="Neg Amplitiude")
    area_plot = hv.Area((times, max_envelope, -max_envelope),vdims = [pos_amplitude, neg_amplitude], kdims=[time]).opts(width=800,height=400,fontsize={'labels': 12},bgcolor="#eaeaea")                                                                         
    area_plot * dmap_time
    audio_view = pn.panel(area_plot * dmap_time)
    display(pn.Column(time_slider, audio_view))
