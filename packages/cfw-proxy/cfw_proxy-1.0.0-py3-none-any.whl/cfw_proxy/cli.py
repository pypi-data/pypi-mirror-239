...
def serve_proxy(args: argparse.Namespace):
    ...
    config.update({
        "ca_cert": os.path.realpath(args.ca_cert) or config.get("ca_cert"),
        "ca_key": os.path.realpath(args.ca_key) or config.get("ca_key"),
        "cert_cachedir": os.path.realpath(args.cert_cachedir) or config.get("cert_cachedir"),
        "port": args.port if args.port else config.get("port", 8843),
        "bind": args.bind or config.get("bind"),
...
def cli():
    ...
    argparser.add_argument("--ca-key", default="ca.key")
    argparser.add_argument("--ca-cert", default="ca.crt")
    argparser.add_argument("--cert-cachedir", default="cert_cache")
    ...
    serve_cmd.add_argument("-p", "--port", type=int, default=8843)
    serve_cmd.add_argument("-b", "--bind", default="127.0.0.1", help="bind address")
