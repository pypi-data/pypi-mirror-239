...
class RandomPILTransform(PILTransform):
...
class AutoContrast(TrivialPILTransform):
...
class Brightness(TrivialPILTransform):
    ...
    def __init__(self, magnitude: float) -> None:
        ...
        magnitude
            ...
            (For convert_img_pt) How much to adjust the brightness. Can be any non negative number.
            0 gives a black image, 1 gives the original image while 2 increases the brightness
            by a factor of 2
        """
        self.magnitude = magnitude

    def convert_img(self, img: Image) -> Image:
        return ImageEnhance.Brightness(img).enhance(self.magnitude)

    def convert_img_pt(self, img: torch.Tensor) -> torch.Tensor:
...
class RandomBrightness(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img: Image) -> Image:
        return ImageEnhance.Brightness(img).enhance(self._magnitude)
...
class Contrast(TrivialPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        return ImageEnhance.Contrast(img).enhance(self.magnitude)

    def convert_img_pt(self, img: torch.Tensor):
...
class RandomContrast(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        return ImageEnhance.Contrast(img).enhance(self._magnitude)
...
class Gamma(TrivialPILTransform):
...
class Saturation(TrivialPILTransform):
    __serialization_attributes__ = ["magnitude"]
...
class Color(TrivialPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def __init__(self, magnitude: float) -> None:
        ...
        ----------
        mangitude
            magnitude of the color balance adjustment, with 0.0 giving a black
            and white image and 1.0 giving the original image.
        """
        self.magnitude = magnitude

    def convert_img(self, img):
        return ImageEnhance.Color(img).enhance(self.magnitude)

    def set_magnitude(self, magnitude: float) -> None:
        self.magnitude = magnitude


class RandomColor(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def __init__(self, magnitude: float) -> None:
        ...
        ----------
        magnitude
            maximum magnitude of the color adjustment.

            Color adjusted by a factor between 1-magnitude and 1+magnitude.

            Required: magnitude must be >= 0.
        """
        if magnitude < 0:
            raise ValueError(
                "magnitude for color adjustment must be non-negative."
            )

        self.magnitude = magnitude

    def convert_img(self, img):
        return ImageEnhance.Color(img).enhance(self._magnitude)
...
class Sharpness(TrivialPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        return ImageEnhance.Sharpness(img).enhance(self.magnitude)
...
class RandomSharpness(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        return ImageEnhance.Sharpness(img).enhance(self._magnitude)
...
class Equalize(TrivialPILTransform):
    __serialization_attributes__ = []
...
class Posterize(TrivialPILTransform):
...
class RandomPosterize(RandomPILTransform):
    __serialization_attributes__ = ["bits"]
...
class Solarize(TrivialPILTransform):
    ...
    def __init__(self, threshold: Union[int, float]) -> None:
        ...
        ----------
        threshold
            should be between 0 and 256. all values above this greyscale threshold
            are inverted.
...
class RandomSolarize(RandomPILTransform):
    __serialization_attributes__ = ["threshold"]
    ...
    def __init__(self, threshold: Union[int, float]) -> None:
        ...
        if threshold < 0 or threshold > 256:
            raise ValueError("Solarizing threshold must be between 0 and 256.")
        self.threshold = threshold

    def set_random_params(self, img):
        self._threshold = np.random.uniform(low=self.threshold, high=256)

    def convert_img(self, img):
        return ImageOps.solarize(img, self._threshold)
...
class Invert(TrivialPILTransform):
    __serialization_attributes__ = []
...
class Cutout(RandomPILTransform):
    ...
    def set_random_params(self, img):
        center = np.array(
            [
                random.randint(0, img.height - 1),
                random.randint(0, img.width - 1),
            ]
        )
    ...
    def convert_img(self, img):
        ...
        img_draw.rectangle(
            [self.xmin, self.ymin, self.xmax, self.ymax], fill="black"
        )
        return img
...
class Rotation(RandomPILTransform):
    ...
    def convert_img(self, img):
        ...
        self._center = np.array([img.width / 2, img.height / 2])
...
class RandomVerticalFlip(RandomPILTransform):
...
class RandomHorizontalFlip(RandomPILTransform):
...
class RandomCrop(RandomPILTransform):
...
class RandomResize(RandomPILTransform):
    ...
    def set_random_params(self, img):
        ...
        if unif < self.p:
            ...
            self.size = (
                int(self.resize * img.size[1]),
                int(self.resize * img.size[0]),
            )
        else:
            self.resize = None
            self.size = None
...
class RandomRotation(RandomPILTransform):
...
class RandomPerspective(RandomPILTransform):
...
class TranslateX(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        self._translation = self._magnitude * img.width
        self._width = img.width
        return img.transform(
            img.size, Image.AFFINE, (1, 0, self._translation, 0, 1, 0)
        )
...
class TranslateY(RandomPILTransform):
    __serialization_attributes__ = ["magnitude"]
    ...
    def convert_img(self, img):
        self._translation = self._magnitude * img.height
        self._height = img.height
        return img.transform(
            img.size, Image.AFFINE, (1, 0, 0, 0, 1, self._translation)
        )
...
class RandomAffine(RandomPILTransform):
    ...
    def __init__(
        ...
        resample: Union[bool, int] = False,  # deprecated
        interpolation: Union[
            bool, int, F.InterpolationMode
        ] = InterpolationMode.NEAREST,
        fillcolor: Union[int, tuple] = 0,  # deprecated
        ...
        interpolation (InterpolationMode): Desired interpolation enum defined by
            ...
            but deprecated since Teddy 0.64.3 and will be removed soon! Please use InterpolationMode enum.
        fillcolor : tuple or int
            @deprecated use fill instead
            Optional fill color (Tuple for RGB Image And int for grayscale) for the area
        ...
        if isinstance(resample, int):
            warnings.warn(
                "Argument 'resample' is deprecated and will be removed soon. "
                "Please use argument 'interpolation' with value InterpolationMode enum."
            )
            interpolation = F._interpolation_modes_from_int(resample)
        elif isinstance(interpolation, int):
            warnings.warn(
                "Argument 'interpolation' of type int is deprecated and will be removed soon. "
    ...
    def convert_img(self, img):
        ...
        return F.affine(
            ...
            fill=self.fillcolor or self.fill,
        )
...
class ColorJitter(TrivialPILTransform):
    __serialization_attributes__ = [
        "brightness",
        "contrast",
        "saturation",
...
class Grayscale(TrivialPILTransform):
    __serialization_attributes__ = ["num_output_channels"]

    def __init__(self, num_output_channels: int = 1):
        self.num_output_channels = num_output_channels
        self.transform = tv_transforms.Grayscale(
...
class Compose(RandomPILTransform):
    """Composes several transforms together."""

    __serialization_attributes__ = ["transforms"]

    def __init__(self, transforms: List[PILTransform]):
        """
        Parameters
        ----------
        transforms : list
    ...
    def set_random_params(self, img):
        for t in self.transforms:
            if isinstance(t, RandomPILTransform):
                t.set_random_params(img)

    def convert_img(self, img):
        for t in self.transforms:
            img = t.convert_img(img)
        return img
...
class CenterCrop(PILTransform):
    ...
    def convert_img(self, img):
        ...
        self.center_pt = np.array([img.height / 2, img.width / 2])
...
class RandomGrayscale(RandomPILTransform):
    __serialization_attributes__ = ["p"]
    ...
    def set_random_params(self, img):
        self.transform = IdentityPILTransform()
        if random.random() < self.p:
...
class _Shear(RandomPILTransform):
    def __init__(
    ...
    def convert_img(self, img):
        self._height, self._width = img.height, img.width
        center = (img.size[0] * 0.5 + 0.5, img.size[1] * 0.5 + 0.5)
...
class Pad(PILTransform):
    ...
    def convert_img(self, img):
        return tv_transforms.functional.pad(
            img,
...
class RandAugment(RandomPILTransform):
    ...
    augmentations : list[tuple]
        ...
        transforms have no magnitude (e.g. AutoContrast, IdentityPILTransform)
    ...
    augmentations = [
        ...
        ("Identity", lambda x: IdentityPILTransform(), (0.0, 1.0)),
        ...
        ("Solarize", RandomSolarize, (255.0, 0.0)),
    ...
    def convert_img(self, *args, **kwargs):
        return self._transform.convert_img(*args, **kwargs)
