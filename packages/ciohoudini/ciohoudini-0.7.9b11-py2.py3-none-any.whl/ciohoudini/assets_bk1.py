...
def resolve_payload_old(node, do_asset_scan, **kwargs):
    """
    ...
    # logger.debug(">>> Checking if any of the assets are in the output folder: {}".format(out_folder))
    for path in path_list:
        logger.debug("Checking if {} is in {}".format(path, out_folder))
    ...
    logger.debug("Resolved payload: path_list:")
    for path in path_list:
        logger.debug(path)
...
def resolve_payload(node, do_asset_scan, **kwargs):
    ...
    # Add to current_assets all the dirs at the top level   that are in the hip file folder
    ...
    # print("filtered_paths after removing the hip folder: {}".format(filtered_paths))
...
def is_within_output_folder(path, output_folder):
    ...
    # print("Checking if {} is within {}, result: {}".format(normalized_path, normalized_output_folder, result))
    return normalized_path.startswith(normalized_output_folder)


# Usage of the `is_within_output_folder` function:
# is_within_output_folder("/path/to/file", "/output/folder")
