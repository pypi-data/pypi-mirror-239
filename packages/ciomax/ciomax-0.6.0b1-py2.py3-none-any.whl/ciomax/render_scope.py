...
class VraySWRenderScope(RenderScope):
    ...
    def get_render_package_paths():
        ...
        max_vray_paths = [p for p in path_list if k.VRAY_MAX_PREFIX in p]
...
class ArnoldRenderScope(RenderScope):
    ...
    def get_render_package_paths():
        ...
        max_arnold_paths = [p for p in path_list if k.ARNOLD_MAX_PREFIX in p]
...
class NativeRenderScope(RenderScope):
    ...
    def get_render_package_paths():
        ...
        path_list = RenderScope.get_compatible_software_paths()
        max_paths = [
            p for p in path_list if p.startswith(k.MAX_PREFIX) and not "/" in p
        ]
