...
class _PgCursor:
    ...
    def _make_connection_postgres(self):
        ...
        '''
        _PgCursor.connection = psycopg2.connect(host = self._host, 
                                      user = self._user, 
                                      password = self._password,
                                      database = self._database,
                                      options="-c search_path=" + self._schema, 
                                      port = self._port)
        _PgCursor.N_CONNECTION += 1
    ...
    def _fetch_data_from_cursor(self, curs, query, param_execute):
        try:
    ...
    def _execute_query(self, query, param_execute = None, check_batch = True):
        ...
        try:
            with _PgCursor.connection.cursor() as curs:
                curs.execute(query, param_execute)
                result = curs.fetchall()
        except psycopg2.InterfaceError as e:
            self._make_connection_postgres()
            with _PgCursor.connection.cursor() as curs:
                result = self._fetch_data_from_cursor(curs, query, param_execute)
    ...
    def _get_general_info(self, filter_by):
        ...
        sid_list = self._get_unique_values(['sid'], filter_by = filter_by)
        sid_list.sort()
        sid_count = len(sid_list)
        topic_list = self._get_unique_values(['topic'], filter_by = filter_by)
        topic_list.sort()
        topic_count = len(topic_list)
        message_count = self._get_counts('rid', filter_by = filter_by)
    ...
    def _get_topic_info(self, filter_by):
        ...
        message_count = self._get_counts('rid', group_by = ['topic'], filter_by = filter_by)
    ...
    def _data(self, data_names: list = None) -> pd.DataFrame:
        '''
    ...
    def _get_data_all(self, data_query = None, additional_columns = None, filter_by = None, order_by = None):
        ...
        #         self._get_unique_values(['topic', 'type'], filter_by = filter_by)
        #         type_names = ['']
    ...
    def data_for_time_plot(self, topic_name, var_name, time_step, sids, remove_nan, inf_vals):
        ...
        **kwargs
        ...
        df = self.topic(topic_var).sid(sids).set_order({'sid': 'asc', 'rid': 'asc'})._data(data_columns)
        if len(df) == 0:
