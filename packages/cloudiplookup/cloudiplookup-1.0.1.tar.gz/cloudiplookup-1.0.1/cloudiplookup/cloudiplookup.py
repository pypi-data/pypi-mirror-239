...
def update_ip_ranges_digital_ocean(download_url):
    ...
    try:
        ...
        databaseInfo["Digital Ocean"] = {'create_date':last_modified,
                                        'total_networks':len(cloudip.keys())-initialLen}
    except Exception as ERR:
...
def main_function():
    ...
    output.add_argument("--csv","-c",dest='csv',action="store_true",default=False,help="0|Print output in csv format (ip,cidr,region,cloud_provider,service,elapsed_time).")
    ...
    if (args.verbose == False):
        logVerbose.__code__ = _log_empty.__code__
