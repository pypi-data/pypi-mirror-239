...
class BaseCollFilter:
    ...
    def user_cf(self, size_per_user=5):
        ...
        """
        similar_dict = self._cal_similar(self.user_items, self.item_users, 'ucf')
        return self._do_cf(similar_dict, size_per_user, self._do_user_cf, 'ucf')
    ...
    def item_cf(self, size_per_user=5):
        ...
        @return {user_id: [(item, score),],}
        """
        similar_dict = self._cal_similar(self.item_users, self.user_items, 'icf')
        return self._do_cf(similar_dict, size_per_user, self._do_item_cf, 'icf')

    def _data_process(self, user_id: str, item_id: str, score: float):
        key = user_id + "-" + item_id
        if key in self.user_item_score:
            self.user_item_score[key] = self.user_item_score[key] + score
        else:
            self.user_item_score[key] = score
    ...
    def _cal_similar(self, dict1: Mapping[str, Collection[str]], dict2: Mapping[str, Collection[str]], cf_type):
        """
        计算相似度
    ...
    def _do_cf(self, similar_dict, size_per_user, cf_func, cf_type):
        ...
        cf_result = cf_func(self.user_items, self.user_item_score, similar_dict, user_items_list, size_per_user)
