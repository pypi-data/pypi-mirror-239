...
class PollCollFilter(BaseCollFilter):
    ...
    def cal_similar(self, cf_type: CFType):
        ...
        last = (self.process_num - 1) * split_size
        results = [self.pool.apply_async(func=self._do_cal_similar,
                                         args=(dict1, items_list[i:i+split_size], self.similar_func))
                   for i in range(0, last, split_size)]

        similar = self._do_cal_similar(dict1, items_list[last:], self.similar_func)
    ...
    def _do_cf(self, similar_dict, size_per_user, cf_type: CFType):
        ...
        cf_result = cf_func(self.user_items, self.user_item_score, similar_dict, user_items_list[last:], size_per_user)
