...
class CollFilter:
    ...
    def __init__(self, data: Iterable[Tuple[str, str, float]], parallel_num=2*os.cpu_count(), similar_func=default_similar_func):
        if parallel_num > 1:
            ...
            self.coll_filter = PollCollFilter(data, parallel_num, similar_func)
        else:
            from cf.base import BaseCollFilter
            self.coll_filter = BaseCollFilter(data, similar_func)

    def user_cf(self, size_per_user=5, user_similar: Mapping[str, Mapping[str, float]] = None) -> Mapping[str, List[Tuple[str, float]]]:
        """
        用户协同过滤
    ...
    def item_cf(self, size_per_user=5, item_similar: Mapping[str, Mapping[str, float]] = None) -> Mapping[str, List[Tuple[str, float]]]:
        """
        物品协同过滤
        @param size_per_user  每个用户推荐结果数目
