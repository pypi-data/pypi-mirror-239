...
class ReportCustomizer:
    ...
    def __init__(self) -> str:
        ...
        ) = FindingReportUtils.get_finding_fragments(self.config, 7) # 7 is the hardcoded index of the findings section: Will be changed in a future update.
        
        return self.assemble_report() # Assembles the report.
    
    def add_report_config_variables(self) -> None:
        for key in self.config.keys():
            _value = self.config[key]
            
            # Signifies that the value is a markdown fragment:
            # Parsing such content variables so that they can use markdown syntax:
            if "_content" in key: _value = MarkdownRenderer.create_markdown(_value)
    ...
    def add_findings_legend(self) -> None:
        ...
        self.report = self.report.replace(
            '{{findings_legend}}',
            # 7 is the hardcoded index of the findings section: Will be changed in a future update.
            ReportLegendUtils.create_finding_severities_legend(7, self.serialized_findings)
        )
