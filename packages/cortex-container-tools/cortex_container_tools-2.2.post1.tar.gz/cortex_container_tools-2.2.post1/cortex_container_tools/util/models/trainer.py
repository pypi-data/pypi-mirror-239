...
class Trainer():
    ...
    def _load_cortex_yaml(self):
        ...
        """
        with open('/mnt/model/cortex.yml', 'r') as file:
        ...
        config['modules'] = {}
        config['module_path'] = f'/mnt/model/{config["module_path"][2:]}'
    ...
    def _setup_pipeline(self):
        ...
        if self._pipeline_id:
            pipeline = cortex.Pipelines.get(resource_id = self._pipeline_id)
            model    = cortex.Models.get(pipeline.model_id)

        else:
            model    = cortex.Models.get(repo_name = self._repo_name)
            pipeline = cortex.Pipelines.create(model[0].id, self._repo_branch, self._repo_hash)

        return {
            'pipeline_id': pipeline.json()['_id'],
            'model_id':    pipeline.json()['modelId']
        }
    ...
    def _initialize_steps(self, pipeline_id: str):
        ...
        if self._use_tracking:
            # Set cortex steps
            self._steps = cortex.Pipelines.create_steps(pipeline_id, _steps).json()
        else:
            # Set normal steps
            self._steps = _steps
    ...
    def _run_pipeline(self, pipeline_id: str, model_id: str):
        return cortex.Pipelines.run(pipeline_id = pipeline_id, model_id = model_id)
    ...
    def _instantiate_model(self):
        ...
        self.model = getattr(model_module, self._cortex_config['model_class'])(model_params, self._model_id, cortex.api_url, headers)
    ...
    def _upload_pipeline(self):
        cortex.Pipelines.upload_files(self._pipeline_id, f'./models/cortex/{self._pipeline_id}/')
    ...
    def run(self):
        try:
            ...
            if self._use_tracking:
                ...
                self._model_id    = pipeline_info['model_id'] # compatbility workaround
                self._pipeline_id = pipeline_info['pipeline_id'] # compatbility workaround

            # Step 2 - Initialize the pipeline steps
            self._initialize_steps(pipeline_info['pipeline_id'])

            # Step 3 - Run the pipeline
            if self._use_tracking:
                self._run_pipeline(
                    pipeline_info['pipeline_id'],
                    pipeline_info['model_id']
                )
