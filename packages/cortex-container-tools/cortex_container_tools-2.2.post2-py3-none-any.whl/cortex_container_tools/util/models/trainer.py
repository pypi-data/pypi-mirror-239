...
class Trainer():
    ...
    def _setup_pipeline(self):
        ...
        if self._pipeline_id:
            pipeline = cortex_sdk.Pipelines.get(resource_id = self._pipeline_id)
            model    = cortex_sdk.Models.get(pipeline.model_id)

        else:
            model    = cortex_sdk.Models.get(repo_name = self._repo_name)
            pipeline = cortex_sdk.Pipelines.create(model[0].id, self._repo_branch, self._repo_hash)
