...
def cli(cosmo):
    ...
    parser.add_argument(
        ...
        dest="version",
        action="store_true",
        help="get Cosmo's version number",
    )

    args = parser.parse_args()

    # This should be pushed into an open_config.py util
    # If --config is used, open the config.yaml file in the Cosmo folder of the user's config dir
    if args.config:
        if args.config_file:
    ...
    if args.local:
        ...
        display_markdown_message(
            """
> Cosmo's local mode is powered by **`LM Studio`**.
