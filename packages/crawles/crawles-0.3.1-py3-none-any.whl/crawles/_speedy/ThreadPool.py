...
class ThreadPool(metaclass=ABCMeta):
    ...
    def __init__(self):
        ...
        self.main_executor = ThreadPoolExecutor(self.concurrency)  # 线程池

        self.lock = Lock()  # 锁
        if self.save_class is not None:  # 存储对象初始化
            self.save_class_ = self.save_class()
            self.consume_executor = ThreadPoolExecutor(self.save_class.concurrency)
    ...
    def run(self):
        start_time = time()
        # 初始化启动线程
        self.main_executor.submit(self.start_request_)

        # 运行消费者
        _consume_list = [self.consume_executor.submit(self.data_save_)
        ...
        self.main_executor.shutdown()  # 关闭线程池
    ...
    def start_request_(self) -> None:  # 初始链接请求
        for index in range(*self.for_index_range):
            request = Request()

            for request_ in self.start_requests(request, index):
                with self.lock:
                    self.request_index += 1
                    request_.index = self.request_index
                self.futures.append(self.main_executor.submit(self.callback_, request_))

    def callback_(self, request_):  # 回调获取
        ...
        for return_ in request_.callback(Item(), request_, response):
            ...
            if return_.__name__ == 'Request':
                with self.lock:  # 全局请求次数锁
                    self.request_index += 1
                    request_.index = self.request_index
                self.futures.append(self.main_executor.submit(self.callback_, copy(request_)))
    ...
    def data_save_(self) -> None:  # 数据存储
        while True:
            try:
                ...
                if self.save_class:
                    self.consume_executor.submit(self.save_class_.save_data, items)
                else:
                    break
            except Empty:
