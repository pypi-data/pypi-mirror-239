...
class Producer(ThreadPoolExecutor):
    ...
    def callback_(self, request_):
        ...
        for return_ in request_.callback(Item(), request_, response):
            ...
            if return_.__name__ == 'Request':
                ...
                self.futures.append(self.submit(self.callback_, copy(request_)))

            elif return_.__name__ == 'Item' or isinstance(return_, dict):
                self.pipeline.queue_.put(return_)

    def start_request_(self, start_requests):
        # 初始链接请求
...
class Consumer(ThreadPoolExecutor):
    def __init__(self, pipeline, save_class_, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.pipeline = pipeline
        ...
        while True:
            try:
                ...
                if self.pipeline.save_class:
                    self.submit(self.pipeline.save_class_.save_data, items)
                else:
                    break
            except Empty:
...
class ThreadPool:
    ...
    def run(self):
        ...
        producer.start_request_(self.start_requests)

        # 创建消费者
        consumer = Consumer(self, self.save_class)
