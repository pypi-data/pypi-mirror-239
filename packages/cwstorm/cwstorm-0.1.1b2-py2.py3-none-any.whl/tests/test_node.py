...
class PrimitiveAccessorsTest(unittest.TestCase):
    ...
    def test_init_attrs(self):
        self.assertEqual(len(self.node.ATTRS), 9)

    def test_creates_attributes(self):
        attributes = dir(self.node)
        self.assertIn("strattr", attributes)
        self.assertIn("liststrattr", attributes)
        self.assertIn("listintattr", attributes)
        self.assertIn("intattr", attributes)
        self.assertIn("push_liststrattr", attributes)
        self.assertIn("push_listintattr", attributes)
    ...
    def test_list_string_setter_and_getter_returns_assigned_value_as_list(self):
        self.node.liststrattr("foo")
        self.assertEqual(self.node.liststrattr(), ["foo"])
