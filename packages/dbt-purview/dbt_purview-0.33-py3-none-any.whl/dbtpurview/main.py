...
def dbtpurview(env,dwhcid,azpurview,path):
   ...
   if(env == "snowflake"):
         ...
         host = str(e1xtras_dict['account'])+".snowflakecomputing.com"
...
def prepare_column_qualified_name(column_name, env, database, schema, host, model_name,type):
   if(env == "databricks"):
       ...
       return qualified_name
   if(env == "snowflake"):
       qualified_name = "snowflake://"+host+"/databases/"+database+"/schemas/"+schema+"/"+type+"/"+model_name+"/columns"+column_name
       return qualified_name   
