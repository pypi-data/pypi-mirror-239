...
class BasePage:
    ...
    def is_element_exist(self, element_name, by_locator):
        ...
        try:
            ...
            self.logger.error(f"Element :: {element_name} :: with locator {by_locator} :: not found.\n{print_stack()}")
            return False
    ...
    def is_element_exist_with_custom_timeout(self, element_name, by_locator, timeout):
        ...
        try:
            ...
            # If element not found
            self.logger.error(
                f"Element {element_name} :: with locator {by_locator} :: doesn't exist or not found.\n{print_stack()}")
            return False
    ...
    def upload_file(self, element_name, by_locator, file_path):
        ...
        try:
            ...
            self.logger.info(f"Uploading file: {file_path} into element :: {element_name}")
            WebDriverWait(self.driver, self.timeout).until(EC.visibility_of_element_located(by_locator)).send_keys(
                file_path)
    ...
    def get_text(self, element_name, by_locator):
        ...
        """
        try:
            self.logger.info(f"Getting the text value for element :: {element_name}")
    ...
    def get_element_attribute_list(self, element_name, by_locator, attribute):
        ...
        try:
            ...
            self.logger.error(f"An error occurred :: {print_stack()}")
            return None
