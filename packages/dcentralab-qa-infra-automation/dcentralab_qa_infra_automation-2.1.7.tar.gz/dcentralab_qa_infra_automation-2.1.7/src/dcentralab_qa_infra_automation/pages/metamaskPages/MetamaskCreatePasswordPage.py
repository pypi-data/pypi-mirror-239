...
class MetamaskCreatePasswordPage(BasePage):
    ...
    def is_page_loaded(self):
        ...
        """
        self.logger(f"Verify if {self.__class__.__name__} is loaded")
        if self.is_element_exist("METAMASK_CREATE_PASSWORD_CONTAINER", METAMASK_CREATE_PASSWORD_CONTAINER):
            self.logger(f"{self.__class__.__name__} is loaded")
            return True
        else:
            self.logger(f"{self.__class__.__name__} is not loaded")
            return False
