...
def generate_package_docs_index_page(path):
    ...
    final_versions = []
    if "latest_github" in versions[:]:
        final_versions.append("latest_github")
    
    final_versions += sorted([v for v in versions if not v == "latest_github"], key=lambda x: float(x[1:]), reverse=True)
