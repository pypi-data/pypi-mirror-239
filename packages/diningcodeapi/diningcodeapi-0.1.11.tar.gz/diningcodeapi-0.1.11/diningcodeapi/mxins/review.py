...
class ReviewMixin:
    def get_reviews(self, store_code: str, page_no: int, page_size: int = 5, proxy=None):
        if page_no == 1:
            soup = self._request_first_page(store_code, proxy)
        else:
            soup = self._request_more_than_second_page(store_code, page_no, page_size, proxy)
    ...
    def _request_first_page(self, store_code, proxy=None):
        response = requests.post(f'https://www.diningcode.com/profile.php?rid={store_code}', headers=HEADERS, proxies={"http":proxy})
