...
envsubst '\$CHANNEL_TASKS_APP_NAME \$CHANNEL_TASKS_STATIC_ROOT \$CHANNEL_TASKS_STATIC_URI \$CHANNEL_TASKS_LISTENER_ADDRESS' \
...
curl -X PUT --data-binary @"${CHANNEL_TASKS_HOME}"/channel-tasks-unit/application.json --unix-socket /var/run/control.unit.sock http://localhost/config/applications/"${CHANNEL_TASKS_APP_NAME}"
curl -X PUT --data-binary @"${CHANNEL_TASKS_HOME}"/channel-tasks-unit/listener.json --unix-socket /var/run/control.unit.sock http://localhost/config/listeners/"${CHANNEL_TASKS_LISTENER_ADDRESS}"
