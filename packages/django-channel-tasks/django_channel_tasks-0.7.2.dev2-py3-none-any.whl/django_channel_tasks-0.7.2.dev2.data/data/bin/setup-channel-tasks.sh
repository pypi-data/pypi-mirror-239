...
envsubst '\$CHANNEL_TASKS_LISTENER_ADDRESS' \
    < "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/listener.template.json > "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/listener.json
envsubst '\$CHANNEL_TASKS_STATIC_ROOT \$CHANNEL_TASKS_STATIC_URI' \
    < "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/static-route.template.json > "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/static-route.json
envsubst '\$CHANNEL_TASKS_APP_NAME' \
    < "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/app-route.template.json > "${CHANNEL_TASKS_HOME}"/channel-tasks-unit/app-route.json
...
curl --unix-socket /var/run/control.unit.sock -X POST -d'{"listeners": {}, "routes": [], "applications": {}}' http://localhost/config/
...
curl --unix-socket /var/run/control.unit.sock -X POST --data-binary @"${CHANNEL_TASKS_HOME}"/channel-tasks-unit/static-route.json http://localhost/config/routes
curl --unix-socket /var/run/control.unit.sock -X POST --data-binary @"${CHANNEL_TASKS_HOME}"/channel-tasks-unit/app-route.json http://localhost/config/routes
