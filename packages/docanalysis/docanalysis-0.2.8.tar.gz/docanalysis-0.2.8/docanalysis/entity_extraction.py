...
class EntityExtraction:
    ...
    def extract_entities_from_papers(
        ...
        if terms_xml_path:
            ...
            if synonyms:
                ...
                self.add_if_file_contains_terms(
                    compiled_terms=synonyms_list,
                    dict_with_parsed_xml=self.sentence_dictionary,
                    searching="has_synonyms",
                )
                if removefalse:
                    self.remove_statements_not_having_xmldict_terms(
                        dict_with_parsed_xml=self.sentence_dictionary
        ...
        if csv_name:
            dict_with_parsed_xml_no_paragrph = (
                self.remove_paragraph_form_parsed_xml_dict(
                    self.sentence_dictionary, "paragraph"
                )
            )
            self.convert_dict_to_csv(
                path=os.path.join(corpus_path, f"{csv_name}"),
                dict_with_parsed_xml=dict_with_parsed_xml_no_paragrph,
            )
        if make_json:
            dict_with_parsed_xml_no_paragrph = (
                self.remove_paragraph_form_parsed_xml_dict(
            ...
            self.convert_dict_to_json(
                path=os.path.join(corpus_path, f"{make_json}"),
                dict_with_parsed_xml=dict_with_parsed_xml_no_paragrph,
            )
        if make_ami_dict:
    ...
    def run_ami_section(self, path):
        ...
        """
        file_list = glob(os.path.join(path, "**", "fulltext.xml"), recursive=True)
        ...
        for section_name in section_names:
            if section_name in self.sections.keys():
                ...
                for section in self.sections[section_name]:
                    self.all_paragraphs[section_name] += glob(
                        os.path.join(path, "**", "sections", "**", section),
                        recursive=True,
                    )
            else:
                logging.error(
    ...
    def _make_dict_attributes(
        self, counter, section, section_path, paragraph_text, sentence
    ):
        """
        ...
        for paragraph in tqdm(dict_with_parsed_xml):
            if len(dict_with_parsed_xml[paragraph]["sentence"]) > 0:
                ...
                (
                    ...
                    abbreviation_start,
                    abbreviation_end,
                ) = self._make_required_lists()
                self._get_entities(
                    entities_names, doc, entities, labels, position_end, position_start
                )
                self._add_lists_to_dict(
    ...
    def _get_entities(
        self, entities_names, doc, entities, labels, position_end, position_start
    ):
        """
    ...
    def make_abb_exp_list(self, result_dictionary):
        ...
        for entry in result_dictionary:
            ...
            if "abb" in sentence_dictionary:
                ...
                (
                    name_list_for_every_dict,
                    term_list_for_every_dict,
                ) = self._make_list_from_dict(pairs_dicts)
                list_of_name_lists.append(name_list_for_every_dict)
                list_of_term_lists.append(term_list_for_every_dict)
        return self._list_of_lists_to_single_list(
            list_of_name_lists
        ), self._list_of_lists_to_single_list(list_of_term_lists)
    ...
    def add_if_file_contains_terms(
        self, compiled_terms, dict_with_parsed_xml, searching="has_terms"
    ):
    ...
    def get_terms_from_ami_xml(self, xml_path):
        ...
        if xml_path in self.dict_of_ami_dict.keys():
            ...
            logging.error(
                f"{xml_path} is not a supported dictionary. Choose from: EO_ACTIVITY, EO_COMPOUND, EO_EXTRACTION, EO_PLANT, EO_PLANT_PART, PLANT_GENUS,EO_TARGET, COUNTRY, DISEASE, DRUG, ORGANIZATION "
            )
    ...
    def _add_parsed_entities_to_lists(
    ...
    def remove_paragraph_form_parsed_xml_dict(
        self, dict_with_parsed_xml, key_to_remove
    ):
    ...
    def remove_statements_not_having_xmldict_terms(
        self, dict_with_parsed_xml, searching="has_terms"
    ):
