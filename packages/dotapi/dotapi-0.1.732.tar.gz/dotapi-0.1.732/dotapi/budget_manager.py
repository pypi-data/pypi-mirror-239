import os, json, time
import dotapi 
from dotapi.utils import ModelResponse
...
class BudgetManager:
    def __init__(self, project_name: str, client_type: str = "local", api_base: Optional[str] = None):
        ...
        self.api_base = api_base or "https://api.dotagent.dev"
        ## load the data or init the initial dictionaries
        self.load_data() 
    
    def print_verbose(self, print_statement):
        if dotapi.set_verbose:
    ...
    def projected_cost(self, model: str, messages: list, user: str):
        ...
        prompt_tokens = dotapi.token_counter(model=model, text=text)
        prompt_cost, _ = dotapi.cost_per_token(model=model, prompt_tokens=prompt_tokens, completion_tokens=0)
    ...
    def update_cost(self, user: str, completion_obj: Optional[ModelResponse] = None, model: Optional[str] = None, input_text: Optional[str] = None, output_text: Optional[str] = None):
        if model and input_text and output_text:
            prompt_tokens = dotapi.token_counter(model=model, messages=[{"role": "user", "content": input_text}])
            completion_tokens = dotapi.token_counter(model=model, messages=[{"role": "user", "content": output_text}])
            prompt_tokens_cost_usd_dollar, completion_tokens_cost_usd_dollar = dotapi.cost_per_token(model=model, prompt_tokens=prompt_tokens, completion_tokens=completion_tokens)
            cost = prompt_tokens_cost_usd_dollar + completion_tokens_cost_usd_dollar
        elif completion_obj: 
            cost = dotapi.completion_cost(completion_response=completion_obj)
            model = completion_obj['model'] # if this throws an error try, model = completion_obj['model']
        else:
            raise ValueError("Either a chat completion object or the text response needs to be passed in. Learn more - https://docs.dotagent.dev/anyllm/budget_manager")
