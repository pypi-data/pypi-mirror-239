...
class LiteDebugger:
    ...
    def __init__(self, email=None):
        self.api_url = "https://api.dotagent.dev/debugger"
        self.validate_environment(email)
        pass

    def validate_environment(self, email):
        try:
            self.user_email = (email or os.getenv("ANYLLM_TOKEN") or os.getenv("ANYLLM_EMAIL"))
            if self.user_email == None: # if users are trying to use_client=True but token not set
                raise ValueError("anyllm.use_client = True but no token or email passed. Please set it in anyllm.token")
            self.dashboard_url = "https://admin.dotagent.dev/" + self.user_email
            try:
                print(
                    f"\033[92mHere's your AnyLLM Dashboard 👉 \033[94m\033[4m{self.dashboard_url}\033[0m"
                )
            except:
                print(f"Here's your AnyLLM Dashboard 👉 {self.dashboard_url}")
            if self.user_email == None:
                raise ValueError(
                    "[Non-Blocking Error] AnyLLMDebugger: Missing ANYLLM_TOKEN. Set it in your environment. Eg.: os.environ['ANYLLM_TOKEN']= <your_email>"
                )
    ...
    def input_log_event(
        ...
        print_verbose(f"LiteDebugger: Pre-API Call Logging for call id {anyllm_call_id}")
        try:
            print_verbose(
                f"AnyLLMDebugger: Logging - Enters input logging function for model {model}"
            )
            ...
            updated_anyllm_params = remove_key_value(anyllm_params, "logger_fn")
            ...
            if call_type == "embedding":
                for message in messages: # assuming the input is a list as required by the embedding function
                    anyllm_data_obj = {
                        ...
                        "end_user": end_user,
                        "status": "initiated",
                        "anyllm_call_id": anyllm_call_id,
                        "user_email": self.user_email,
                        "anyllm_params": updated_anyllm_params,
                        "optional_params": optional_params,
                    }
                    print_verbose(
                        f"AnyLLMDebugger: Logging - logged data obj {anyllm_data_obj}"
                    )
                    response = requests.post(
                        url=self.api_url,
                        headers={"content-type": "application/json"},
                        data=json.dumps(anyllm_data_obj),
                    )
    ...
    def post_call_log_event(self, original_response, anyllm_call_id, print_verbose, call_type, stream):
        print_verbose(f"LiteDebugger: Post-API Call Logging for call id {anyllm_call_id}")
        try:
            if call_type == "embedding":
                anyllm_data_obj = {
                    "status": "received",
            ...
            print_verbose(f"litedebugger post-call data object - {anyllm_data_obj}")
            response = requests.post(
                url=self.api_url,
                headers={"content-type": "application/json"},
                data=json.dumps(anyllm_data_obj),
    ...
    def log_event(
        ...
        print_verbose(f"LiteDebugger: Success/Failure Call Logging for call id {anyllm_call_id}")
        try:
            print_verbose(
                f"AnyLLMDebugger: Success/Failure Logging - Enters handler logging function for function {call_type} and stream set to {stream} with response object {response_obj}"
            )
            ...
            if call_type == "completion" and stream == False:
                ...
                print_verbose(
                    f"LiteDebugger: Logging - final data object: {anyllm_data_obj}"
                )
                response = requests.post(
                    url=self.api_url,
                    headers={"content-type": "application/json"},
