...
class EndPoints:
    ...
    def help():
        ...
        print("\t\t1. __init__(team, project): Initializes an EndPoints instance with the specified team and "
              "project IDs.")
        print("\t\t2. create_triton(endpoint_name, plan, model_id, model_path='', replicas=1)")
        print("\t\t3. create_pytorch(endpoint_name, plan, model_id, model_path='', replicas=1)")
        print("\t\t4. create(endpoint_name, framework, plan, container_name, container_type, model_id, replicas=1, "
              "disc_size=10, model_path="", env_variables=[], mount_path="", registry_endpoint="", "
              "auth_type='pass', username="", password="", docker_config=""): "
              "Creates an endpoint with the provided details.")
        print("\t\t5. get(endpoint_id): Retrieves information about a specific endpoint using its ID.")
        print("\t\t6. logs(endpoint_id): Retrieves logs of a specific endpoint using its ID.")
        print("\t\t7. stop(endpoint_id): Stops a specific endpoint using its ID.")
        print("\t\t8. start(endpoint_id): Starts a specific endpoint using its ID.")
        print("\t\t9. list(): Lists all endpoints associated with the team and project.")
        print("\t\t10. delete(endpoint_id): Deletes an endpoint with the given ID.")
