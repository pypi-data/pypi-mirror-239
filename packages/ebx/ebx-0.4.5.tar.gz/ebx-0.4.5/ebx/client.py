...
from ebx.constants.api import API_SECRETS_FILE, API_SECRETS_PATH
class ClientManager():
...
def get_client(name=None) -> Client:
    """Gets the client api singleton.
    
    Returns:
        Client: The client api.
    """
    if name is not None:
        return ClientManager().get_client_by_name(name)
    elif ClientManager().using_no_context():
        return ClientManager().get_first_client()
    else:
        return ClientManager().get_current_context()
...
def auth_using(authenticator: AbstractAuthentication, config: ClientConfig = None, name: str = None) -> Client:
    ...
    return ClientManager().set_client(name=name, **kwargs)
