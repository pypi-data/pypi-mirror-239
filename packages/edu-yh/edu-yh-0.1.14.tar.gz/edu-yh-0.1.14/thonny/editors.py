    ...
    def show_file_info(self, event):
        ...
        if file["image"]:
            if file["image"][0].startswith("pyimage1"):
                values_str = file["values"][0]
                values_str = values_str.replace("'", "\"")  # 将单引号替换为双引号
                values_dict = json.loads("{" + values_str + "}")  # 将字符串转换为字典
                path = values_dict.get("path", "")
                label = values_dict.get("label", "")
                ppid = values_dict.get("id", "")
                print(f"path: {path}, label: {label}, id: {ppid}")
                # path = (
