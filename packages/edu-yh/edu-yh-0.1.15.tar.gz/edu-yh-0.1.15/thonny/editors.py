    ...
    def show_file_info(self, event):
        ...
        if file["image"]:
            if file["image"][0].startswith("pyimage1"):
                values_list = file["values"]
                file_info_str = str(values_list)
                path = re.search("'path':\s*'([^']+)'", file_info_str).group(1)

                file_info_text += f"文件夹名: {file['text']}\n"
                file_info_text += f"文件路径: {path}\n"
                self.open_button.configure(state=tk.NORMAL)
        else:
            file_info_text += f"文件名: {file['text']}\n"
            try:
                values_list = file["values"]
                file_info_str = str(values_list)
                owner = re.search("'owner':\s*'(\w+)'", file_info_str).group(1)
                path = re.search("'path':\s*'([^']+)'", file_info_str).group(1)
                size = int(re.search("'size':\s*(\d+)", file_info_str).group(1))
                last_modified_time = re.search("'lastModifiedTime':\s*'([^']+)'", file_info_str).group(1)

                print(f"owner: {owner}")
                print(f"path: {path}")
                print(f"size: {size}")
                print(f"lastModifiedTime: {last_modified_time}")
                file_info_text += f"文件路径: {path}\n"
                file_info_text += f"创建者: {owner}\n"
                file_info_text += f"文件大小: {size} bytes\n"
                file_info_text += f"最后修改时间: {last_modified_time}"
            except:
                print("ss")
