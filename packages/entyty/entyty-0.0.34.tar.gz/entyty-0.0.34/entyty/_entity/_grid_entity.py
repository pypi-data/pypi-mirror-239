...
class AbstractGridEntity(LogicalEntity):
    log('Initializing abstract grid entity class.')
    ...
    def cell(self, cell):
        if cell is not None:
            if self.cell is not None:
                log(f'[{self.name}] ALREADY OCCUPIES A CELL: {self.cell}')
                return
            self._cell = cell
...
class GridEntity(AbstractGridEntity):
    ...
    dispatcher = AbstractGridEntity.dispatcher
    ...
    def __init__(
        ...
        log(f'Initializing grid entity [{name}].')
        super(GridEntity, self).__init__(scene, name)
        self.parent = parent
        self.grid = grid
        log(f'[{name}] Finding initial cell.')
        init_cell = self.grid.random_cell(attr=('passable', True))
        self.cell = init_cell
        log(f'[{name}] Initial cell: {self.cell}')
        self.cell_history = []
        self.last_cell = None
        ...
        log(f'[{name}] Occupying init cell - {self.cell.designation}.')
    ...
    def end_turn(self):
        ...
        self._movements_remaining = self.movements
        self.actions = {'move': {i: {'direction': None, 'from': None, 'to': None} for i in range(self.speed // 5)}}
    ...
    def set_path_to(self, destination: object):
        log(f'[{self.name}] SETTING PATH TO: {destination} FROM: {self.cell} DISTANCE: {self.grid.get_distance(self.cell.designation, destination.designation, "cells")}')
    ...
    def vacate(self):
        log(f'[{self.name}] VACATING CELL: {self.cell}')
    ...
    def occupy(self, cell_to_occupy: object = None):
        if self.cell is None and not cell_to_occupy.occupied or self.last_cell is not None:
            log(f'[{self.name}] OCCUPYING CELL: {cell_to_occupy}')
    ...
    def move(self, cell_to_move_to: object = None):
        ...
        mvmnt_index = self.movements - self._movements_remaining
        if cell_to_move_to in [self.grid[cell] for cell in self.cell.adjacent]:
            self.actions['move'][mvmnt_index] = {'direction': f'{list(_DIRECTION_MAP.keys())[self.cell.adjacent.index(cell_to_move_to.designation)]}', 'from': f'{self.cell.designation}', 'to': f'{cell_to_move_to.designation}'}
        else:
            print('Invalid move')
            return
        self.vacate()
    ...
    def move_in_direction(self, direction):
        ...
        log(f'[{self.name}] MOVING IN DIRECTION: {direction}')
