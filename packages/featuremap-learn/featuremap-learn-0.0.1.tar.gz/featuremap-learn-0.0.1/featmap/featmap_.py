...
import featmap.distances as dist

import featmap.sparse as sparse

from featmap.utils import (
    submatrix,
    ts,
    csr_unique,
    fast_knn_indices,
)
from featmap.spectral import spectral_layout
from featmap.layouts import (
...
def raise_disconnected_warning(
    ...
    if verbose & (vertices_disconnected == 0) & (edges_removed > 0):
        ...
        warn(
            ...
            f"Use featmap.utils.disconnected_vertices() to identify them.",
        )
...
def tangent_space_approximation(
    ...
    # dists = featmap_kwds["graph_dists"]
    n_neighbors = featmap_kwds["n_neighbors"]
    n_neighbors_in_guage = int(featmap_kwds["gauge_coefficient"] * n_neighbors)
    knn_index = featmap_kwds["_knn_indices"]
    ...
    featmap_kwds["U"] = np.array(gauge_u).astype(np.float32, copy=True) # For visualization of local 
    featmap_kwds["Singular_value"] = np.array(singular_values).astype(np.float32, copy=True) # For visualization of local 
    featmap_kwds["VH"] = np.array(gauge_vh).astype(np.float32, copy=True)
...
def tangent_space_embedding(
    ...
    if method == 'umap':    
        ...
        # T1 = time.time()
        import umap
        umap_embedding = umap.UMAP(n_neighbors=30,random_state=random_state, metric='euclidean',min_dist=0.3).fit_transform(X=X_pca)
        featmap_kwds['gauge_v1_emb'] = umap_embedding
        # T2 = time.time()
        ...
        # T1 = time.time()
        umap_embedding = umap.UMAP(n_neighbors=30, random_state=random_state, metric='euclidean',min_dist=0.3).fit_transform(X=X_pca)
        featmap_kwds['gauge_v2_emb'] = umap_embedding
        # T2 = time.time()
        # print(f'UMAP time is {T2-T1}')
        ...
        featmap_kwds['gauge_v1_emb'] = spectral_embedding
        ...
        featmap_kwds['gauge_v1_emb'] = isomap_embedding
    ...
    featmap_kwds["VH_smoothing"] = np.array(gauge_vh).astype(np.float32, copy=True)
    # featmap_kwds["VH_embedding_original"] = np.array(gauge_vh_embedding).astype(np.float32, copy=True)
    featmap_kwds["VH_embedding"] = np.array(gauge_vh_mean_embedding).astype(np.float32, copy=True) # VH_embedding after average
...
def variation_embedding(
    ...
    ############################################
    gauge_vh = featmap_kwds["VH_smoothing"].copy()
  
    singular_values_collection = featmap_kwds["Singular_value"].copy()
    ...
    featmap_kwds["variation_pc"] = np.array(velocity).astype(np.float32, copy=True)
    ...
    featmap_kwds["pc_loadings_scale"] = pc_loadings_scale

    ######################################
    # Variation embedding
    ########################################
        
    data_original = featmap_kwds['variation_pc'].copy()
    ...
    import umap
    emb_variation = umap.UMAP(random_state=random_state, n_neighbors=30,metric='euclidean',min_dist=0.3, spread=1, n_components=2).fit(emb_svd)
    featmap_kwds["variation_embedding"] = emb_variation.embedding_
...
def simplicial_set_embedding_with_tangent_space_embedding(
    ...
    data: array of shape (n_samples, n_features)
        The source data to be embedded by featMAP.
    ...
    #     the embedding according to the featMAP algorithm.

    featmap_kwds: dict
        Key word arguments to be used by the featMAP optimization.
    ...
    aux_data: dict
        Auxiliary output returned with the embedding. When featMAP extension
    ...
    T1 = time.time()
    tangent_space_approximation(data, graph, featmap_kwds)
    T2 = time.time()
    ...
    T1 = time.time()
    tangent_space_embedding(data, featmap_kwds, random_state)
    T2 = time.time()
    ...
    singular_values = featmap_kwds["Singular_value"].copy()
    ...
    featmap_kwds["R"] = R.astype(np.float32, copy=True)
    featmap_kwds["mu"] = graph.data
    featmap_kwds["mu_sum"] = mu_sum
    ...
    if output_feat:
        ...
        VH_embedding = featmap_kwds["VH_embedding"]
        ...
        featmap_kwds["rad_emb_no_log"] = np.sqrt(re_sum_without_log) # Variance of Gaussian distribution
        featmap_kwds["emb_graph"] = emb_graph
        featmap_kwds["emb_dists"] = emb_dists
        featmap_kwds["emb_knn_indices"] = knn_indices
    ...
    if variation_emb_flag == True:
        variation_embedding(data,featmap_kwds,random_state)     
...
class featMAP(BaseEstimator):
    ...
    low_memory: bool (optional, default True)
        ...
        memory. If you find that featMAP is failing due to memory constraints
    ...
    featmap: bool (optional, default False)
        Specifies whether the featity-augmented objective of featMAP
    ...
    feat_lambda: float (optional, default 2.0)
        ...
        in featMAP. Higher values prioritize density preservation over the
        featMAP objective, and vice versa for values closer to zero. Setting this
    ...
    feat_frac: float (optional, default 0.3)
        ...
        feature-augmented objective is used in featMAP. The first
    ...
    output_feat: float (optional, default False)
        ...
        can also be used when featmap=False to calculate the densities for
        UMAP embeddings.
    ...
    def _validate_parameters(self):
        ...
        self._featmap_kwds = {
            "lambda": self.feat_lambda, #if self.featmap else 0.0,
            "frac": self.feat_frac, #if self.featmap else 0.0,
            "gauge_coefficient": self.feat_gauge_coefficient, #if self.featmap else 0.0,
        ...
        if self.output_metric not in ("euclidean", "l2"):
            raise ValueError(
                "Non-Euclidean output metric not supported for featMAP."
            )
    ...
    def fit(self, X, y=None):
        ...
        if X[index].shape[0] <= self.n_neighbors:
            ...
            # if self.featmap:
            self._featmap_kwds["n_neighbors"] = self._n_neighbors
        else:
        ...
        if self.metric == "precomputed" and self._sparse_data:
            ...
            (
                ...
                True,
                self.verbose,
                # self.featmap or self.output_feat,
                self.output_feat,
        ...
        # if self.featmap or self.output_feat:
        self._featmap_kwds["graph_dists"] = self.graph_dists_
        self._featmap_kwds["n_neighbors"] = self.n_neighbors
        # self._featmap_kwds["n_neighbors_in_guage"] = self.n_neighbors # TODO: n_neighbors_in_guage same as n_neighbors or not
        self._featmap_kwds["_knn_indices"] = self._knn_indices
    ...
    def transform(self, X):
        ...
        #         "Transforming data into an existing embedding not supported for featMAP."
        #     )
