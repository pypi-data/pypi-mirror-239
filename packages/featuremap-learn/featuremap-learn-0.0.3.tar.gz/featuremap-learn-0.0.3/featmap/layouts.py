...
from featmap.utils import tau_rand_int
...
def _optimize_layout_euclidean_featmap_epoch_init_grad(
    head_embedding,
    tail_embedding,
    head,
    tail,
...
def optimize_layout_euclidean_anisotropic_projection(
    ...
    featmap: bool (optional, default False)
        Whether to use the feature-augmented featMAP objective
    featmap_kwds: dict (optional, default None)
        Auxiliary data for featMAP
    ...
    feat_init_fn = numba.njit(
         _optimize_layout_euclidean_featmap_epoch_init_grad,
         fastmath=True,
         parallel=parallel,
         nopython=True,
         cache=True
    ...
    # feat_init_fn = _optimize_layout_euclidean_featmap_epoch_init_grad

    feat_mu_tot = np.sum(featmap_kwds["mu_sum"]) / 2  # sum of all edges' existing probability, float, shape of (1,)
    # should we modify lambda? Yes, we should
    #TODO: modify lambda
    feat_lambda = featmap_kwds["lambda"] 
    feat_R = featmap_kwds["R"] # array shape of (n_vertices d)
    feat_VH = featmap_kwds["VH"]
    feat_VH_embedding = featmap_kwds["VH_embedding"] # array of shape (n_vertices, dim)
    # feat_rotation_angle = featmap_kwds["rotation_angle"]
    feat_mu = featmap_kwds["mu"] # edge probability
    ...
    feat_var_shift = featmap_kwds["var_shift"]
    # else: 
    #     feat_mu_tot = 0
    #     feat_lambda = 0
    ...
    for n in tqdm(range(n_epochs), **tqdm_kwds):
        ...
        featmap_flag = (
            # featmap and 
            (featmap_kwds["lambda"] > 0)
            and (((n + 1) / float(n_epochs)) > (1 - featmap_kwds["frac"]))
        )
        # print(f'featmap_flag_{featmap_flag}')
        if featmap_flag:
            ...
            # T2 = time.time()
            # print(f'FeatMAP initialization time is {T2-T1}')
