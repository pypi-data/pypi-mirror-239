...
from .utils import ANY, Any, BaseNoiseComponent
...
class GateNoise(BaseNoiseComponent):
    ...
    There are currently two subclasses of GateNoise:
        ...
        - GenericGateNoise: Noise that is triggered by an unspecified gate. The number of qubits that the gate acts on can be specified, or set to 'any'.
    """
    ...
    def generic_qubit_noise_lookup(self, n_gate_qubits: int | Any) -> QubitNoise | None:
        """
        ...
        """
        # Search for a generic noise entry with the right number of qubits or matching 'any'
        for qubit_noise in self._generic_qubit_noise:
            if n_gate_qubits == ANY == qubit_noise.n_qubits:
                return qubit_noise
            elif (
                isinstance(n_gate_qubits, int)
                and isinstance(qubit_noise.n_qubits, int)
                and n_gate_qubits == qubit_noise.n_qubits
            ):
                return qubit_noise
    ...
    def get_noise_channels(self, gate_qubits: Qubits, trace: bool = False):
        ...
        # Failing that, try to find a generic qubit noise object matching any qubits
        if qubit_noise is None:
            qubit_noise = self.generic_qubit_noise_lookup(ANY)
        ...
        if found_generic and qubit_noise is not None:
            # Convert instance of Any to string
            qubit_repr = (
                qubit_noise.n_qubits
                if isinstance(qubit_noise.n_qubits, int)
                else str(qubit_noise.n_qubits)
            )
            trace_dict["generic_qubit"] = {"qubits": qubit_repr}
        else:
