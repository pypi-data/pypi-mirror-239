...
from ..serializers import cirq_gate_map, qiskit_gate_map, serialize_circuit
...
class NoiseModel(BaseNoiseComponent):
    ...
    supported_gate_sizes: dict[str, int] = Field(
    ...
    def validate_supported_gate_sizes(cls, supported_gate_sizes, values):
        ...
        if supported_gate_sizes is None:
            if using_cirq:
                return {
                    g: utils.supported_gate_sizes[cirq_gate_map[g]]
                    for g in cirq_gate_map.keys()
                }
            elif using_qiskit:
                return {
                    g: utils.supported_gate_sizes[qiskit_gate_map[g]]
                    for g in qiskit_gate_map.keys()
                }
            else:
                return utils.supported_gate_sizes.copy()
        ...
        if any(n < -1 or n == 0 for n in supported_gate_sizes.values()):
            raise ValueError(
                f"Gate sizes must be a positive integer, or -1. Got {supported_gate_sizes}."
            )
        ...
        if using_qiskit and any(
            ...
            != utils.supported_gate_sizes[qiskit_gate_map[g]]
        ...
        if using_cirq and any(
            g in cirq_gate_map
            and supported_gate_sizes[g] != utils.supported_gate_sizes[cirq_gate_map[g]]
            for g in supported_gate_sizes.keys()
        ):
    ...
    def add_supported_gate(
        ...
        if not isinstance(n_qubits, int) or n_qubits < -1 or n_qubits == 0:
            raise ValueError(
                f"Gate size must be a positive integer, or -1. Got {n_qubits}."
            )
    ...
    def add(
        ...
        Special cases to consider:
            Triggering conditions:
                ...
                - ``gate_qubits`` is ANY: The channel will be triggered by the given gate (which could be ANY) for which a more specific (with specified qubits) entry has not been found.
        ...
        :param gate_qubits: The qubits that the gate acts on. Can be ANY (or any instance of Any) to match any qubits.
        ...
        if not isinstance(gate_qubits, (int, Any)):
            # Check that the gate qubits are a list or tuple
            if not isinstance(gate_qubits, Sequence):
                raise ValueError(
                    f"Error adding noise rule: Gate qubits are of unrecognized type. They should be list of qubits (qiskit / cirq qubit objects or strings), or int or Any."
                )
        ...
        # Case 1: Specific QubitNoise object (gate_qubits are a list of qubit objects, not an integer or instance of Any)
        ...
        # Case 2: Generic QubitNoise object, gate_qubits is int or Any
        elif isinstance(gate_qubits, Any) or isinstance(gate_qubits, int):
        ...
        # Vacuous else for mypy
            ...
            raise ValueError(
                "Gate qubits are not sequences of qubit object, 'ANY' or int as required."
            )
