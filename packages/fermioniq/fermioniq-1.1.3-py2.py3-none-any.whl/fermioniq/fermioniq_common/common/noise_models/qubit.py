...
from .utils import ANY, BaseNoiseComponent, QubitInternal, tuples_to_lists
...
class QubitNoise(BaseNoiseComponent):
    ...
    # TODO: this always gets 'internalTargets'
...
class GenericQubitNoise(QubitNoise):
    ...
    n_qubits: int | Any = Field(
        description="The number of qubits that trigger this particular QubitNoise. Can be 'any' to represent any number of qubits (default).",
    )

    @validator("n_qubits")
    def _validate_n_qubits(cls, v):
        if isinstance(v, int) or v == ANY:
            return v
        raise ValueError("value is not a valid integer or of type Any.")
    ...
    def _validate_channel(
        ...
        # Case 2: The channel acts on the same qubits as the gate that triggered it.
        elif isinstance(self.n_qubits, int):
            # Case 2.1: The channel acts on the same number of qubits as the QubitNoise is triggered by, or on single qubits.
            # Then the particular qubits that this channel is applied to will be determined at run-time.
            if channel.n_qubits in (
                1,
                self.n_qubits,
            ):
                return (
        ...
        # Case 3: The channel acts on the same qubits as the gate that triggered it, and the QubitNoise object does not specify how many qubits it is triggered by.
        elif self.n_qubits == ANY:
            # Case 3.1: The channel acts on single qubits. Then the particular qubits that this channel is applied to will be determined at run-time.
        ...
        if isinstance(self.n_qubits, int):
            if self.n_qubits == 1:
                raise ValueError(
            ...
            raise ValueError(
                f"Target qubits for channel specified as 'same' as gate qubits, but the channel acts on {channel.n_qubits} qubits. "
                f"The channel should act on 1 qubit to be applied to the same qubits as the gate that triggers it."
            )
