        ...
        if isinstance(value, str):
            ...
            for list_item in value:
                if isinstance(list_item, str):
                    ...
                    dict_interpolated = do_dict_interpolation(list_item, global_config)
                    interpolated_list.append(dict_interpolated)
                elif isinstance(list_item, list):
                    dict_interpolated = do_dict_interpolation({'placeholder': list_item}, global_config)
                    interpolated_list.append(dict_interpolated['placeholder'])
        elif isinstance(value, dict):
            dict_interpolated = do_dict_interpolation(value, global_config)
            interpolated_dict[key] = dict_interpolated
    return interpolated_dict


def do_string_interpolation(raw_string, global_config):
    ...
    for match in matches:
        ...
        actual_value = get_dict_value_from_json_path(global_config, match.group().strip()[2:-2].strip())
        interpolated_string = interpolated_string.replace(matched_text, actual_value, 1)
    return interpolated_string


def interpolate(target, global_config):
    if isinstance(target, str):
        stripped_key = target.strip()
        if len(stripped_key) < len(target):
            LOGGER.warning(f"Removing extra spaces when using the key {stripped_key}. Correct your Json file...")
        return do_string_interpolation(target, global_config)
    if isinstance(target, dict):
        return do_dict_interpolation(target, global_config)