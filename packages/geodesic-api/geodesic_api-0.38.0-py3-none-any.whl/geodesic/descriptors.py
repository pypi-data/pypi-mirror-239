...
class _ListDescr(_BaseDescr):
    ...
    def _set(self, obj: object, value: list):
        ...
        if len(value) > 0 and isinstance(self.item_type, _BaseDescr):
            tmp = _APIObject()
            self.item_type.__set_name__(tmp, "tmp")
            for i, v in enumerate(value):
                self.item_type._set(tmp, v)
                value[i] = tmp["tmp"]
    ...
    def _validate(self, obj: object, value: list) -> None:
        ...
        if self.item_type is not None:
            if len(value) > 0:
                if isinstance(self.item_type, _BaseDescr):
                    for v in value:
                        self.item_type._validate({}, v)
