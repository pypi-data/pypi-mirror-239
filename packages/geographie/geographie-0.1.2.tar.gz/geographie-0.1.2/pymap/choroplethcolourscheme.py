...
from . import ChoroplethColourRange
...
class ChoroplethColourScheme(dict):
    ...
    def add_colour(self, colour: Color | ChoroplethColourRange, values: any,
        ...
        - colour: The colour or range of colours that will be added to the map
    ...
    def add_values_from_colour(self, values, colour):
        for c in self['colours']:
    ...
    def add_values_from_colour_id(self, values, colour_id):
        for c in self['colours']:
            if c['values_type'] == 'list' and c['id'] == colour_id:
                if type(values) == str:
                    c['values'].append(values)
                else:
                    c['values'] = c['values'] + values
