...
from gertrude.domain.task_management.value_objects import PersonId, TaskId
...
class Interactor:
    ...
    def execute(self, request: Request) -> None:
        ...
        if not request.person_id:
            return self.presenter.missing_person_id()
        ...
        try:
            person_id = PersonId.instanciate(request.person_id)
        except ValueError:
            return self.presenter.incorrect_person_id()

        try:
            task = self.tasks.load(task_id)
            self.history << task.delegate_to(person_id)
        except TaskNotFound:
            return self.presenter.task_not_found()
        except TransitionNotAllowed as exc:
            return self.presenter.transition_not_allowed(exc.current, exc.next)

        self.presenter.task_delegated()
