...
- ~Fix error with the order of arguments provided~ : Wasn't actually a problem!
- ~Enable support for non-binary classification tasks. (f1-score and more scores are `binary` by default)~ : Now non-binary classification
tasks are supported too!
- ~Add `to_Dataframe` function that will return a Df as a format of model scores visualization~ : Done! Now, you can use `GMSModule.to_df()` function!
