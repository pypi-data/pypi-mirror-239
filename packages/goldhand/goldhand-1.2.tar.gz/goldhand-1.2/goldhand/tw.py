...
class Tw:
    ...
    def get_plotly_title(self, ticker, static_plot=False):
        if static_plot:
            if '-USD' in ticker:
                ...
                plotly_title = f"{t['name']} ({t['ticker']}) | ${round(t['price'], 2)} | {t['sector']} | {t['industry']}<br>Market capitalization: {t['market_cap_text'].replace('💲', '') } | Workers: {round(t['n_emp']):,} <br>Sector location: {t['sec_loc']} | Industry location: {t['ind_loc']}"
