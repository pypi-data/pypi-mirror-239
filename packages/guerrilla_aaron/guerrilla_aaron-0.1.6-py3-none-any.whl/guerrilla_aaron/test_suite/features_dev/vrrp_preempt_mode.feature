...
Topology:
 ...
 +----------------+
    ...
    Scenario: Preempt Mode
        # if preempt_mode is disabled or local priority is greater than neighbor -> reset Master_Down_Timer
        When  set up vrrp on the "DUT" with the content of the table
              ...
              | enable | 3       | 1    | 192.168.127.253 | 100      | disable      | 120           | enable      | lan       | 10000          |
        And   send 1 vrrp packet with the content of the table from "HOST" to "DUT"
              | version | type | vrid | addr_list       | priority |
              | 3       | 1    | 1    | 192.168.127.253 | 100      |
        And   wait for 3 advertisement interval
        Then  vrrp status should be "BACKUP" on "DUT"
        # if preempt_mode is enabled and local priority is greater than neighbor -> discard the ADVERTISEMENT
        When  set up vrrp on the "DUT" with the content of the table
              | status | version | vrid | vip             | priority | preempt_mode | preempt_delay | accept_mode | interface | adver_interval |
