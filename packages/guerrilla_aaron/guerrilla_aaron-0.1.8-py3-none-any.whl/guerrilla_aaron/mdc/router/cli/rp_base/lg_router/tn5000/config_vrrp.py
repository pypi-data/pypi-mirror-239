...
PROMPT_MAIN = r'\n[^\n]+[^\)]# '
PROMPT_CONFIG = r'\n.*\(.*\)# '
ALL_PROMPTS = [PROMPT_MAIN, PROMPT_CONFIG]
    ...
    def set_vrrp_rule(self,
        ...
        if preempt_mode:
            ...
            if preempt_mode == "enable":
                self._s.command_expect(f'preemption')
            elif preempt_mode == "disable":
                self._s.command_expect(f'no preemption')
            else:
        ...
        if adver_interval:
            self._s.command_expect(f'adver-interval {adver_interval}')
        return self
