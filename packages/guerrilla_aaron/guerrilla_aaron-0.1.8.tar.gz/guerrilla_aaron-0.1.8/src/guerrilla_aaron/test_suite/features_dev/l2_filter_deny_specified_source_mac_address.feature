Feature: [Firewall - L2 Policy]
	...
	#Purpose: 確保source mac address可正常作用
	#
	#Checkpoint:
	#
	## 當traffic的source mac address符合設定的source mac address時，可成功被filter
	## 當traffic的source mac address不符合設定的source mac address時，不會被filter
	@sanity @l2-filter @edrg9010_v2 @edrg9010_v3 @edr8010_v3 @tn4900_v3
	Scenario: Deny Specified Source Mac Address
		# Topology:
		#     +-------------------------------------------------------+
		...
		## check point 2
		When  Change "HOST_A" MAC address, which is different from the original one
