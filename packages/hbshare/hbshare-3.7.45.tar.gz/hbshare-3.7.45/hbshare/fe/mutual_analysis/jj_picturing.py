...
def style_pic_all(jj_base_info,asofdate,fre, th1=0.5, th2=0.5, if_percentage=True,if_prv=False,fre2='Q'):
    ...
    value_p_hbs.loc[(value_p_hbs['风格偏好'] != '均衡')
                    & (value_p_hbs[ '均衡绝对暴露(持仓)'] >= 0.7), '风格偏好'] = '均衡'

    value_p_hbs.loc[(value_p_hbs['风格偏好'] != '成长')
                    & (value_p_hbs['均衡绝对暴露(持仓)']  < 0.7)
    ...
    size_p_hbs = label_style(size_p_hbs, '集中度(持仓)', '换手率(持仓)',
                             '规模风格类型', '规模偏好', th1, th2)
    for col in ['大盘', '中小盘']:
        size_p_hbs.loc[(size_p_hbs['规模偏好'] == '均衡') & (size_p_hbs[col+'绝对暴露(持仓)'] >= 0.7), '规模偏好'] = col
...
def mutual_pool_picture(asofdate1,asofdate2):

    #read the pool file
    core_pool=\
        pd.read_excel(r"E:\GitFolder\docs\基金池跟踪\公募池跟踪\公募跟踪池.xlsx"
                      ,sheet_name='公募核心池金工标签')[['基金代码','基金名称','当前负责人员']]
    core_pool['基金代码']=[("000000"+str(x).replace('.OF',''))[-6:] for x in core_pool['基金代码']]


    #get the mutual 30 pool
    pool_30=\
        pd.read_excel(r"E:\GitFolder\docs\基金池跟踪\公募池跟踪\公募跟踪池.xlsx"
                      ,sheet_name='公募30池金工标签')[['基金代码','基金名称','当前负责人员']]
    pool_30['基金代码']=[("000000"+str(x).replace('.OF',''))[-6:] for x in pool_30['基金代码']]

    #get the agency pool
    agent_pool=\
        pd.read_excel(r"E:\GitFolder\docs\基金池跟踪\公募池跟踪\公募跟踪池.xlsx"
                      ,sheet_name='公募机构池金工标签')[['基金代码','基金名称','当前负责人员']]
    agent_pool['基金代码']=[("000000"+str(x).replace('.OF',''))[-6:] for x in agent_pool['基金代码']]


    jjdm_list=\
        core_pool['基金代码'].tolist()+\
        core_pool['基金代码'].tolist()+\
        agent_pool['基金代码'].tolist()

    #get industry_pic
    sql="""select jjdm,`一级行业类型`, `一级行业集中度`, `一级行业换手率`,`龙头占比(时序均值)`,
    `二级行业类型`, `二级行业集中度`, `二级行业换手率`, `三级行业类型`, `三级行业集中度`,`三级行业换手率`
     from jjpic_industry_p where jjdm in ({0}) and asofdate='{1}' """\
        .format(util.list_sql_condition(jjdm_list),asofdate2)
    ind_pic=pd.read_sql(sql,con=localdb)


    #get the industry stratage

    ind_detail_date = pd.read_sql("SELECT max(asofdate) as max_date from jjpic_industry_detail_1 where asofdate<='{}' "
                              .format(asofdate2),con=localdb)['max_date'][0]
    for i in range(3):
        latest_date=ind_detail_date
        sql = "SELECT * from jjpic_industry_detail_{2} where jjdm in ({0}) and asofdate='{1}' " \
            .format(util.list_sql_condition(jjdm_list), latest_date,i+1)
       ...
       '无', '无', '无', '无', '无', '无', '无', '无']
            ...
            ind_pic=pd.merge(ind_pic,tempdf[['jjdm','一级行业策略', '成长行业占比',
       '价值行业占比']],how='left',on='jjdm')
            ...
            ind_pic = pd.merge(ind_pic, tempdf[['jjdm', '行业精选个股策略', '二级行业策略', '二级价值行业占比', '二级成长行业占比',
                                                      '成长综合排名', '价值综合排名']],
                                  how='left', on='jjdm')


    #get theme pic
    sql=""" select * from jjpic_theme_p where jjdm in ({0}) and asofdate='{1}'
    """.format(util.list_sql_condition(jjdm_list),asofdate2)
    theme_pic=\
        pd.read_sql(sql,con=localdb).drop(['基金简称','asofdate'],axis=1)

    #get style pic
    sql="select * from jjpic_value_p_hbs where jjdm in ({0}) and asofdate='{1}'"\
        .format(util.list_sql_condition(jjdm_list),asofdate1)
    style_pic=pd.read_sql(sql,con=localdb)[['jjdm', '风格类型', '风格偏好', '成长绝对暴露(持仓)',
       '价值绝对暴露(持仓)', '集中度排名(持仓)', '换手率排名(持仓)', '成长暴露排名(持仓)', '价值暴露排名(持仓)']].rename(columns={'集中度排名(持仓)':'风格集中度排名',
                                                                            '换手率排名(持仓)':'风格换手率排名'})

    sql="select * from jjpic_size_p_hbs where jjdm in ({0}) and asofdate='{1}'"\
        .format(util.list_sql_condition(jjdm_list),asofdate1)
    size_pic=pd.read_sql(sql,con=localdb)[['jjdm', '规模风格类型','规模偏好', '大盘绝对暴露(持仓)',
       '中小盘绝对暴露(持仓)', '集中度排名(持仓)', '换手率排名(持仓)', '大盘暴露排名(持仓)', '中小盘暴露排名(持仓)']].rename(columns={'个股风格A':'个股交易风格',
                                                                            '个股风格B':'交易策略'}).rename(columns={'集中度排名(持仓)':'规模集中度排名',
                                                                            '换手率排名(持仓)':'规模换手率排名'})

    #get the stock wise pic
    sql="select * from jjpic_stock_p where jjdm in ({0}) and asofdate='{1}'"\
        .format(util.list_sql_condition(jjdm_list),asofdate1)
    stock_pic_a=\
        pd.read_sql(sql,con=localdb)[['jjdm', '个股风格A', '个股风格B', '个股集中度',
       '持股数量',  '平均仓位', '仓位换手率', 'PE', 'PB', 'ROE', '股息率']].rename(columns={'个股风格A':'个股交易风格',
                                                                            '个股风格B':'交易策略'})

    sql="""select jjdm,`左侧标签`,`新股次新股偏好`,`换手率`,`换手率_rank` as 换手率排名,
    `平均持有时间(出持仓前)` as 平均持有时间,`平均持有时间(出持仓前)_rank` as 平均持有时间排名
    from jjpic_stock_tp where jjdm in ({0}) and asofdate='{1}'"""\
        .format(util.list_sql_condition(jjdm_list),asofdate1)
    stock_pic_b=pd.read_sql(sql,con=localdb)

    def merge_pic(core_pool):

        core_pool=pd.merge(core_pool,ind_pic,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        core_pool=pd.merge(core_pool,theme_pic,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        core_pool=pd.merge(core_pool,style_pic,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        core_pool=pd.merge(core_pool,size_pic,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        core_pool=pd.merge(core_pool,stock_pic_a,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        core_pool=pd.merge(core_pool,stock_pic_b,how='left',
                           left_on='基金代码',right_on='jjdm').drop('jjdm',axis=1)
        return  core_pool

    core_pool=merge_pic(core_pool)
    pool_30 = merge_pic(pool_30)
    agent_pool = merge_pic(agent_pool)

    core_pool['基金代码']=[str(x)+'.OF' for x in core_pool['基金代码']]
    pool_30['基金代码']=[str(x)+'.OF' for x in pool_30['基金代码']]
    agent_pool['基金代码']=[str(x)+'.OF' for x in agent_pool['基金代码']]

    import xlwings as xw
    filename=r"E:\GitFolder\docs\基金池跟踪\公募池跟踪\公募跟踪池.xlsx"
    app = xw.App(visible=False)
    wb =app.books.open(filename)

    ws = wb.sheets['公募核心池金工标签']
    ws["A1"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool
    ws = wb.sheets['公募30池金工标签']
    ws["A1"].options(pd.DataFrame, header=1, index=False, expand='table').value = pool_30
    ws = wb.sheets['公募机构池金工标签']
    ws["A1"].options(pd.DataFrame, header=1, index=False, expand='table').value = agent_pool
...
if __name__ == '__main__':
    ...
    jjdm_list=pd.read_excel(dir)

    sql="select fqdwjz,jzrq,jjdm from st_hedge.t_st_rhb where  jjdm in ({0}) and hbdr!=99999    "\
        .format(util.list_sql_condition(jjdm_list['基金代码'].tolist()))
    navdf=hbdb.db2df(sql, db='highuser').pivot_table('fqdwjz','jzrq','jjdm')
    navdf.rename(columns=dict(zip(jjdm_list['基金代码'].tolist(),jjdm_list['代表产品'].tolist())))
    ...
    #
    # data2=pd.read_sql("select * from hbs_industry_class1_exp where jjdm in ({}) and jsrq='20221230' "
    #                   .format(util.list_sql_condition(jjdm_list)),con=localdb)
    #
    ...
    mutual_pool_picture('20221230', '20230331')
    # jjdm_list=util.get_potient_mutual_stock_funds('20201231')
    # sql="select jjdm,hb1n from st_fund.t_st_gm_nhb where tjnf=2020 and hb1n!=99999 and jjdm in ({})"\
    ...
    # save_entire_jjpic2db(asofdate1='20230331',asofdate2='20221230',if_prv=False,fre='Q')
    ...
    jjdm_list=['000924','001476','270028','377240','001410','001856','001975','005968']
    jjdm_list=['180001',  '660012','090015','003940' ,'519915']
    jjdm_list = ['001743']
    jjdm_list=['000577','000739','001410','001476','001583','001705','001856','002340','005241','005827','005968'
        ,'006624','163415','377240','450004','450009','519002','519126','519133','688888']
    jjdm_list=['410001','005244','519712','688888','006567','002289','001643','001521','005660','005161','320022','001917','001832','320003','210004','000242','000061','257010']
    jjdm_list=['005827','005241','008901','006179','519915','001832','005004','519714','004868','519710','090016','519019']
    jjdm_list=['008234','260108','007287','008133','610005','162605','320012','003940','003190','005543','006179']
    jjdm_list=['001404','001955','004666','110002','007346']
    jjdm_list=['001476','001705','002340','007449','450004','450009','005241','519126','377240','000739','005968','006624','519133','000577','001410','001583','009049','002910','005827','011251','010381']
    #'001216'
    ...
    # save_pic_as_excelfromlocaldb(asofdate='20220831',jjdm_list=jjdm_list,if_trade_history=True)
    # #
    # jjdm='000001'
