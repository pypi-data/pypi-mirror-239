...
async def get_recently_accessed(request: web.Request) -> web.Response:
    ...
    async with _lock:
        logger.debug('In synchronized zone for getting recent accesses')
        if volume_ids:
        ...
        pipeline = [{ "$match": { "$and": [
                            {'$or': [{'owner': sub}, {'$and': [{"shares.user": sub}, {'shares.permissions': {'$in': [perm.name for perm in PermissionGroup.GETTER_PERMS.perms]}}]}]},
