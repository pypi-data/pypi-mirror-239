...
async def opener(request: Request, collection: str, volume_id: Optional[str] = None, include_result=True) -> Response:
    """
    ...
    :param volume_id: the id string of the volume containing the requested HEA object. If None, the root volume is
    assumed.
    :param include_result: boolean value indicating whether to include the requested desktop object in the response
    body. For REST endpoints defined with an action containing URL parameters that are not in the endpoint's route,
    include_result must be True. The default value is True.
    ...
    return await response.get_multiple_choices(request, result if result is not None and include_result else None)
