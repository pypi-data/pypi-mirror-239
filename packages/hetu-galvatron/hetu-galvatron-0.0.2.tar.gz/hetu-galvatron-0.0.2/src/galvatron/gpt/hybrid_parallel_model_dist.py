...
def construct_hybrid_parallel_model(model, model_config, training_args, hybrid_parallel_configs):
    ...
    from .GPTLMHeadmodel_tensor_parallel import GPTMLP_tp
    ...
    for i in range(config.num_hidden_layers):
        ...
        # setattr(layer, 'mlp', GPTMLP_tp(config, tp_group = tp_groups_enc[i]))

    # [Step 2] Construct Sequantial modules
    from .GPTLMHeadmodel_pipeline import GPTEmbeddings_, GPTLayers_, GPTPreNorm_, GPTCls_
    model = PipeSequential()
    model.add_module('embeddings', GPTEmbeddings_(gpt_model))
    for i in range(config.num_hidden_layers):
        enc = GPTLayers_(gpt_model, i, i + 1)
        model.add_module('layer_%d'%i, enc)
    model.add_module('prenorm', GPTPreNorm_(gpt_model))
    model.add_module('cls', GPTCls_(gpt_model))
