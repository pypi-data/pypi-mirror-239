...
import argparse
import copy
from .llama_config_utils import config_from_checkpoint
...
def parallelism_optimization(args):
    ...
    config = config_from_checkpoint('./llama-config', args.model_size)
    hidden_size, seq_len, layer_num, head_num = config.hidden_size, config.max_position_embeddings, config.num_hidden_layers, config.num_attention_heads
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--model_size", type=str, default='gpt-1.5b', help="Model size.", choices=['llama-7b', 'llama-13b', 'llama-30b']
    )
    parser.add_argument(
