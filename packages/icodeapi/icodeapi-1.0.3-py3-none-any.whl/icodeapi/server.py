...
class IcodeServer:
    ...
    def __init__(self, api : AsyncIcodeAPI, ban : set[str] = set()):
    ...
    def CheckMessage(self,
        ...
        '''
        Autocheck the messages in messages hub, and call `checkFunc`. if `checkFunc` return True, execute the corresponding function.  
        Parameters:  
            `workList` : `list[str] | None`  
                Check messages in this list.  
                If the parameter is None, check messages in all works.  
            `checkFunc` : `Callable[[str, str], bool]`  
                After getting messages, `CheckMessage` will call this function, and full in the message content and userId.  
                If `checkFunc` return True, add the message to `msgs` list.  
                `checkFunc` should have two parameters:  
                    `content` : `str`  
                        Message content.  
                    `userId` : `str`  
                        Who send the message.  
            `autoRead` : `bool`  
                After running complete, auto read the messages.  
            `checkRead` : `bool`  
                Whether to check unread messages.  
            `checkNum` : int  
                How many messages to check.  
            `name` : str  
                The function's name.  
        `CheckMessage` will return a other function. You should full in a function(`Awaitable | Callable`) to the function.  
        When a message that requires action is detected, will call the function.  
        Parameters the function can have:  
            `content` : `str`  
                Message content.  
            `userId` : `str`  
                Who send the message.  
            `api` : `icodeapi.AsyncIcodeAPI`  
                The bot's AsyncIcodeAPI.
            `bot` : `icodeapi.server.IcodeServer`
                The IcodeServer object.
        Finally, the last Awaitable `inner` will return **The function you filled in's returns**, **Function Object** and **The function's name**.  
        Like this:
        ```python
        [
        # First call
        ...
        ```
        You can use Decorator to use it simply. 
        '''
...
class ServerPool:
    ...
    async def RunServers(self):
        coros = [i.run() for i in self.__servers]
        await asyncio.gather(* coros)
