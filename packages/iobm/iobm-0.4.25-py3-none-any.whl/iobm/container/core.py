...
class cGAN():
    def __init__(
        ...
        self.generator = DistributedDataParallel(self.generator, device_ids=[self.device], broadcast_buffers=False)
        self.discriminator = DistributedDataParallel(self.discriminator, device_ids=[self.device], broadcast_buffers=False)
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for index, batch in enumerate(progress_bar):
                ...
                D_real_loss = F.cross_entropy(D_real_output.to(self.device), real_target.to(self.device))
                ...
                D_fake_loss = F.cross_entropy(D_fake_output.to(self.device), fake_target.to(self.device))
                ...
                G_loss = F.cross_entropy(G_output.to(self.device), real_target.to(self.device))
