        ...
        for k in range(maxInnerIter):
            ...
            try:
                ...
                if SSE(1e9) <= SSE(1):
                    ...
                    if debug:
                        print('SSE(0) {SSE(0)} seems to be smaller than SSE(1) {SSE(1)}, I am going to assume it is also smaller than SSE(1e9) {SSE(1e9)} and that SSE(1e-6) {SSE(1e-6)} is smaller than them all')
