...
class CanMessage:
    ...
    :type extended_id: bool
    """
    __slots__ = 'arbitration_id', 'dlc', 'data', 'is_extended_id', 'is_fd', 'bitrate_switch', 'timestamp'

    def __init__(self, arbitration_id=None, dlc=None, data=None, extended_id=False, is_fd=False, bitrate_switch=False, timestamp=False):
...
class PDU:
    ...
    class Type:
        ...
        def to_string(type) -> str:
            if type == PDU.Type.SINGLE_FRAME:
                return "SingleFrame"
            elif type == PDU.Type.FIRST_FRAME:
                return "FirstFrame"
            elif type == PDU.Type.CONSECUTIVE_FRAME:
                return "ConsecutiveFrame"
            elif type == PDU.Type.FLOW_CONTROL:
...
class TransportLayer:
    ...
    def process_rx(self, msg):
        ...
        if not self.address.is_for_me(msg):
            self.cf_received = False
            return 
        
        st = (msg.timestamp - self.last_timestamp) * 1000
        ...
        if pdu.type == PDU.Type.CONSECUTIVE_FRAME:
            if self.first_cf is False:
        ...
        if self.rx_state == self.RxState.IDLE:
            ...
            if pdu.type == PDU.Type.SINGLE_FRAME:
                ...
                self.cf_timestamp = msg.timestamp
        ...
        self.logger.debug("<%f> (%f)\t %s\t %s\t %s" % (msg.timestamp, st, "receive", PDU.Type.to_string(pdu.type), binascii.hexlify(msg.data)))
    ...
    def process_tx(self):
        ...
        if self.pending_flow_control_tx:
            ...
            if not self.params.listen_mode: # Inhibit Flow Control in listen mode.
                flow_control_frame = self.make_flow_control(flow_status=self.pending_flowcontrol_status)
                self.logger.debug("<%f> (%f)\t %s\t %s\t %s" % (0, 0, "transmit", "FlowControl", binascii.hexlify(flow_control_frame.data)))
                return flow_control_frame;	# No need to wait.
        ...
        if output_msg is not None:
            ...
            #self.logger.debug("<%f> (%f)\t %s\t %s\t %s" % (0, 0, "transmit", PDU.Type.to_string(pdu.type), binascii.hexlify(msg.data)))
...
class CanStack(TransportLayer):
    ...
    def rx_canbus(self):
        ...
        if msg is not None:
            return CanMessage(arbitration_id=msg.arbitration_id, data=msg.data, extended_id=msg.is_extended_id, is_fd=msg.is_fd, bitrate_switch=msg.bitrate_switch, timestamp=msg.timestamp)
