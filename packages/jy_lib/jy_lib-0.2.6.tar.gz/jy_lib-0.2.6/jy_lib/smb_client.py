...
class SmbClient(SMBConnection):
    ...
    def isdir(self, path: AnyStr) -> bool:
        while True:
            try:
                fo: SharedFile = self.getAttributes(
                    service_name=self.service_name, path=path
                )
    ...
    def upload_file(self, path: AnyStr, data: AnyStr | SupportsBytes | IO) -> float:
        """上传一个已经完成的文件"""
        ...
        while True:
            try:
                ...
                size: int = self.storeFile(
                    service_name=self.service_name, path=tmp_path, file_obj=bio
                )
                self.move(src=tmp_path, dst=path)
                elapse: int = int((time.time() - t1) * 1000)
                speed: float = size / max(1.0, elapse)
                logger.debug(
                    f"文件上传成功 PID={os.getpid()} "
                    f"TIME={elapse:3d}ms SIZE={size / 1024 ** 2:4.1f}MB PATH={path}"
                )
                return speed
            except NotConnectedError:
                self.reload()
    ...
    def download_file(self, path: AnyStr, log: bool = True) -> io.BytesIO:
        assert self.check_path(path=path)
        bio = io.BytesIO()
        t1: float = time.time()
        while True:
            try:
                self.retrieveFile(
                    service_name=self.service_name, path=path, file_obj=bio
                )
                elapse: int = int((time.time() - t1) * 1000)
                size: int = bio.tell()
                if log:
        ...
        while True:
            try:
                if self.isfile(path=path):
                    self.deleteFiles(
                        service_name=self.service_name, path_file_pattern=path
                    )
                    elapse: int = int((time.time() - t1) * 1000)
                    logger.trace(
                        f"文件删除成功 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}"
                    )
                    ...
                    elapse: int = int((time.time() - t1) * 1000)
                    logger.trace(
                        f"目录删除成功 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}"
                    )
                return
            except NotConnectedError:
                self.reload()
    ...
    def rmtree(self, path: AnyStr):
        ...
        if self.isfile(path):
            ...
            items: List[SharedFile] = self.listPath(
                service_name=self.service_name, path=path
            )
            for item in items:
                ...
                if not item.isDirectory:
                    self.deleteFiles(
                        service_name=self.service_name, path_file_pattern=item_path
                    )
                else:
            ...
            try:
                ...
                if self.isdir(src):
                    if self.isdir(dst):
                        # 合并目录
                        items: List[SharedFile] = self.listPath(
                            service_name=self.service_name, path=src
                        )
                        for item in items:
                            if item.filename in [os.path.curdir, os.path.pardir]:
                                continue
                            ...
                            self.move(
                                src=item_path, dst=os.path.join(dst, item.filename)
                            )
                        ...
                        # 移动目录
                        self.rename(
                            service_name=self.service_name, old_path=src, new_path=dst
                        )
