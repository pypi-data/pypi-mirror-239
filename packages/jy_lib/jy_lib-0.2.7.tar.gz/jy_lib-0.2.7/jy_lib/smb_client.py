    ...
    def upload_dir(self, path: AnyStr, source: AnyStr):
        ...
        try:
            for f in os.listdir(source):
                remotefilepath: str = os.path.join(path, f)  # type: ignore
                sourcefilepath: str = os.path.join(source, f)  # type: ignore
                if os.path.isfile(sourcefilepath):
                    self.upload_file(path=remotefilepath, data=sourcefilepath)
                elif os.path.isdir(sourcefilepath):
                    self.upload_dir(path=remotefilepath, source=sourcefilepath)
        except Exception as e:
            elapse: int = int((time.time() - t1) * 1000)
            logger.error(f"[{e.__class__.__name__}]{e}")
    ...
    def download_dir(self, path: AnyStr, target: AnyStr):
        ...
        try:
            for f in self.listdir(path=path):
                remotefilepath: str = os.path.join(path, f.filename)  # type: ignore
                targetfilepath: str = os.path.join(target, f.filename)  # type: ignore
                if self.isfile(remotefilepath):
                    self.download_file(path=remotefilepath, target=targetfilepath)
                elif self.isdir(remotefilepath):
                    self.download_dir(path=remotefilepath, target=targetfilepath)
        except Exception as e:
            elapse: int = int((time.time() - t1) * 1000)
            logger.error(f"[{e.__class__.__name__}]{e}")
