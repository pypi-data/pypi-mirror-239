...
@app.command()
def generate_podcast(name: str, desc: str):
    ...
    # (CONTENT) Generate scripts for 3 segments
    script_segments = []
    for i in range(3):
        comment = input(f"Enter topic for segment {i + 1}: ")
        script_segment = open_ai_stuff.generate_response(
            podcast_segment.SYSTEM_PROMPT, podcast_segment.PROMPT.format(topic=comment)
        )
        script_segments.append(script_segment)  # Add the comment to the list

    # (ADS) Generate scripts for 2 advertisements
    script_ads = []
    for j in range(2):
        comment = input(f"Enter topic for advertisement {i + 1}: ")
        script_ad = open_ai_stuff.generate_response(
            podcast_ads.SYSTEM_PROMPT, podcast_ads.PROMPT.format(product=comment)
        )
    ...
    # Load music segments
    music_forest = AudioSegment.from_mp3("kyms/music/whistle-vibes-172471.mp3")
    music_beachside = AudioSegment.from_mp3(
        "kyms/music/lofi-chill-medium-version-159456.mp3"
    )
    music_feriado = AudioSegment.from_mp3("kyms/music/bolero-161191.mp3")
    music_typewriter = AudioSegment.from_mp3(
        "kyms/music/scandinavianz-thessaloniki-free-download-173689.mp3"
    )
