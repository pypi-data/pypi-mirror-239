...
class Router:
    ...
    def _health_check(self):
        ...
        for deployment in self.model_list: 
            ...
            try: 
                ...
                logging.debug(f"call made: {start_time}")
    ...
    def get_available_deployment(self,
        ...
        if len(self.healthy_deployments) > 0:
            for item in self.healthy_deployments:
                if item["model_name"] == model: # first one in queue will be the one with the most availability
                    return item
        else: 
    ...
    def completion(self,
        ...
        response = litellm.completion(**{**data, "messages": messages, **kwargs})
