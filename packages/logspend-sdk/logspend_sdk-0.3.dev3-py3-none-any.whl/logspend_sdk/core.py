...
class LogSpendLogger:
    ...
    def send(self, data):
        ...
        if not data.get("input") or not data.get("output"):
            logger.warning("Input or Output data missing. Will skip sending LLM log to LogSpend.")
            print("Input or Output data missing. Will skip sending LLM log to LogSpend.")
            return None
        ...
        with httpx.Client() as client:
            try:
                ...
                if response.is_success:
                    ...
                    print(f"HTTP Error when sending LLM log to LogSpend: {response.status_code}")
                    return None
            except Exception as e:
                logger.error(f"Error sending LLM log to LogSpend: {e}")
                print(f"Error sending LLM log to LogSpend: {e}")
                return None