...
# df = calendar.schedule.loc["2022-12-29":"2025-01-04"].copy()  # 使用 .copy() 创建一个新的 DataFrame
# df.loc[:, 'date'] = df['open'].dt.date
# df.loc[:, 'dayint'] = df['date'].apply(lambda x: int(x.strftime('%Y%m%d')))  # 注意这里应该是 '%Y%m%d' 而不是 '%Y%m%m'
# cal_df = df[['date', 'dayint']]
...
class lyycalendar_class:
    ...
    def __init__(self):
        ...
        # 后 100天日期
        next_date = today + timedelta(days=100)
...
if __name__ == "__main__":
    ...
    # 调用库中的函数，将参数传递给它们
    print(lyytc.计算相隔天数_byIndex("2023-10-27", "2023-11-08"))
