...
log = logging.getLogger('mddocs.config')
...
class Config(utils.UserDict):
    ...
    def load_file(self, config_file):
        try:
            ...
            # MdDocs knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "MdDocs encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('mddocs.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `mddocs.yml` filename will loaded.
