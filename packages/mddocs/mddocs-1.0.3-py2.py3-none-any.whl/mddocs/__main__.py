...
from mddocs import __version__
from mddocs import utils
from mddocs import exceptions
from mddocs import config
from mddocs.commands import build, gh_deploy, new, serve
...
class State(object):
    ...
    def __init__(self, log_name='mddocs', level=logging.INFO):
...
config_help = "Provide a specific MdDocs config"
dev_addr_help = ("IP address and port to serve documentation locally (default: "
                 "localhost:8000)")
strict_help = ("Enable strict mode. This will cause MdDocs to abort the build "
               "on any warnings.")
...
commit_message_help = ("A commit message to use when committing to the "
                       "Github Pages remote branch. Commit {sha} and MdDocs {version} are available as expansions")
...
ignore_version_help = "Ignore check that build is not being deployed with an older version of MdDocs."
...
def cli():
    """
    MdDocs - Project documentation with Markdown.
    """
...
def build_command(clean, config_file, strict, theme, theme_dir, site_dir):
    """Build the MdDocs documentation"""
