...
class MemcacheSocket:
    ...
    def _get_single_header(self) -> memoryview:
        ...
        # endl_pos = -1
        # # search_pos = self._pos
        # search_pos = self._pos
        # while endl_pos < 0:
        #     while search_pos < self._read - 1:  # -1 to avoid out of bounds
        #         if self._buf[search_pos] == 13 and self._buf[search_pos + 1] == 10:
        #             endl_pos = search_pos
        #             break
        #         search_pos += 1

        #     if endl_pos < 0:
        #         # Not found, read more data
        ...
        # return self._buf[pos:endl_pos]
        # # header = self._buf_view[self._pos : endl_pos]
        # if endl_pos - pos < 3:
        #     # No flags
        #     return self._buf[pos : min(pos + 2, endl_pos)], b""
        # else:
        #     return self._buf[pos : pos + 2], self._buf[pos + 3 : endl_pos]
        # # if endl_pos - self._pos < 2:
        # #     # Not enough data for a header
        # #     raise MemcacheError("Bad response. Header too short")

        # # rc_end_pos = min(endl_pos, self._pos + 2)
        # # return_code = self._buf[self._pos : min(endl_pos, self._pos + 2)]
        # # header = self._buf[self._pos : endl_pos]
        # # return header
