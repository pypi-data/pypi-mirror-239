    ...
    rule create_raw_hdf5:
        ...
        output:
            raw_hdf5 = os.path.join(
                                ...
                                "/{sample}/raw_feature_bc_matrix.h5"),
            cluster_barcode = os.path.join(
                                config["output"]["profile"],
                                "{sample}/raw_barcode_leiden_cluster.tsv")
        params:
            raw_mtx_dir = os.path.join(
                                config["output"]["host"],
                                "starsolo_count/{sample}/Solo.out/Gene/raw"),
            create_hdf5_script = config["scripts"]["create_hdf5"],
        # conda:
        #     "../envs/create_hdf5.yaml"
        log:
            os.path.join(config["logs"]["profile"],
                        "{sample}_create_hdf5.log")
        shell:
            '''
            python {params.create_hdf5_script} \
            -g {input.genes_file} \
            -m {input.matrix_file} \
            -b {input.barcodes_file} \
            -i {params.raw_mtx_dir} \
            --output_barcode {output.cluster_barcode} \
    ...
    if config["params"]["host"]["starsolo"]["soloType"]=="CB_UMI_Simple":
        rule cellbender_filter:
            ...
            params:
                ...
                gpu_on = "--cpu-threads 20" if config["params"]["host"]["cellbender"]["gpu"] == "off" else "--cuda"
            log:
                os.path.join(config["logs"]["profile"],
