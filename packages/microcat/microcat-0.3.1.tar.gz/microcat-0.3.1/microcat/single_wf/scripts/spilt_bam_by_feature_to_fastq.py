...
class FastqWriter:
    def __init__(self, barcodes, prefix, output_dir):
        self.prefix = prefix
    ...
    def write_record_to_barcode(self, rec, barcode):
        ...
        self._out_files[barcode].write(f">{rec.query_name}\n{rec.query_sequence}\n")

    def _open_file_for_barcode(self, barcode):
        output_path = os.path.join(self.output_dir, f"{self.prefix}_{barcode}_unpaired.fasta")
        self._out_files[barcode] = open(output_path, "w")
...
def main(input_bam, barcodes_file, output_prefix, contigs, barcode_tag, output_dir):
    ...
    writer = FastqWriter(barcodes=barcodes, prefix=output_prefix, output_dir=output_dir)
    ...
    for sread in alignment:
        try:
            ...
            if sread.is_paired:
                if sread.is_read1:
                    writer.write_record_to_barcode(rec=sread, barcode=f"{barcode}_r1_paired")
                elif sread.is_read2:
                    writer.write_record_to_barcode(rec=sread, barcode=f"{barcode}_r2_paired")
            else:
