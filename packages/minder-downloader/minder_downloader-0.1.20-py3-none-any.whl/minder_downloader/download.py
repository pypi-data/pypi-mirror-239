...
class MinderDatasetDownload:
    ...
    def __init__(self, 
        ...
        if columns:
            self.data_request['datasets'] = columns
        if organizations:
...
def load(datasets: list,since: Union[dt.datetime, None] = None, until: Union[dt.datetime, None] = None, organizations: Union[list, None] = None,columns:  Union[Dict, None] = None) -> pd.DataFrame:
    """
    Downloads data from the Minder research portal and returns it as a Pandas DataFrame.
    Parameters:
    - since: A datetime object representing the start date of the desired data.
    ...
    - datasets: A list of strings representing the names of the datasets to download.
    - organizations (optional): A list of strings representing the names of the organizations to download data from.
    - columns (optional): A dictionary (per dataset) of strings representing the names of the columns to download.

    Returns:
    A Pandas DataFrame with the downloaded data.
    """        
    downloader = MinderDatasetDownload(datasets, since, until, organizations, columns)
