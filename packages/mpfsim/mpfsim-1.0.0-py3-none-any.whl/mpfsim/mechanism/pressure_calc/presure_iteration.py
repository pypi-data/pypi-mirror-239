...
def multiphase_flow_iteration(
    ...
    for i_segment in range(1, N):
        ...
        mylog.logger.debug('开始计算第%s/%s段' % (i_segment, N - 1))
        # 1 根据假设增量计算的下游压力为:
        ...
        for i_iter in range(100):  # 避免无法收敛
            ...
            if flag_convergence:
                mylog.logger.debug('第%s段 经过%s轮收敛' % (i_segment, i_iter))
                break
        else:
...
def mpf(method_list, angle=None):
    ...
    P_up = unit.unit_pressure(20.18, 'MPa', 'Pa')  # 管段上游压力 (Pa)
    # lt = 2010  # 管线总长 (m) 3000
    lt = 1000  # 管线总长 (m) 3000
    fw = 0.3  # 标况的含水率 (-) 0.7  # 影响段塞流连续液相是油或是水
    # Rp = 100  # 生产气油比 (m^3/m^3)
    Rp = 300  # 生产气油比 (m^3/m^3)
    ...
    delta_P_hypo = 1e6  # 一段管段内假设的压力降落 (Pa)  # 每千米垂直10MPa
