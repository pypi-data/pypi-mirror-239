...
def make_flatten_disk(disk,
    ...
    if base is None:
        dirname = "".join(layers_name)+"-".join(doping_add_absorb)+"--uc"
        ...
        name = ""
        for i, j in zip(base_list[:-1], carbide_nitride_list):
            name = name+f"{i}{j}"

        name = name+base_list[-1]

        name_site = ""

        if terminal is not None:
            name = f"{terminal[0]}{name}{terminal[1]}"
            name_site = "^".join(terminal_site) if not isinstance(terminal_site, str) else terminal_site
        else:
            pass

        if doping is not None:
            doping = "-"+"-".join(doping)
            dop_site = "^".join(dop_site)
            name_site = "-".join([name_site, dop_site])
        else:
            doping = ""
            # dop_site = ""

        if add_absorb is not None:
            add_absorb = "@"+"@".join(add_absorb)
            add_absorb_site = "^".join(add_absorb_site)
            name_site = "-".join([name_site, add_absorb_site])
        else:
            add_absorb = ""
            # add_absorb_site = ""

        name = "".join([name, doping, add_absorb])
        dirname = "--".join((name, name_site))

    if suffix=="auto":
        suffix = f"mx-{uuid.uuid1()}"
    else:
        suffix = f"mx-{suffix}"
        suffix = suffix.replace("mx-mx-", "mx-")

    dirname = "--".join((formula, dirname, suffix))
    disk = path.Path(disk) / dirname
    return disk

def make_random_disk(disk,formula) -> path.Path:
    dirname = f"{formula}--uc--uc--mx-{uuid.uuid1()}"
    disk = path.Path(disk) / dirname
    return disk
