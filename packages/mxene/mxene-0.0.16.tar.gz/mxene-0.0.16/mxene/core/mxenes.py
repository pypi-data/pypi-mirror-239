...
from mxene.organize.disk import make_flatten_disk, make_disk
...
class MXene(Structure):
    ...
    def check_terminal_sites_by_3_layer(self, up_down="up", tol=0.3, array=None, **kwargs):
        ...
        l = min((layer3_index.shape[0], layer3_index.shape[0], layer3_index.shape[0]))

        coords1 = array[layer1_index[:l], :2]
        coords2 = array[layer2_index[:l], :2]
        coords3 = array[layer3_index[:l], :2]
        ...
        rhcp = np.min(np.sum((coords_add_hcp[:,:,None] - coords1[:,None,:]) ** 2, axis=-1) ** 0.5)
        rtop = np.min(np.sum((coords_add_top[:,:,None] - coords1[:,None,:]) ** 2, axis=-1) ** 0.5)
    ...
    def get_msg(self, detail=True, ignore_index=None, tol=0.4, force_plane=True, force_finite=False, ):
        ...
        for i in range(start, end):
            sug_index = np.where(labels == i)[0]
            names2.append([np.array(names)[sug_index], i])

        doping_add_absorb = []
        doping_add_absorb_sug_index = []

        layers_name = []
        for name_l in names2:
            names_d = Counter(name_l[0])
            if len(names_d) == 1:
                if list(names_d.values())[0] > 2:
                    layers_name.append(list(names_d.keys())[0])
                else:
                    doping_add_absorb.append(list(names_d.keys())[0])
                    doping_add_absorb_sug_index.append(name_l[1])
            else:
                cm = names_d.most_common(1)[0][0]
                layers_name.append(cm)
                for i in names_d.keys():
                    if i != cm:
                        doping_add_absorb.append(i)
                        doping_add_absorb_sug_index.append(name_l[1])

        if not detail:
            return {"formula": formula,
                    "layers_name": layers_name,
                    "doping_add_absorb": doping_add_absorb,
                    "base": None,
                    "carbide_nitride": None,
                    "terminal": None,
                    "doping": None,
                    "add_absorb": None,
                    "terminal_site": "uc",
        ...
        assert len(carbide_nitride) > 0, "carbide_nitride should more than 1."

        dop_site = []
        add_absorb_site = []

        for i, s in zip(doping_add_absorb, doping_add_absorb_sug_index):
            # just for doping
            if any((i in self._predefined_tm_list, i in self._predefined_nm_list[1:])) and i not in base:
                site = "nm" if i in self._predefined_nm_list else "tm"
                doping.append(i)
                if s >= (end - start) / 2:
                    dop_site.append(f"U_{site}")
                else:
                    dop_site.append(f"D_{site}")

            else:
                add_absorb.append(i)
                if s >= (end - start) / 2:
                    add_absorb_site.append("U_Si")
                else:
                    add_absorb_site.append("D_Si")
        ...
        if not terminal:
            ...
            # terminal_site = []
            try:
                terminal_site_up = mx.check_terminal_sites_by_3_layer(up_down="up", tol=tol)
            except BaseException as e:
                terminal_site_up = "uc"
            try:
                terminal_site_down = mx.check_terminal_sites_by_3_layer(up_down="down", tol=tol)
            except BaseException as e:
                terminal_site_down = "uc"

            if terminal_site_up != terminal_site_down:
                terminal_site = [f"D_{terminal_site_down}", f"U_{terminal_site_up}"]
            else:
                if terminal_site_down == "uc":
                    terminal_site = "uc"
                else:
                    terminal_site = [f"DU_{terminal_site_down}"]

        return {"formula": formula,
                "layers_name": layers_name,
                "doping_add_absorb": doping_add_absorb,
                "base": base,
                "carbide_nitride": carbide_nitride,
                "terminal": terminal,
                "doping": doping,
                "add_absorb": add_absorb,
                "terminal_site": terminal_site,
                "dop_site": dop_site,
                "add_absorb_site": add_absorb_site,
                }

    def get_flatten_disk_auto(self, disk='.', suffix="auto",
                              ignore_index=None, tol=0.4, force_plane=True, force_finite=False):
        try:
            msg = self.get_msg(detail=True, ignore_index=ignore_index, tol=tol,
                               force_plane=force_plane, force_finite=force_finite)
            disk = make_flatten_disk(disk=disk, **msg, suffix=suffix)
        except BaseException as e:
            # raise e
            from mxene.organize.disk import make_random_disk
            formula = self.composition.formula
            formula = formula.replace(" ","")
            disk = make_random_disk(disk, formula=formula)
        return disk

    def get_flatten_disk_manually(self, disk='.', layers_name=None, doping_add_absorb=None,
                                  base=None, carbide_nitride=None, terminal=None, doping=None,
                                  add_absorb=None,
                                  terminal_site=None, dop_site=None, add_absorb_site=None,
                                  suffix="auto"):
        formula = self.composition.formula
        formula = formula.replace(" ", "")
        disk = make_flatten_disk(disk, formula, layers_name, doping_add_absorb, base, carbide_nitride,
                                 terminal,
                                 doping,
                                 add_absorb,
                                 terminal_site,
                                 dop_site,
                                 add_absorb_site, suffix=suffix)
