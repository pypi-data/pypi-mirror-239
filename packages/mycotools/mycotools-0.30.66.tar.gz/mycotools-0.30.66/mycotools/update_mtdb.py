...
def initDB(init_dir, branch, envs, dbtype, date = None, 
           ...
           repo = None, rank2lineages = {}, prev_config = {}):
    ...
    if any(v != config[k] for k, v in prev_config.items()):
        eprint('\nERROR: discrepant configuration with initialization '
             + 'parameters.\nResume with old parameters or reinitialize.')
        eprint(f'\nOld configuration: {prev_config}')
   ...
   #             ome_fna = os.environ['MYCOFNA'] + update_ome + '.fna'
    ...
    if args.init:
        ...
        output, config = initDB( 
            ...
            rank2lineages = rank2lineages, prev_config = config
            )
        for env in envs:
