...
def results(y_true = [], y_pred = [], threshold=0.5, results_dict = {}, problem_type = 'binary classification', run_id = None):
    ...
    if len(y_true) != 0 and len(y_pred) != 0:
      ...
      preds_df.to_csv(run_dir_global+'/labels_and_predictions.csv', index=False)

      results_dict.update({'labels_and_preds_path': run_dir_global+'/labels_and_predictions.csv'})
...
def load(run_id = None, compute_results=False):
    ...
    if compute_results == True:
        ...
        df = pd.read_csv(response_dict['labels_and_preds_path'])
