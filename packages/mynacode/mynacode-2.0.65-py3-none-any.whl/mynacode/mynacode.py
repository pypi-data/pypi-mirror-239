...
def torch_model(path, metric, value, goal='max', track={}, run_id=None):
...
def keras_model(model=None, name='best_model', metric=None, value=None, goal='max', epoch=None, run_id=None):
    ...
    if model:
        ...
        if save_model_flag == 1:
          ...
          results_dict.update({'metric': metric})
          results_dict.update({'metric_goal': goal})
          results_dict.update({'best_'+metric: value})
          results_dict.update({'best_'+metric+'_epoch': epoch})
