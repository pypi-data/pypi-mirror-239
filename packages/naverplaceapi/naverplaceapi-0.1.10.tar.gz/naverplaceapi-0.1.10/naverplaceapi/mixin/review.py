...
class ReviewMixin:
    def get_visitor_reviews(self, business_id: str, page_no: int, page_cnt: int, proxy=None):
        data = query.get_visitor_reviews.create(business_id, page_no, page_cnt)
        response = requests.post("https://pcmap-api.place.naver.com/graphql", headers=HEADERS, data=json.dumps(data), proxies=proxy)
    ...
    def get_ugc_reviews(self, business_id: str, page_no: int, page_cnt: int, proxy=None):
    ...
    def get_visitor_review_stats(self, business_id: str, proxy=None):
    ...
    def get_visitor_review_photos_in_visitor_review_tab(self, store_id: str, page_no: int, page_size: int, proxy=None):
    ...
    def get_visitor_review_theme_lists(self, store_id: str, page_no, page_size, proxy=None):
