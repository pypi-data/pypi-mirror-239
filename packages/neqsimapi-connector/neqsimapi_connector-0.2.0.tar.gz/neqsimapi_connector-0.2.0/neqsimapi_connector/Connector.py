                ...
                async with semaphore:
                    ...
                    if async_endpoint:
                        ...
                        if res.status_code != 202:
                            ...
                            raise Exception(
                                "Verify url, data and that end point is async. Query was not accepted."
                            )

                        if not (
                            isinstance(res.json(), dict) and "id" in res.json().keys()
                        ):
                            raise Exception("Failed getting id from endpoint")
