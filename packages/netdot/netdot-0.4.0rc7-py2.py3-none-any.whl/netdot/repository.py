    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            auto_update (bool, optional): (When dry_run=True) Automatically prepare any changes . Defaults to False.
...
'''
    ...
    def _generate_markdown_for_class(
        ...
        lines.append(f'<a id="./generated-api-docs#class-netdot{cls.__name__}"></a>\n')
            ...
            if fields:
                ...
                for field in filter(None, fields):
                    if field.default is not None:
                        lines.append(f'| {field.name} | {field.type.__name__} | {field.default} |')
                    else:
    ...
    def show_changes_as_tables(self, terse=defaults.TERSE, select_cols=None):  # pragma: no cover  (wrapper method)
        ...
        """
        self.proposed_changes.show_changes_as_tables(terse, select_cols)
    ...
    def save_changes(self, confirm=True):
        ...
        if self._lock.acquire(blocking=False):
            ...
            raise RuntimeError('Unable to acquire lock for save_changes(). Is another thread using this repository at this moment?')
...
def _generate_xlink_by_id_getter(dataclass, xlink, xlink_class):
    def xlink_by_id_getter(
        ...
        data_list = self.connection.get_objects_by_filter(
            ...
            xlink.replace('_xlink', ''),  # Should this instead be xlink_class.NETDOT_TABLE_NAME?
            other,
            **url_params,
        )
        return [dataclass.from_DTO(data).with_repository(self) for data in data_list]
