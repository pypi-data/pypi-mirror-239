...
class DECAFPlugin(Plugin):
    ...
    def get_dag(
        ...
        if struct_learning_search_method == "hillclimb":
            ...
            raw_dag = estimators.PC(data=X).estimate(scoring_method=scoring_method, show_progress=False)
    ...
    def _encode_edges(
        ...
        for src_col in edges:
            if src_col not in self.original_features:
                raise ValueError(f"biased_edges: src_col {src_col} not found in original columns")
            for dst_col in edges[src_col]:
                if dst_col not in self.original_features:
                    raise ValueError(f"biased_edges: dst_col {dst_col} not found in original columns")

        encoded_dict: Dict[int, List[int]] = {}
        for src_col in edges:
            for encoded_src_col in encoder.get_column_info(src_col).transformed_features:
                encoded_dict[encoded_src_col] = []

                for dst_col in edges[src_col]:
                    for encoded_dst_col in encoder.get_column_info(dst_col).transformed_features:
        ...
        **kwargs: Any,
        ...
        for lcol, rcol in dag:
            if lcol not in X.columns:
                raise ValueError(f"DAG value {lcol} not found in the training dataset: {X.dataframe()}")

            if rcol not in X.columns:
                raise ValueError(f"DAG value {rcol} not found in the training dataset: {X.dataframe()}")
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        def _sample(count: int) -> pd.DataFrame:
            ...
            vals = self.model.gen_synthetic(seed_values, biased_edges=encoded_biased_edges).detach().cpu().numpy()

            output = self.baseline_generator.decode(pd.DataFrame(vals, columns=self.encoded_features)).sample(count)
