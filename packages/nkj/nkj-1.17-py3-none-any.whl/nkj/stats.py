...
PVALUE_SINGLESTAR = 0.05
PVALUE_DOUBLESTARTS = 0.01
PVALUE_TRIPLESTARTS = 0.001
_DEFAULT_PVALUE = PVALUE_SINGLESTAR
...
# 外れ値検定: Smirnov-Grubbs test (スミルノフーグラブス検定)
# 母集団が正規分布であると仮定し，平均値から一番離れているデータについて，それが有意水準以下でしか現れないとき，そのデータを除去する．

def smirnovgrubbs(x, alpha=_DEFAULT_PVALUE):
...
def outlier_removal(x, alpha=_DEFAULT_PVALUE, mode='smirnov-grubbs'):
	if (mode == 'smirnov-grubbs'):
		return smirnovgrubbs(x, alpha)
	else:
		__NOT_IMPLEMENTED__
