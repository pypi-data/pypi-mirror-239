...
#def process_sentences_and_update_tags(df):
#    """
#    Process each unique sentence, get its NER tags, and update the DataFrame.
#    """
#    updated_ner_tags = []  # List to hold the updated NER tags for all rows
#
#    for sentence in df['Sentence'].unique():
#        try:
#            ner_tags = infer(sentence)  # Get the NER tags using the actual infer function
#            sentence_words = df[df['Sentence'] == sentence]['Word'].tolist()
#            
#            if len(sentence_words) != len(ner_tags):
#                # Handle mismatch in the number of NER tags
#                print(f"Warning: Mismatch detected for sentence: {sentence} , {len(sentence_words)} , {len(ner_tags)}, {df[df['Sentence'] == sentence]['Word'].tolist()}")
#                ner_tags = ['NaN' * len(sentence_words)]  # Filling with 'NaN' or any default value
#
#            updated_ner_tags.extend(ner_tags)
#            
#        except Exception as e:
#            print(f"Error processing sentence: {sentence}. Error: {str(e)}")
#            updated_ner_tags.extend(['NaN'] * len(sentence_words))  # Filling with 'NaN' in case of error
#
#    df['NER tags'] = updated_ner_tags  # Update the 'NER tags' column in the DataFrame
#    return df
    ...
    if args.text is not None:
        ...
        for sentence in df['Sentence'].unique():  # iterating over unique sentences            
            ...
            if len(ner_tags) != len(df[i:i+len(ner_tags)]):
                print("Exit with ner tags = ", ner_tags, " and length : ", len(ner_tags), " and df is " , df[i:i+len(ner_tags)], " with length : ", len(df[i:i+len(ner_tags)]))
                return 
