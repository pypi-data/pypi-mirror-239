...
class SDOF_system():
    #TODO the SDOF system class requires severe rework because it assumes that the system is always underdamped.
    ...
    def response_params(self,x0:float, v0:float, F0:float , w:float):
        """returns the paratemers of the total solution for excitation F0*cos(wt)

        Args:
            x0 (float): initial displacement
            v0 (float): intiial velocity
            F0 (float): Force amplitude 
            w (float): frequnecy excitation
        ...
        X_p= F0/np.sqrt( (self.k-self.m*w**2)**2 + (self.c*w)**2)
        phi_p = np.arctan2(self.c*w, self.k - self.m*w)
        X_0 = np.sqrt( (x0- X_p*np.cos(phi_p))**2 
                + 1/(self.wd**2)*(self.zeta*self.wn*(x0-X_p*np.cos(phi_p))+v0-w*X_p*np.sin(phi_p))**2)    
        phi_0 = np.arctan2(self.zeta*self.wn*(x0-X_p*np.cos(phi_p))+v0-w*X_p*np.sin(phi_p), self.wd*(x0- X_p*np.cos(phi_p)) )
        return {"Xss":X_p, "phi_ss":phi_p,"X_tra":X_0,"phi_tra":phi_0, "form": f"{X_0:.5g}cos({self.wn}*t-{phi_0}) + {X_p:.5g}cos({w}*t-{phi_p})"}
