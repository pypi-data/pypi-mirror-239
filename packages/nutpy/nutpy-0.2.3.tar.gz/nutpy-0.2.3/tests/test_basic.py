...
def test_profile_TAT_trace():
    ...
    _, TAT = vs.analytical_profile(
        SSP=[45.0, 50.0, 10.0, 93.0, 7.5],
...
def test_analytic_profile_interface():
    ...
    _, TAT = Nut_test.analytical_profile(
        quantity="TAT", precession="trace", sensor_id=None, fig_flag=False
    )
...
def test_analytic_map_interface():
    ...
    _, TAT = Nut_test.analytical_map(
        quantity="TAT", precession="None", sensor_id=None, fig_flag=False
    )
...
def test_attitude_emulation():
    ...
    nut_test = Nut(SSP=SSP, Tsim=Tsim, q0=q0)
    nut_test.generate_attitude()

    _, q = kr.emulate_attitude(Tsim, 8640, q0, SSP)
...
def test_sensor_analytic():
    ...
    LB = Nut(SSP=[45, 55, 10, 93])

    _, _ = LB.analytical_profile(sensor_id=200, quantity="MAT")
...
def test_TAT():
    ...
    _, TAT_n = LB.numerical_profile(quantity="TAT", fig_flag=False)

    _, TAT_a = LB.analytical_profile(quantity="TAT", fig_flag=False)
...
def test_MAT():
    ...
    _, MAT_n = LB.numerical_profile(quantity="MAT", fig_flag=False)

    _, MAT_a = LB.analytical_profile(quantity="MAT", fig_flag=False)
...
def test_MAX():
    ...
    _, MAX_n = LB.numerical_profile(quantity="MAX", fig_flag=False)

    _, MAX_a = LB.analytical_profile(quantity="MAX", fig_flag=False)
