...
def manage(
    ...
    append_deployment_timestamp_to: str = "~/share/Deployments",
    ...
    **kwargs
    ...
    if USING_IPYTHON:
        ...
        parser.add_argument("--append_deployment_timestamp_to", type=str, default=append_deployment_timestamp_to, help=f"Path of text file to append a timestamp of deployment matching the deployment log for future reference. If empty, will not append a timestamp, Defaults to '{append_deployment_timestamp_to}'")
        ...
        append_deployment_timestamp_to = args.append_deployment_timestamp_to
    ...
    # %%#####################################
    # append deployment timestamp if desired
    #########################################

    if append_deployment_timestamp_to:
        cmd = f"{Path(log_file_path).name[:-4]} >> {append_deployment_timestamp_to}"
        logger.info(cmd)
        _stdin, _stdout, _stderr = ssh.exec_command(cmd)
        logger.info(
            f">>> {_stderr.read().decode().strip()} {_stdout.read().decode().strip()}")
