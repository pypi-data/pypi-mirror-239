...
class ProductTemplate(models.Model):
    ...
    product_catalog = fields.Boolean(
        string='Show in Product Catalog',
        default=True,
        required=False)

    def _get_combination_info_catalog(self, combination=False, product_id=False, add_qty=1,
        ...
        if self.env.context.get('website_id'):
            ...
            price = taxes.compute_all(combination_info['price'], product.currency_id, quantity_1, product, partner)[
                tax_display]
            list_price = price
            combination_info['price_extra'] = self.env['account.tax']._fix_tax_included_price_company(
                combination_info['price_extra'], product.sudo().taxes_id, taxes, company_id)
            price_extra = \
                taxes.compute_all(combination_info['price_extra'], product.currency_id, quantity_1, product, partner)[
                    tax_display]
            has_discounted_price = product.currency_id.compare_amounts(list_price, price) == 1
