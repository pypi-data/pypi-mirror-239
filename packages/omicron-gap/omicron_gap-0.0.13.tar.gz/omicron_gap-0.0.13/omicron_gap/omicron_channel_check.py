...
from omicron_gap.gap_utils import get_default_ifo, get_chan_list
...
def main():
    ...
    for group in groups:
        ...
        if frame_type not in frame_channels.keys():
            frame_channels[frame_type] = get_chan_list(ifo, frame_type, gps, gps + 1, logger)
        grp_missing = list()
        for chan in channels:
            if chan not in frame_channels[frame_type]:
                logger.critical(f'Missing channel {chan} in group: {group}')
                grp_missing.append(chan)
        missing_channels[group] = grp_missing
        nmissing += len(grp_missing)
