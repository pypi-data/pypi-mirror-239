...
class FeatureRemoving(_ComponentBase):
    ...
    def remove_features_by_correlation(
        ...
        estimator = self._select_estimator_for_removing_features(_estimator, _X, _y, _cv_obj)
    ...
    def remove_features_by_selection(
        ...
        estimator = self._select_estimator_for_removing_features(_estimator, X, _y, _cv_obj)
        ...
        self._kept_features_by_selection = X.columns[self.feature_selector_.get_support()].to_list()
        ...
        if plotting:
            self._plot_optimal_num_of_feats(min_features_to_select, self.feature_selector_)
    ...
    def _select_estimator_for_removing_features(self, estimator: EstimatorType, X, y, _cv_obj):
        if estimator is None:
            estimator = RandomForestClassifier() if self.type_estimator == "classifier" else RandomForestRegressor()
            estimator = self._find_best_random_forest_estimator(estimator, X, y, _cv_obj)
        else:
    ...
    def _find_best_random_forest_estimator(self, estimator, X, y, _cv_obj) -> EstimatorType:
        """Find the best randomforest estimator for using RandomizedSearchCV or cross_val_score."""
    ...
    def _plot_feature_importances(self, X: pd.DataFrame, feature_selector_: RFECV) -> None:
    ...
    def _plot_optimal_num_of_feats(self, min_features_to_select: int, feature_selector_: RFECV) -> None:
