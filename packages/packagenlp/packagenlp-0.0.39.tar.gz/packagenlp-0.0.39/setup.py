import setuptools
import os
import shutil
from setuptools import setup, Command
import zipfile

# Créez une classe personnalisée qui hérite de Command
class InstallCommand(Command):
    user_options = []

    def initialize_options(self):
        pass

    def finalize_options(self):
        pass

    def post_install(self):
        treetagger_zip = os.path.join(os.path.dirname(__file__), 'packagenlp', 'resources', 'TreeTagger.zip')
        target_dir = 'C:\\TreeTagger'  # Répertoire d'installation cible

        if not os.path.exists(target_dir):
            shutil.ensure_directory_exists(target_dir)
            with zipfile.ZipFile(treetagger_zip, 'r') as zip_ref:
                zip_ref.extractall(target_dir)
                
setuptools.setup(
    name="packagenlp",
    packages=['packagenlp'],
    version="0.0.39",
    author="Business & Decision",
    description="A package for NLP",
    classifiers=[
        "Programming Language :: Python :: 3",
        "License :: OSI Approved :: MIT License",
        "Operating System :: OS Independent",
    ],
    keywords=["NLP", "Français", "Langues", "Traitements"],
    download_url="https://gitlab.com/business-decision-data-science/packagenlp/-/package_files/82622627/download",
    python_requires='>=3.6',
    install_requires=[
        'pandas',
        'nltk',
        'spacy',
        'unidecode',
        'treetaggerwrapper',
        'streamlit',
        'plotly',
        'wordcloud'
    ],
    include_package_data=True,
    package_data={'': ['data/stopWords_spacy_en.csv', 'data/stopWords_spacy_fr.csv','data/image.jpg','data/Logo_B_D.png', 'resources/TreeTagger.zip']},
    cmdclass={
        'install': InstallCommand,
    },
)
