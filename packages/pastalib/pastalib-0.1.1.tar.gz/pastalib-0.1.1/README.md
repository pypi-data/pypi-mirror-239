...
<p align="center"> Tell Your Model Where to Attend: Post-hoc Attention Steering for LLMs, code for the PASTA paper (<a href="https://arxiv.org/abs/2310.14034">Zhang et al. 2023</a>). 
</p>
...
1. `pastalib` works with any models that apply causal attention by summing up query-key inner product with attention mask. `pastalib` currently only supports LLAMA, LLAMA-2 and GPT-J (more models in progress!) can be applied to LLMs in a plug-and-play manner. For example, `LlamaForCausalLM` and `GPTJForCausalLM` from `transformers`, whose attention moduels apply attention masks following `torch.matmul(query, key) + attention_mask`.  To apply the attention steering for model generation, we initialize `PASTA` as follows.
