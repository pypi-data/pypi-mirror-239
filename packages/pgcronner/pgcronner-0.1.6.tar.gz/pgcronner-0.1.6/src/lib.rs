...
fn validate_job(job: &Job) -> anyhow::Result<(), PyErr> {
    if job.name.is_empty() {
        return Err(PyValueError::new_err("Job name cannot be empty!"));
    }
    if job.command.is_empty() {
        return Err(PyValueError::new_err("Job command cannot be empty!"));
    }
    if job.schedule.is_empty() {
        return Err(PyValueError::new_err("Job schedule cannot be empty!"));
    }
    if job.command.contains("CALL") && !job.command.ends_with("();") {
        if job.source.is_empty() {
            return Err(PyValueError::new_err("Job should have a source!"));
        }
        return Err(PyValueError::new_err("Job command should end with a ();"));
    }
    Ok(())
}

fn append_prefix_to_stored_procedure(stored_procedure: &str, prefix: &str) -> String {
    format!("{}__{}", prefix, stored_procedure)
}
...
impl PgCronner {
    ...
    fn add(&mut self, job: Job) -> PyResult<bool> {
        ...
        let command = if job.command.contains("CALL") {
            let name = get_stored_procedure_name(&job.command, &job.name);
            let name = append_prefix_to_stored_procedure(&name, "pgcronner");
    ...
    fn sync(&mut self, _py: Python) -> PyResult<bool> {
        let jobs: Vec<Job> = self
            ...
            .collect::<Vec<Job>>();
        info!("Fetched {} jobs from DB", jobs.len());
        ...
        jobs.into_iter().for_each(|job| {
            ...
            job.source.is_empty().not().then(|| {
                ...
                info!("Creating stored procedure: {}", name);
                let _ =
