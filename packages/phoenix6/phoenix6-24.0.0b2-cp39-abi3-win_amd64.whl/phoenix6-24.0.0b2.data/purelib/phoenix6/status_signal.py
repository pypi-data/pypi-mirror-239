...
class StatusSignal(BaseStatusSignal, Generic[T]):
    ...
    def __init__(self, error: StatusCode, device_identifier: DeviceIdentifier, spn: int, report_if_old_func: Callable[[], None], generator: Callable[[], dict[int, 'StatusSignal']], signal_name: str, signal_type: type):
        ...
        """
        super().__init__(device_identifier, spn, signal_name)
        if error is None:
            ...
            self.__report_if_old_func = report_if_old_func
            self.__signal_type = signal_type
        else:
            self.__contains_underlying_types = False
            self.__basic_type_map = None
            self.__report_if_old_func = lambda: None
    ...
    def __refresh_mappable(self, wait_for_signal: bool, timeout_seconds: float):
    ...
    def __refresh_nonmappable(self, wait_for_signal: bool, timeout_seconds: float):
        ...
        self._all_timestamps.update(
            ...
            0, TimestampSource.Device, False
        )

    def __update_value(self, wait_for_signal: bool, timeout: float, report_error: bool):
    ...
    def wait_for_update(self, timeout_seconds: float, report_error: bool = True) -> 'StatusSignal[T]':
        """
