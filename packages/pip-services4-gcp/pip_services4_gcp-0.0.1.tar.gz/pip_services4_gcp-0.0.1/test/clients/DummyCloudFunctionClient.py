...
class DummyCloudFunctionClient(CloudFunctionClient, IDummyClient):
    def get_dummies(self, context: Optional[IContext], filter_params: FilterParams, paging: PagingParams) -> DataPage:
        response = self._call('dummies.get_dummies', context, {
    ...
    def get_dummy_by_id(self, context: Optional[IContext], dummy_id: str) -> Optional[Dummy]:
        response = self._call('dummies.get_dummy_by_id', context, {'dummy_id': dummy_id})
    ...
    def create_dummy(self, context: Optional[IContext], dummy: Dummy) -> Dummy:
        response = self._call('dummies.create_dummy', context, {'dummy': dummy.to_dict()})

        if response:
            return Dummy(**response)

    def update_dummy(self, context: Optional[IContext], dummy: Dummy) -> Dummy:
        response = self._call('dummies.update_dummy', context, {'dummy': dummy.to_dict()})

        if response:
            return Dummy(**response)

    def delete_dummy(self, context: Optional[IContext], dummy_id: str) -> Dummy:
        response = self._call('dummies.delete_dummy', context, {'dummy_id': dummy_id})
