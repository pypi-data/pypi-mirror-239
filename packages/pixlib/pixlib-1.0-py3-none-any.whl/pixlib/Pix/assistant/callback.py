...
from pixlib.Pix.assistant.host import where_hosted
...
async def update_callback(_, cb: CallbackQuery):
    ...
    if HOSTED_ON == "Heroku":
        ...
        try:
            ...
            "<b>[HEROKU]:</b> <i>Pixsuvy Deployment is in Progress...</i>"
            )
        except:
            pass
        ups_rem.fetch(ac_br)
        ...
        try:
            await cb.edit_message_text(
            "<i>Pixsuvy Updated Successfully! Userbot can be used again.</i>"
            )
...
async def changelog_callback(client, cb: CallbackQuery):
    ...
    if changelog:
        if len(changelog) > 4096:
            await cb.edit_message_text("<b>Changelog is too large, sent as a file.</b>")
            ...
            await client.send_document(
                msg.chat.id,
                "output.txt",
                caption=f"**Click the ** `Update` Button **To Update the Userbot.**",
