...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pixlib.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pixlib.Client.sign_in` or :meth:`~pixlib.Client.sign_up`).

        next_type (:obj:`~pixlib.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pixlib.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pixlib.Client.resend_code`.
    """
