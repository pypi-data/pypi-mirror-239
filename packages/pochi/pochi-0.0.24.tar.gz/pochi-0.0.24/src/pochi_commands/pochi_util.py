...
class PochiUtil:
    ...
    def get_connection(self, options):
        ...
        try:
            ...
            if (not options.default_connection.is_defined):
                LoggingManager.display_message(
                    "invalid_connection_name_issue",
                    [options.project_config.default_connection,
                    options.default_connection.connection_file_path]
                )
                has_errors = True
                return has_errors
            ...
            if ("private_key_path" in options.default_connection):
                ...
                self.__connection = snowflake.connector.connect(
                    ...
                    private_key=pkb,
                    role=getattr(options.default_connection, "role", None),
                    warehouse=getattr(options.default_connection, "warehouse", None)
                )
            else:
