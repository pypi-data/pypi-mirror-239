...
def sub(regex, cb, text, count=0):
    def capture(m):
        g = s = m.span(0)
        if m.groups():
            g = m.span(1)
        return text[s[0]:g[0]] + cb(text[g[0]:g[1]]) + text[g[1]:s[1]]
    return re.sub(regex, capture, text, count=count)
...
@click.option('-r', '--regex', help='match regex')
@click.option('-s', '--sub', 'substitution', help='text substitution')
@click.option('--add', type=int, help='perform math addition on matched regex')
def rename(paths, rename_dirs, regex, substitution, add):
    ...
    renamed = ''
    if regex is not None and (substitution is not None or add is not None):
        cb = (lambda s: str(int(s)+add)) if add is not None else (lambda s: substitution.replace('$1', s))
        renamed = sub(regex, cb, original)
    else:
