...
from .backend import ShieldRestHttpClient, ShieldAccessRequest, ShieldAccessResult
...
class PAIGPlugin:
    ...
    def __init__(self, **kwargs):
        ...
        # First reading the configurations from config file
        ...
        if not self.shield_server_key_id:
            if "shield_server_key_id" in kwargs:
                ...
                raise PAIGException(ErrorMessage.SHIELD_SERVER_KEY_ID_NOT_PROVIDED)
        ...
        if not self.shield_server_public_key:
            if "shield_server_public_key" in kwargs:
                ...
                raise PAIGException(ErrorMessage.SHIELD_SERVER_PUBLIC_KEY_NOT_PROVIDED)

        if not self.shield_plugin_key_id:
            if "shield_plugin_key_id" in kwargs:
                self.shield_plugin_key_id = kwargs["shield_plugin_key_id"]
            else:
                raise PAIGException(ErrorMessage.SHIELD_PLUGIN_KEY_ID_NOT_PROVIDED)
        ...
        if not self.shield_plugin_private_key:
            if "shield_plugin_private_key" in kwargs:
                ...
                raise PAIGException(ErrorMessage.SHIELD_PLUGIN_PRIVATE_KEY_NOT_PROVIDED)
    ...
    def read_options_from_app_config(self):
        ...
        if application_config_file is None:
            ...
            application_config_dir = os.path.join(os.getcwd(), "privacera")
...
def setup(**options):
    ...
    if _paig_plugin is not None:
        raise PAIGException(ErrorMessage.PAIG_IS_ALREADY_INITIALIZED)
...
def check_access(**kwargs):
    ...
    if not access_result.get_is_allowed():
        raise AccessControlException(403, access_result.get_response_messages())
    else:
        return access_result.get_response_messages()[0].get_response_text()


def dummy_access_denied():
    raise AccessControlException(403, "Access Denied")
