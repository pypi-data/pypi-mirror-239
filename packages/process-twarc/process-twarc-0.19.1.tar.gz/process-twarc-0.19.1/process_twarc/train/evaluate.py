...
def check_results(
    ...
    for group, df in best_trials.items():
        ...
        tokenizer = AutoTokenizer.from_pretrained(parameters["path_to_tokenizer"])
        ...
        for idx, row in tqdm(df.iterrows(), desc=f"Checking {group}"):
            ...
            if os.path.exists(path_to_model):
                ...
                metric = get_("metric_for_best_trial")
                results = lambda split: trainer.evaluate(datasets[split])[metric]

                new_df.at[idx, "dev_results"] = results("development")
                new_df.at[idx, "test_results"] = results("test")
            else:
...
def copy_best_trials(
        ...
        best_trials_dir: str,
        n_best_trials: int=int(),
        metric_for_best_trial: str="test_results"
):
    ...
    for _, df in best_trials.items():
        if n_best_trials:
            ...
            if direction == "MAXIMIZE":
                ...
                ascending=True
            df = df.sort_values(metric_for_best_trial, ascending=ascending)[:n_best_trials].reset_index(drop=True)
