...
class AstroWeather:
    ...
    def __init__(
        ...
        # Astro Routines
        self._astro_routines = AstronomicalRoutines(self._latitude, self._longitude, self._elevation, self._timezone_info, self._test_datetime)
    ...
    async def _get_forecast_data(self, forecast_type, hours_to_show) -> None:
        ...
        for row in self._weather_data_seventimer:
            ...
            if (
                ...
                # Continue hourly
                for i in range(0, 3):
                    details = self._weather_data_metno[metno_index + cnt + i].get("data", {}).get("instant", {}).get("details", {})
                    ...
                    if self._weather_data_metno[metno_index + cnt + i].get("data", {}).get("next_6_hours", {}) == {}:
                        ...
                        break
                    item["weather"] = (
                        self._weather_data_metno[metno_index + cnt + i].get("data", {}).get("next_1_hours", {}).get("summary", {}).get("symbol_code", "")
                    )
                    item["weather6"] = (
                        self._weather_data_metno[metno_index + cnt + i].get("data", {}).get("next_6_hours", {}).get("summary", {}).get("symbol_code", "")
                    )
    ...
    async def _get_deepsky_forecast(self):
        ...
        if sun_next_setting > sun_next_rising:
            _LOGGER.debug("already in darkness")
            # sun_next_setting = await self._astro_routines.sun_previous_setting_astro()

        _LOGGER.debug(
            "sun_next_setting: %s, sun_next_rising: %s, night_duration_astronomical: %s", 
            str(sun_next_setting),
            str(sun_next_rising),
            round(night_duration_astronomical / 3600, 0)
        )

        start_index = -1
        # self._forecast_data[0].forecast_time.hour % 24
        ...
        for days in range(0, 2):
            ...
            start_weather = ""
            interval_points = []
            for idx in range(start_index, start_index + int(round(night_duration_astronomical / 3600, 0) + 1)):
                ...
                _LOGGER.debug(
                    "Idex: %s, Hour of day: %s, cloud_area_fraction: %s, seeing: %s, transparency: %s, conditions: %s",
                    str(idx),
        ...
        #     # Hour of day needs to be in local time
        #     # hour_of_day = (row.forecast_time.hour + utc_to_local_diff) % 24
        #     hour_of_day = row.forecast_time.hour % 24
        #     _LOGGER.debug(
        #         "Timestamp hour: %s, hour of day: %s,  setting.hour: %s, rising.hour: %s",
        #         row.forecast_time.hour,
        #         hour_of_day,
        #         sun_next_setting.hour,
        #         sun_next_rising.hour,
        #     )
        ...
        #     _LOGGER.debug(
        #         "Hour of day: %s, Interval point: %s, cloud_area_fraction: %s, seeing: %s, transparency: %s, conditions: %s",
        #         hour_of_day,
        #         row.forecast_time.hour,
        #         str(cloud_area_fraction),
        #         str(seeing),
