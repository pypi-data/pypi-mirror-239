...
Url = Annotated[AnyUrl, UrlConstraints(max_length=2083, allowed_schemes=['http', 'https'], host_required=True)]
...
class КонтекстОбъекта:
    ...
    def _записатьДанные(
            self, режимЗаписиДокумента=РежимЗаписиДокумента.Запись, режимПроведенияДокумента=РежимПроведенияДокумента.Неоперативный
    ):
        ...
        settings = сериализатор.Фабрика.PostObject_Settings(AdditionalProperties=сериализатор.ВЗначениеБром(self.__dict__["__additionalProperties"]),
                                                            ExchangeLoadMode=self.__dict__["__isExchangeLoadMode"], DocumentWriteMode=режимЗаписиДокумента.name,
    ...
    def ЗагрузитьДанные(self):
        ...
        settings = сериализатор.Фабрика.GetObject_Settings(FieldAutoinclusion=сериализатор.Фабрика.RequestFieldAutoinclusionSettings(DefaultFields=True, CustomFields=True, Tables=True))

        refSoap = self.Клиент().SoapКлиент.service.GetObject(сериализатор.ВЗначениеБром(self.__dict__["__reference"]), settings)
...
class ОбъектСсылка(Ссылка):
    ...
    def __eq__(self, other):
        ...
        return self.Клиент() == other.Клиент() and self.ТипКоллекции() == other.ТипКоллекции() and self.ИмяКоллекции() == other.ИмяКоллекции() and self.УникальныйИдентификатор() == other.УникальныйИдентификатор()
...
class ОбъектМенеджер(МодульМенеджер):
    ...
    def ПолучитьСсылку(self, идентификатор):
        return self.__dict__["_bromClient"].Контекст().ПолучитьОбъектСсылку(self.__dict__["_moduleMetadata"].ПолноеИмя(), идентификатор)

    def ПустаяСсылка(self):
        return self.__dict__["_bromClient"].Контекст().ПолучитьОбъектСсылку(self.__dict__["_moduleMetadata"].ПолноеИмя(), УникальныйИдентификатор.Пустой())

    def СоздатьОбъект(self):
        return КонтекстОбъекта.ПолучитьКонтекстОбъекта(self.ПустаяСсылка())
...
class МетаданныеКонфигурация(УзелМетаданных):
    ...
    def __init(self):
        ...
        self.__fullNameToPathMap = {"Справочник"        : "Справочники", "Документ": "Документы", "Перечисление": "Перечисления", "ПланВидовХарактеристик": "ПланыВидовХарактеристик",
                                    "ПланСчетов"        : "ПланыСчетов", "ПланВидовРасчета": "ПланыВидовРасчета", "БизнесПроцесс": "БизнесПроцессы", "Задача": "Задачи", "Константа": "Константы",
                                    "ОбщийМодуль"       : "ОбщиеМодули", "ПараметрСеанса": "ПараметрыСеанса", "КритерийОтбора": "КритерииОтбора", "Обработка": "Обработки", "Отчет": "Отчеты",
                                    "ЖурналДокументов"  : "ЖурналыДокументов",

                                    "РегистрСведений"   : "РегистрыСведений", "РегистрНакопления": "РегистрыНакопления", "РегистрБухгалтерии": "РегистрыБухгалтерии",
                                    "РегистрРасчета"    : "РегистрыРасчета",

                                    "Последовательность": "Последовательности",

                                    "Реквизит"          : "Реквизиты", "ТабличнаяЧасть": "ТабличныеЧасти"}
...
class МетаданныеМодуль(УзелМетаданных):
    def __init__(self, родитель, metadata):
        super().__init__(родитель.Корень(), родитель, metadata.Name, metadata.FullName, metadata.Title)
...
class МетаданныеРеквизит(УзелМетаданных):
    def __init__(self, родитель, metadata):
        super().__init__(родитель.Корень(), родитель, metadata.Name, "{0}.Реквизит.{1}".format(родитель.ПолноеИмя(), metadata.Name), metadata.Title)


class МетаданныеТабличнаяЧасть(УзелМетаданных):
    def __init__(self, родитель, metadata):
        super().__init__(родитель.Корень(), родитель, metadata.Name, "{0}.ТабличнаяЧасть.{1}".format(родитель.ПолноеИмя(), metadata.Name), metadata.Title)
        ПостроительМетаданных.ЗагрузитьРеквизиты(родитель.Корень(), self, metadata.Attribute)
...
class ПостроительМетаданных:
    ...
    def ЗагрузитьРеквизиты(корень, узелРодитель, attributesSOAP):
        реквизиты = МетаданныеКоллекция(корень, узелРодитель, "Реквизиты", "{0}.Реквизиты".format(узелРодитель.ПолноеИмя()), "Реквизиты")
        if attributesSOAP:
    ...
    def ЗагрузитьТабличныеЧасти(корень, узелРодитель, tablesSOAP):
        табличныеЧасти = МетаданныеКоллекция(корень, узелРодитель, "ТабличныеЧасти", "{0}.ТабличныеЧасти".format(узелРодитель.ПолноеИмя()), "ТабличныеЧасти")
        if tablesSOAP:
...
class ОписаниеТипов:
    def __init__(
            self, типы, квалификаторыЧисла=None, квалификаторыСтроки=None, квалификаторыДаты=None, квалификаторыДвоичныхДанных=None
    ):
        self.__types = типы
...
class ПеречислениеСсылка(Ссылка):
    ...
    def __eq__(self, other):
        ...
        return self.Клиент() == other.Клиент() and self.ИмяКоллекции() == other.ИмяКоллекции() and self.Имя() == other.Имя()
...
class Сериализатор:
    ...
    def __ВЗначениеБром_Соответствие(self, значение):
        ...
        for key, value in значение.items():
            keyValues.append(self.__factory.ValueKeyValue(Key=self.ВЗначениеБром(key), Value=self.ВЗначениеБром(value)))
        return self.__factory.ValueMap(KeyValue=keyValues)

    def __ВЗначениеБром_ПеречислениеСсылка(self, значение):
        return self.__factory.ValueEnumRef(Type=значение.ПолноеИмяТипа(), Value=значение.Имя())

    def __ВЗначениеБром_ОбъектСсылка(self, значение):
        return self.__factory.ValueObjectRef(Type=значение.ПолноеИмяТипа(), Value=str(значение.УникальныйИдентификатор()))
    ...
    def __ВЗначениеБром_ОписаниеТипов(self, значение):
        ...
        return self.__factory.ValueTypeDescription(Item=types,
                                                   NumberQualifiers=None if not значение.КвалификаторыЧисла else self.__factory.NumberQualifiers(Digits=значение.КвалификаторыЧисла.Разрядность,
                                                                                                                                                 FractionDigits=значение.КвалификаторыЧисла.РазрядностьДробнойЧасти,
                                                                                                                                                 OnlyPositive=(
                                                                                                                                                         значение.КвалификаторыЧисла.ДопустимыйЗнак == ДопустимыЗнак.Неотрицательный)),
                                                   StringQualifiers=None if not значение.КвалификаторыСтроки else self.__factory.StringQualifiers(Length=значение.КвалификаторыСтроки.Длина,
                                                                                                                                                  AllowedLength=значение.КвалификаторыСтроки.ДопустимаяДлина.name),
                                                   DateQualifiers=None if not значение.КвалификаторыДаты else self.__factory.DateQualifiers(DateFractions=значение.КвалификаторыДаты.ЧастиДаты.name, ),
                                                   BinaryDataQualifiers=None if not значение.КвалификаторыДвоичныхДанных else self.__factory.BinaryDataQualifiers(
                                                           Length=значение.КвалификаторыДвоичныхДанных.Длина, AllowedLength=значение.КвалификаторыДвоичныхДанных.ДопустимаяДлина.name))

    def __ВЗначениеБром_Граница(self, значение):
        return self.__factory.ValueBoundary(Value=self.ВЗначениеБром(значение.Значение), Type=значение.ВидГраницы.name)

    def __ВЗначениеБром_МоментВремени(self, значение):
        return self.__factory.ValuePointInTime(Date=значение.Дата, Ref=self.ВЗначениеБром(значение.Ссылка))
    ...
    def __ВЗначениеБром_ТаблицаЗначений(self, значение):
        ...
        return self.__factory.ValueTable(Column=columns, Row=rows)
    ...
    def __ВЗначениеБром_ДеревоЗначений(self, значение):
        ...
        return self.__factory.ValueTree(Column=columns, Row=rows)
    ...
    def __ВЗначениеБром_ДеревоЗначений_Строка(self, дерево, rows, строки):
        for стр in строки:
            ...
            rows.append(self.__factory.DataTableRow(Property=props, Row=newRows))
    ...
    def ИзЗначенияБром(self, значение):
        ...
        if имяКласса == r'ValueKeyValue':
            return КлючИЗначение(ключ=self.ИзЗначенияБром(значение.Key), значение=self.ИзЗначенияБром(значение.Value))
    ...
    def __ИзЗначенияБром_ОписаниеТипов(self, значение):
        ...
        return ОписаниеТипов(типы=типы, квалификаторыЧисла=None if not значение.NumberQualifiers else КвалификаторыЧисла(значение.NumberQualifiers.Digits, значение.NumberQualifiers.FractionDigits,
                                                                                                                         ДопустимыЗнак.Неотрицательный if значение.NumberQualifiers.OnlyPositive else ДопустимыЗнак.Любой),
                             квалификаторыСтроки=None if not значение.StringQualifiers else КвалификаторыСтроки(значение.StringQualifiers.Length,
                                                                                                                ДопустимаяДлина[значение.StringQualifiers.AllowedLength]),
                             квалификаторыДаты=None if not значение.DateQualifiers else КвалификаторыДаты(ЧастиДаты[значение.DateQualifiers.DateFractions]),
                             квалификаторыДвоичныхДанных=None if not значение.BinaryDataQualifiers else КвалификаторыДвоичныхДанных(значение.BinaryDataQualifiers.Length,
                                                                                                                                    ДопустимаяДлина[значение.BinaryDataQualifiers.AllowedLength]))

    def __ИзЗначенияБром_Граница(self, значение):
        return Граница(self.ИзЗначенияБром(значение.Value), ВидГраницы[значение.Type])

    def __ИзЗначенияБром_МоментВремени(self, значение):
        return МоментВремени(значение.Date, self.ИзЗначенияБром(значение.Ref))
...
class ServerSideException(ConnectionError):
    class Model(pydantic.BaseModel):
        ...
        exception: "ServerSideException.Model" = pydantic.Field(default=None, validation_alias=pydantic.AliasChoices('exception', 'inner'))
...
class БромКлиент:  #
    ...
    class _HTTPAdapter(HTTPAdapter):
        ...
        def send(self, request, stream=False, timeout=None, verify=True, cert=None, proxies=None) -> Response:
            response = super().send(request=request, stream=stream, timeout=timeout, verify=verify, cert=cert, proxies=proxies)
    ...
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения="", timeout: tuple[float, float] = (3.05, 10), operation_timeout: tuple[float, float] = (3.05, 10)):
        ...
        self.__adapter = self._HTTPAdapter()

        params: dict = getattr(session := Session(), r'params')
        params.setdefault(r'l', r'ru')
        params.setdefault(r'vl', r'ru_RU')

        session.auth = HTTPBasicAuth(username=пользователь, password=пароль)

        session.mount("https://", self.__adapter)
        session.mount("http://", self.__adapter)

        from requests.exceptions import HTTPError

        try:
            self.__soapClient = Client(wsdl=публикация.rstrip('/') + "/ws/brom_api?wsdl", settings=State(
                    state=dict(strict=False, forbid_dtd=False, forbid_entities=False, forbid_external=False, xml_huge_tree=True, force_https=False, extra_http_headers=[],
                               xsd_ignore_sequence_order=True, raw_response=False)), transport=Transport(session=session, timeout=timeout, operation_timeout=operation_timeout, cache=InMemoryCache()))
    ...
    def __executeQuery(
            self, текстЗапроса, параметры=None, отбор=None, поля=None, порядок=None, пакетный=False, типОбхода=ОбходРезультатаЗапроса.Прямой, включатьВременныеДанные=False
    ):
        ...
        if параметры:
            for ключ, значение in параметры.items():
                params.append(фабрика.RequestParameter(Key=ключ, Value=self.Сериализатор().ВЗначениеБром(значение)))

        filters = []
        if отбор:
            for условиеОтбора in отбор:
                filters.append(фабрика.RequestFilter(Key=условиеОтбора.Ключ, Value=self.Сериализатор().ВЗначениеБром(условиеОтбора.Значение), ComparisonType=условиеОтбора.ВидСравнения.name))

        fields = []
        if поля:
            for поле in поля:
                fields.append(фабрика.RequestField(Key=поле.Ключ, Name=поле.Псевдоним))

        sort = []
        if порядок:
            for сортировка in sort:
                sort.append(фабрика.RequestSort(Key=сортировка.Ключ, Direction="Убыв" if сортировка.Направление == НаправлениеСортировки.Убывание else "Возр"))

        settings = фабрика.ExecuteRequest_Settings(Parameter=params, Filter=filters, Field=fields, Sort=sort, QueryResultIteration=типОбхода.name, IncludeTemporalData=включатьВременныеДанные)
    ...
    def ВыполнитьЗапрос(
            self, текстЗапроса, параметры=None, отборы=None, поля=None, порядок=None, типОбхода=ОбходРезультатаЗапроса.Прямой
    ):
        return self.__executeQuery(текстЗапроса, параметры, отборы, поля, порядок, False, типОбхода)

    def ВыполнитьПакетныйЗапрос(
            self, текстЗапроса, параметры=None, отборы=None, поля=None, порядок=None, типОбхода=ОбходРезультатаЗапроса.Прямой, включатьВременныеДанные=False
    ):
    ...
    def ПолучитьДанныеОбъекта(self, ссылка, поля=None, автозагрузкаПолей=None):
        ...
        if поля:
            for поле in поля:
                fields.append(фабрика.RequestField(Key=поле))

        settings = фабрика.GetObject_Settings(Field=fields, AddSkippedProperties=True, PropertiesHierarchyType="Hierarchy",
                                              FieldAutoinclusion=фабрика.RequestFieldAutoinclusionSettings(DefaultFields=автозагрузкаПолей.ЗагружатьСтандартныеРеквизиты,
...
class Запрос:
    ...
    def Выполнить(self, типОбхода=ОбходРезультатаЗапроса.Прямой):
        return self.__bromClient.ВыполнитьЗапрос(self.__text, self.__parameters, self.__filters, self.__fields, self.__sort, типОбхода)

    def ВыполнитьПакет(self, типОбхода=ОбходРезультатаЗапроса.Прямой, включатьВременныеДанные=False):
        return self.__bromClient.ВыполнитьПакетныйЗапрос(self.__text, self.__parameters, self.__filters, self.__fields, self.__sort, типОбхода, включатьВременныеДанные)
...
class КонтекстДанных:
    ...
    def ЗагрузитьДанныеОбъекта(self, ссылка):
        ...
        settings = фабрика.GetObject_Settings(AddSkippedProperties=True, FieldAutoinclusion=фабрика.RequestFieldAutoinclusionSettings(DefaultFields=True, CustomFields=True, Tables=True))
...
class ПеречислениеМенеджер(МодульМенеджер):
    ...
    def ПустаяСсылка(self):
        return self.__dict__["_bromClient"].Контекст().ПолучитьПеречислениеСсылку(self.__dict__["_moduleMetadata"].ПолноеИмя(), "")
...
class Селектор:
    ...
    def Выполнить(self):
        ...
        for filter in self.Отбор:
            filters.append(фабрика.RequestFilter(Key=filter.Ключ, Value=self.Клиент.Сериализатор().ВЗначениеБром(filter.Значение), ComparisonType=filter.ВидСравнения.name))

        sorts = []
        for sort in self.Порядок:
            sorts.append(фабрика.RequestSort(Key=sort.Ключ, Direction="Убыв" if sort.Направление == НаправлениеСортировки.Убывание else "Возр"))

        settings = фабрика.GetObjectList_Settings(AddSkippedProperties=True, PropertiesHierarchyType="Hierarchy",
                                                  FieldAutoinclusion=фабрика.RequestFieldAutoinclusionSettings(DefaultFields=self.АвтозагрузкаПолей.ЗагружатьСтандартныеРеквизиты,
                                                                                                               CustomFields=self.АвтозагрузкаПолей.ЗагружатьПользовательскиеРеквизиты,
                                                                                                               Tables=self.АвтозагрузкаПолей.ЗагружатьТабличныеЧасти), Limit=self.__limit, Field=fields,
                                                  Filter=filters, Sort=sorts)

        objectList = self.Клиент.SoapКлиент.service.GetObjectList(self.ТипКоллекции.name, self.ИмяКоллекции, settings)
