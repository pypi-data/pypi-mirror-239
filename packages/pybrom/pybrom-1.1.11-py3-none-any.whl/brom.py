...
class БромКлиент:  #
    ...
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения="", timeout: tuple[float, float] = (3.05, 10), operation_timeout: tuple[float, float] = (3.05, 10)):
        ...
        self.__soapClient = Client(wsdl=публикация.rstrip('/') + "/ws/brom_api?wsdl", settings=State(
                state=dict(strict=False, forbid_dtd=False, forbid_entities=False, forbid_external=False, xml_huge_tree=True, force_https=False, extra_http_headers=[], xsd_ignore_sequence_order=True,
                           raw_response=False)), transport=Transport(session=session, timeout=timeout, operation_timeout=operation_timeout, cache=InMemoryCache()))
