...
class БромКлиент:  #
    ...
    class _HTTPAdapter(HTTPAdapter):
        ...
        def send(self, request, stream=False, timeout=self.timeout, verify=True, cert=None, proxies=None) -> Response:
    ...
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения=""):
        ...
        try:
            self.__soapClient = Client(wsdl=публикация.rstrip('/') + "/ws/brom_api?wsdl", transport=Transport(session=session, cache=InMemoryCache()))
