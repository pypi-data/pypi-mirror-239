...
class БромКлиент:  #
    ...
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения="", timeout: tuple[float, float] = (3.05, 10), operation_timeout: tuple[float, float] = (3.05, 10)):
        ...
        session.mount("https://", self._HTTPAdapter())
        session.mount("http://", self._HTTPAdapter())
        from requests.exceptions import HTTPError
        try:
            self.__soapClient = Client(wsdl=публикация.rstrip('/') + "/ws/brom_api?wsdl",
                                       transport=Transport(session=session, timeout=timeout, operation_timeout=operation_timeout, cache=InMemoryCache()))
