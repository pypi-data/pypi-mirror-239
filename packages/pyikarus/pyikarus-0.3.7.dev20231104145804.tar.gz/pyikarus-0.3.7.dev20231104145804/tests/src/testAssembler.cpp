...
void checkAssembledQuantities(TestSuiteType& t, SparseType& sType, DenseType& dType, DOFSize dofSize) {
  ...
  t.check(sType.rows() == static_cast<Eigen::Index>(dofSize)) << "DOFsCheck via rows: " << sType.rows() << "rows and " << dofSize << " DOFs";
  if (not(std::is_same_v<SparseType, Eigen::VectorXd>))
    t.check(sType.cols() == static_cast<Eigen::Index>(dofSize)) << "DOFsCheck via columns: " << sType.cols() << "cols and " << dofSize << " DOFs";
}
