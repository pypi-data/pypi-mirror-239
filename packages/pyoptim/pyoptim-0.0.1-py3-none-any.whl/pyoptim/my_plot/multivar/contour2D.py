...
def sgd(f,X,tol=0.01,step_size=0.01):
...
#--------II. MDGS-loop---------------------------------------------------------------------------------------------------------------
    ...
    print('Le plot contour2D apparaitra dans quelques secondes ...')
    print('Merci de patienter !')
    fig, ax = plt.subplots(figsize=(15,15))
    
    theta0_grid = np.linspace(-1100,1100,1000)
    theta1_grid = np.linspace(-1100,1100,1000)
    J_grid=np.empty((1000,1000))
    for i in range(1000):
        for j in range(1000):
            J_grid[i,j]=f([theta0_grid[i], theta1_grid[j]])
    ...
    G=[]
    for k,l in zip(Coordonnées_x,Coordonnées_y):
        G.append([k,l])

    N=len(G)

    for j in range(1,N):
        ax.annotate('', xy=G[j], xytext=G[j-1], arrowprops={'arrowstyle': '->', 'color': 'r', 'lw': 1}, va='center', ha='center')
    ax.scatter(*zip(*G),c='k',s=40, lw=0)
...
#__________________________________________________________________________________________________________________________________
    ...
    ax.scatter([2]*2,[0.5]*2,s=[50,10], color=['k','w'])
