...
# NOTE
# This implementation is not memory efficient for cases when there are many features
# with low number of bins and only few features with high number of bins.
# If this is your case and it becomes a problem, just split features into groups using
# the number of bins as a guidance and embed the groups separately.
class PiecewiseLinearEmbeddings(nn.Module):
    ...
    **Shape**
    ...
    - Input: ``(*, n_features)``
    - Output
        - ``d_embedding is not None``: ``(*, n_features, d_embedding)``
        - ``d_embedding is not None and flat_compact``: ``(*, total_n_bins)``

    where ``total_n_bins = sum(len(b) - 1 for b in bins)``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            flat_compact: if True, then the module's output is two dimensional and
                embeddings becomes "Q"/"T" depending on how bins were computed.
        """
        if d_embedding is None:
            # Constructing Q/T embeddings of the shape `(*, n_features, max_n_bins)`
            # is not allowed.
            if not flat_compact:
                raise ValueError('if d_embedding is None, flat_compact must be True')
            if activation:
                raise ValueError(
                    'activation can be True only when d_embedding is not None'
                )
        else:
            if flat_compact:
                raise ValueError(
                    'flat_compact can be True only when d_embedding is None'
                )
            if d_embedding <= 0:
                raise ValueError(
        ...
        # mask is false for padding bins.
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # This is the formula from Equation 1 in the paper.
                        ...
                        itertools.chain.from_iterable(
                            [
                                ...
                                *(
                                    []
                                    if n_bins == 2
                                    else [x[..., 1 : count - 1].clamp(0.0, 1.0)]
                                ),
        ...
        if self.linear is not None:
            ...
            assert self._flat_compact, 'Internal error'
            return (
                x.flatten(-2)
                if self._same_bin_count
                # else torch.cat(
                #     [x[..., i, :count] for i, count in enumerate(self._bin_counts)],
                #     dim=-1,
                # )
                else x[:, self.mask]
            )
