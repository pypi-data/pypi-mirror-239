...
class ExtractZohoDesk(SalureConnect):
    ...
    def get_all_zoho_tickets(self, query_params=""):
        """
        This function gets the zoho contact information from zoho desk and saves the data to df_zoho_contacts
        :return:
        """
    ...
    def _single_call(self, url):
        """
        This function helps the API calls to do a single call in one function
        :return:
        """
        response = requests.request("GET", url, headers=self.zoho_header, data=self.zoho_payload)
        if response.status_code == 401:
            self._initialize_zoho_header()
            response = requests.request("GET", url, headers=self.zoho_header, data=self.zoho_payload)
