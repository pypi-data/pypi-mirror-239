...
def add_lvol_ha(name, size, host_id_or_name, ha_type, pool_id_or_name, use_comp, use_crypto,
    ...
    lvol.hostname = host_node
    lvol.node_id = host_node
...
def add_lvol_on_node(lvol, snode, ha_comm_addrs=None, ha_inode_self=None):
    ...
    ret = rpc_client.bdev_distrib_create(
        ...
        lvol.distr_bs, names, lvol.distr_chunk_bs, ha_comm_addrs, ha_inode_self)
    if not ret:
        logger.error("failed to create Distr bdev")
        return False
    ...
    if not ret:
        logger.error("failed to create lvs")
        # return False
    ret = rpc_client.create_lvol(lvol.lvol_name, lvol.size, lvol.lvs_name, lvol.uuid)
    if not ret:
        logger.error("failed to create LVol on the storage node")
        return False, "failed to create LVol on the storage node"
...
def recreate_lvol(lvol_id):
    ...
    ret = rpc_client.bdev_distrib_create(
        lvol.base_bdev, lvol.vuid, lvol.ndcs, lvol.npcs, num_blocks,
        lvol.distr_bs, names, lvol.distr_chunk_bs)
    if not ret:
        logger.error("failed to create Distr bdev")
        return False

    ret = rpc_client.create_lvstore(f"LVS_{lvol.vuid}", lvol.base_bdev)
    if not ret:
        logger.error("failed to create lvs")
        return False

    ret = rpc_client.create_lvol(lvol.get_id(), lvol.size, f"LVS_{lvol.vuid}")
    if not ret:
        logger.error("failed to create LVol on the storage node")
        return False, "failed to create LVol on the storage node"
    ...
    logger.info(f"add lvol {lvol.get_id()} to subsystem")
    ret = rpc_client.nvmf_subsystem_add_ns(subsystem_nqn, lvol.get_id())
