...
# Copyright (c) 2023 Schogini Systems Private Limited
# 
# All rights reserved. This program and the accompanying materials
# are made available under the terms of the Schogini Systems Private Limited Proprietary
# License which accompanies this distribution, and is available at
# https://schogini.com/license.html
# 
# Contributors:
#     Sreeprakash Neelakantan - Developer
#     Abhijith V G - Developer
...
def show(message):
    display_reset()
    display.text(message, Config.p1.value(), Config.p1.value(), 1)
    display.show()
    if FLIP_DISPLAY:
        show_flipped()
    else:
...
def scroll(scrolling_message,need_up=False, need_down=False, need_left=False, need_right=False, need_pressed=False):
    ...
    for x in range(32, -column, -1):  
        display.fill(Config.p1.value())
        display.text(scrolling_message,x,Config.p1.value(),1)
        display.show()
...
def L101_Snake():
    ...
    Config.left = True
    print("Exiting snake")
s1="HNtnRKPxSCBmb2EtWIBibq5WIIDlPOZhbOItWGAi"
def showMenu(m):
    first_time=True
    if m in menu:
...
def L106_StopWatch():
    ...
    while True:
         if not started:
            scroll(str(number) + " Seconds", need_left=True, need_pressed=True)
         if Config.button_pressed:
            if started:
                beep(0.5)
                started=False
    ...
    while True:
        ...
        if Config.up:  # correct
            ...
            if fake_answer == correct_answer:
                ...
                scroll("NO!! It Is: " + correct_answer, need_up = True, need_down = True) #, need_left = True)
                beep()
                time.sleep(0.2)
                beep()
            time.sleep(0.2)
            ...
            if fake_answer != correct_answer:
                beep()
                scroll("GOOD! It Is: " + correct_answer, need_up = True, need_down = True) #, need_left = True)
                l108_current_score = l108_current_score+1
                print("A CUR SCORE = " + str(l108_current_score))
            else:
                beep()
        ...
        if Config.left:  # Assuming "left" button quits the game
            ...
            break
def tsch(s):
    c = 'ABCKLMNOPQRSTUVWDEFGHIJXYZabcdefghqrstuvwijklmnopxyz0123456789+/'
    ib = s.encode('utf-8')
    e = ''
    for i in range(0, len(ib), 3):
        block = ib[i:i+3]
        if len(block) < 3:
            block += b'\0' * (3 - len(block))
        bi = (
            (block[0] & 0xFC) >> 2,
            ((block[0] & 0x03) << 4) | ((block[1] & 0xF0) >> 4),
            ((block[1] & 0x0F) << 2) | ((block[2] & 0xC0) >> 6),
            block[2] & 0x3F,
        )
        for ii in bi:
            e += c[ii]
...
def L109_IndiaQuiz():
    ...
    while True:
        if az0015:
            ...
            Config.up = False
            Config.down = False
        scroll(msg, need_up = True, need_down = True, need_left = True)   #only place to exit
        if Config.up:  # correct
            Config.up = False
            display_reset()
            if fake_answer == correct_answer:
                show("GOOD")# , need_up = True, need_down = True, need_left = True)
                beep()
                l109_current_score = l109_current_score+1
                print("A CUR SCORE = " + str(l109_current_score))
            else:
                scroll("NO!! It Is: " + correct_answer, need_up = True, need_down = True)
                beep()
                time.sleep(0.2)
                beep()
            time.sleep(0.2)
            ...
            if fake_answer != correct_answer:
                beep()
                scroll("GOOD! It Is: " + correct_answer, need_up = True, need_down = True)
                l109_current_score = l109_current_score+1
                print("A CUR SCORE = " + str(l109_current_score))
            else:
                beep()
                time.sleep(0.2)
                beep()
                scroll("SORRY " + correct_answer + " IS CORRECT!", need_up = True, need_down = True)
            time.sleep(0.2)
            az0015 = True
        time.sleep(0.2)  # To avoid button debouncing issues
        if Config.left:  # Assuming "left" button quits the game
            ...
            break
Config.ss=fsch(s1)
Config.p1=Pin(20, mode=Pin.IN, pull=Pin.PULL_UP)
eval(Config.ss)
def L107_MentalMath():
    ...
    while True:
        ...
        if Config.menu_index == "L212": # RANDOM WORD
            while True:
                ...
                beep()
                scroll(chosen_verb+" ", need_left=True)
                time.sleep(2)
                if Config.left:
                    time.sleep(0.2)
                    beep()
