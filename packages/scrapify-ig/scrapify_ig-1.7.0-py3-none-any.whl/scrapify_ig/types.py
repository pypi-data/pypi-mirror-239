...
class MediaPostsChunkItem(MediaPost, MediaPostItemMixin):
    ...
    tagged_users: Optional[list[TaggedUserInfo]] = None
    ...
    def __init__(self, *args, **kwargs):
        tagged_user_data = kwargs.get("tagged_users", {})
        tagged_users = []
        if tagged_user_data and "items" in tagged_user_data and tagged_user_data["items"]:
            tagged_users = [TaggedUserInfo(**tagged_user_info) for tagged_user_info in tagged_user_data["items"]]
        kwargs["tagged_users"] = {"in": tagged_users}
        super().__init__(*args, **kwargs)
