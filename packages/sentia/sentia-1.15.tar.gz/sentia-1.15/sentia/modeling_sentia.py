...
class ImageGenerationHead(nn.Module):
    def __init__(self, config: SENTIAConfig):
        ...
        self.generator = nn.Sequential(
            nn.ConvTranspose1d(genererator_size, 256, kernel_size=3),
            nn.SiLU(),
            ...
            nn.ConvTranspose1d(64, 512*512*3, kernel_size=3),
            nn.Tanh()
        )
    ...
    def forward(self, x):
        x = x.transpose(1, 2)
        y = self.generator(x).transpose(1, 2).unsqueeze(0)
        return y
class SENTIAWithGAN(SENTIAPreTrainedModel):
    ...
    def __init__(self, config: SENTIAConfig, add_latent=True):
        ...
        self.encoder_layers = nn.ModuleList([SENTIADecoderLayer(config) for _ in range(config.num_hidden_layers // 2)])
        self.norm = SENTIARMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class SENTIAForImageGeneration(SENTIAPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if labels.dim() > 2:
                raise RuntimeError(
                    f"labels must be a 2 dimensional tensor of size (bsz, image_size,)"+
                    f"but is of size {labels.size()}"+
                    f"Make sure to flatten the label images before passing them."
                )
        # Text encoder
        ...
        image_outputs = torch.clamp(self.image_head(encoder_states.last_hidden_state), 0, 1)
        ...
        if labels is not None:
            ...
            mse_loss = nn.MSELoss()(image_outputs[0, 0, 0, :].view(512, 512, 3), labels[0, :].view(512, 512, 3))
