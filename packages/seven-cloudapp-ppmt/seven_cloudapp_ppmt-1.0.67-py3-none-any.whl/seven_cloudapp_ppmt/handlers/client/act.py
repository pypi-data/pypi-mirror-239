...
@LastEditTime: 2023-11-03 15:53:31
...
class PrizeListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if self.check_lpush(queue_name, open_id) == False:
            return self.reponse_json_error("HintMessage", "当前人数过多,请稍后再来")
        machine_info_model = MachineInfoModel(context=self)
        act_prize_model = ActPrizeModel(context=self)
        ...
        identifier = self.acquire_lock(queue_name,acquire_time=10,time_out=5)
        ...
        for act_prize in act_prize_process_list:
            update_result = act_prize_model.update_table("surplus=surplus-1", "id=%s", params=[act_prize.id])
