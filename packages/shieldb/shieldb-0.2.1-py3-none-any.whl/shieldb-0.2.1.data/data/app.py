    ...
    if conn is not None:
        if command_line_args.action == 'delete':
            if command_line_args.table and command_line_args.percentage:
                confirm = input(f'{command_line_args.table} will be masked, do you confirm? (Y/N): ').lower()
                if confirm != 'n':
            ...
            if command_line_args.table:
                if command_line_args.columns is not None:
                    confirm = input("{} columns will be masked, do you confirm? (Y/N): ".format(', '.join(command_line_args.columns))).lower()
                else:
                    confirm = input("All columns will be masked, do you confirm? (Y/N): ").lower()
                if confirm != 'n':
