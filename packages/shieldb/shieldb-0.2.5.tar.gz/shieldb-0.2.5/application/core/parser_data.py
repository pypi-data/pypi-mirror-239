...
def init_data_processing():
    parser = argparse.ArgumentParser(prog='PROG', usage='%(prog)s [options]', description='Script for deleting or masking data from a database\nIn order for '
                                                 'the project to work, you need to set the db to work with in the '
                                                 'first stage. The variable you need to set should be SQLALCHEMY_DATABASE_URI')
    ...
    parser.add_argument('--percentage', type=float, default=0, help='Percentage of data to be deleted (default: 0), PERCENTAGE SHOULD BE BETWEEN 0 AND 100.')
    parser.add_argument('--columns', nargs='+', default=[], help='The columns to be masked will be entered with spaces.')
