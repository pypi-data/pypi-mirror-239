...
def find_peers_china(industry='',top=5,itype='T'):
    """
    功能：
    ...
    if industry != '':
        if not isinstance(industry,str):
            print("  #Error(find_peers_china): expecting an industry name for",industry)
            return None
        
        ddict=mixed_industry_stocks(industries=[industry],top=top,printout=True)
        tickerlist=list(ddict)
