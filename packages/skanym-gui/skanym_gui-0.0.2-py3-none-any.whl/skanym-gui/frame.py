...
def load_animator_1():
    ...
    try:
        ...
        # gh_utils.trace(str(SERIALIZED_ANIMATION_PATH))
        # gh_utils.trace(animated_models[anim_1])
        file_path = SERIALIZED_ANIMATION_PATH.joinpath(animated_models[anim_1] + ".pkl")
...
def load_animator_2():
    ...
    try:
        file_not_found_message = None
        file_path = SERIALIZED_ANIMATION_PATH.joinpath(animated_models[anim_2] + ".pkl")
        animator = load_serialized(file_path)
        joint_list = animator.skeleton.as_joint_list()
        animator.skeleton.compute_bind_pose()
...
if is_open == 1:
    ...
    if import_button := gh_imgui.button("Import...", 200, 20):
