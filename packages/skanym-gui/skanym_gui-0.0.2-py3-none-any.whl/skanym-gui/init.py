...
gh_utils.trace("HELLO FROM PYTHON INIT SCRIPT")
...
# LEARN AND TEST OS PATHLIB
SERIALIZED_ANIMATION_PATH = Path("../animated_models/serialized")
RAW_ANIMATION_PATH = Path("../animated_models/raw")

# Data folders
if not SERIALIZED_ANIMATION_PATH.exists():
    SERIALIZED_ANIMATION_PATH.mkdir(parents=True)
if not RAW_ANIMATION_PATH.exists():
    RAW_ANIMATION_PATH.mkdir(parents=True)
...
gh_imgui.file_browser_init(file_browser_flags)

gh_imgui.file_browser_set_title("Import external .fbx or .glb file")
gh_imgui.file_browser_set_current_directory(str(RAW_ANIMATION_PATH))
