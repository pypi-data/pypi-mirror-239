...
from snb_node.tools.sysinfo import get_sys_info
from http.client import responses
from jinja2 import TemplateNotFound

from snb_node.tools.utils import url_path_join, url_is_absolute, url_escape, urldecode_unix_socket_path, is_hidden
#from i18n import combine_translations
...
class AuthenticatedHandler(web.RequestHandler):
    """
    desc: 经过身份验证的用户的requestthandler
    """
    ...
    def content_security_policy(self):
        ...
        return '; '.join([ "frame-ancestors 'self'"])
...
class IPythonHandler(AuthenticatedHandler):
    ...
    def contents_js_source(self):
        self.log.debug("Using contents: %s", self.settings.get('contents_js_source',
                                                               'snb_node/contents'))
        return self.settings.get('contents_js_source', 'snb_node/contents')
