...
from snb_node.base.zmqhandlers import AuthenticatedZMQStreamHandler, deserialize_binary_message
...
class MainKernelHandler(APIHandler):
    ...
    def get(self, ws_uid):
        ...
        res = {"code": 200, "msg": "success", "data": kernels}
...
class KernelHandler(APIHandler):
    ...
    # @web.authenticated
    @gen.coroutine
    def delete(self, ws_uid, kernel_id):
        km = self.kernel_manager
...
class KernelActionHandler(APIHandler):
    ...
    # @web.authenticated
    @gen.coroutine
    def post(self,ws_uid, kernel_id, action):
        km = self.kernel_manager
        if action == 'interrupt':
        ...
        if action == 'restart':
            ...
            try:
                ...
                model = yield maybe_future(km.kernel_model(kernel_id))
                res = {"code": 200, "msg": "success", "data": model}
                self.write(json.dumps(res, default=json_default))
        self.finish()
...
class ZMQChannelsHandler(AuthenticatedZMQStreamHandler):
    ...
    def get(self,ws_uid, kernel_id):
        ...
        try:
            ...
            self.set_status(status_code=404, reason=str(e))
            self.close()
...
_ws_uid = r"(?P<ws_uid>[^/]+)"
default_handlers = [
    (fr"/api/snb/node/kernels/{_ws_uid}", MainKernelHandler),
    (fr"/api/snb/node/kernels/{_ws_uid}/{_kernel_id_regex}", KernelHandler),
    (fr"/api/snb/node/kernels/{_ws_uid}/{_kernel_id_regex}/{_kernel_action_regex}", KernelActionHandler),
    (fr"/api/snb/node/kernels/{_ws_uid}/{_kernel_id_regex}/channels", ZMQChannelsHandler),
]
