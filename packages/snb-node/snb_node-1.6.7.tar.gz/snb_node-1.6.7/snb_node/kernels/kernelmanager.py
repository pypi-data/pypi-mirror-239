...
from snb_node.config.Config import cull_idle_timeout_int
from snb_node.tools.utils import maybe_future, to_os_path, exists
from snb_node.tools.tz import utcnow, isoformat
from ipython_genutils.py3compat import getcwd

from snb_node.prometheus.metrics import KERNEL_CURRENTLY_RUNNING_TOTAL
...
class MappingKernelManager(MultiKernelManager):
    """
    desc: 一个处理笔记本映射和HTTP错误处理的KernelManager
    """

    # default: 为HasTraits对象上的Trait分配动态默认值的装饰器
    ...
    # validate：当Trait被设置时，用于注册HasTraits对象状态的交叉验证器的装饰器
    @validate('root_dir')
    ...
    # 超时时间(以秒为单位)，在此之后，内核被认为是空闲的并准备就绪。值为0或更低将禁用剔除。非常短的超时可能导致内核崩溃。
    # 针对网络连接差的用户进行筛选
    cull_idle_timeout = Integer(cull_idle_timeout_int, config=True,
                                help="""Timeout (in seconds) after which a kernel is considered idle and ready to be 
                                culled. Values of 0 or lower disable culling. Very short timeouts may result in kernels 
                                being culled for users with poor network connections."""
                                )

    # 删除空闲kernel
    cull_interval_default = 300  # 5 minutes
    cull_interval = Integer(cull_interval_default, config=True,
                            help="""The interval (in seconds) on which to check for idle kernels exceeding the cull 
                            timeout value."""
                            )

    # 是否考虑剔除具有一个或多个连接的内核。 仅当cull_idle_timeout > 0时有效
    cull_connected = Bool(False, config=True,
                          help="""Whether to consider culling kernels which have one or more connections. 
                          Only effective if cull_idle_timeout > 0."""
                          )
    # 是否考虑对忙碌的内核进行剔除。仅当cull_idle_timeout > 0时有效
    cull_busy = Bool(False, config=True,
                     help="""Whether to consider culling kernels which are busy. 
                     Only effective if cull_idle_timeout > 0."""
                     )

    # 是否应该在内存中缓冲来自前端已断开连接的内核的消息。
    # 当为True(默认值)时，消息将被缓冲并在connect上重试，以避免由于连接中断而丢失消息。
    # 如果长时间运行的内核在没有前端连接的情况下会产生过多的输出，则禁用
    buffer_offline_messages = Bool(True, config=True,
                                   help="""Whether messages from kernels whose frontends have disconnected should be 
                                   ...
                                   long-running kernels will produce too much output while no frontends are connected.
                                    """
                                   )

    # 放弃内核的超时时间(秒)。在启动和重新启动内核时，我们通过发送 kernel_info_requests 来检查内核是否正在运行和响应。
    # 这将设置以秒为单位的超时时间，用于判断内核在被假定为死亡之前所需要的时间。这会影响MappingKernelManager(处理内核重启)
    # 和ZMQChannelsHandler(处理启动)。
    kernel_info_timeout = Float(60, config=True,
                                help="""Timeout for giving up on a kernel (in seconds). On starting and restarting 
                                kernels, we check whether the kernel is running and responsive by sending 
                                kernel_info_requests. This sets the timeout in seconds for how long the kernel can take
                                before being presumed dead. This affects the MappingKernelManager 
                                (which handles kernel restarts) and the ZMQChannelsHandler (which handles the startup).
                                """
                                )
