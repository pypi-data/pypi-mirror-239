...
class DeltaTableManager:
    ...
    def loadTable(self, tableName, offset=None, chunk_size=100000, bucket=None, dpath=None):        
    ...
    def loadTables(self, tableNames, bucket=None, dpath=None):
    ...
    def delSert(self, dataFrame, condition, tableName, bucket=None, dpath=None):
        ...
        if DeltaTable.isDeltaTable(self.spark, target_table) :  # 테이블이 이미 존재하는 경우에만 DELETE 실행
            ...
            condition = condition.lstrip("where").lstrip("WHERE").lstrip() # WHERE 시작인 경우 제거 
