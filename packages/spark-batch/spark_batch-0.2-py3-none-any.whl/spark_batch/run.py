...
def main():
    ...
    if not hasattr(args, 'config'):
        print("Please specify a configuration file using the --config option.")
        parser.print_help()
        return
    ...
    if args.subcommand == "ingest_full":
        ...
        source_df, target_df = em.ingest_full(args.source_objects, args.target_object)
    elif args.subcommand == "ingest_increment":
        em.init_rsm(args.source_type, args.source_topic, args.target_type, args.target_topic, args.batch_size)

        source_inc_query = args.source_inc_query
        target_condition = args.target_condition
        source_df, target_df = em.ingest_increment(args.source_objects, args.target_object, source_inc_query, target_condition)
