...
class EltManager:
    ...
    def ingest_full(self, source_objects, target_object) :    
        ...
        self.logger.info(f"ETL 시작 : [ {targetInfo} ]")
        ...
        while True:
            # Oracle 데이터 읽기
            source_df = self.source_tm.loadTable(sourceTable, offset, self.chunk_size)
            ...
            self.logger.info(f"소스 로딩 Chunk : {sourceInfo} / offset={offset}, self.chunk_size={chunk_read_size} / elipsed={datetime.datetime.now() - start_time}")
            ...
            if offset == 0:
                ...
                self.target_tm.saveTable(source_df_lower, targetTable, mode="overwrite")
            else: 
                self.target_tm.saveTable(source_df, targetTable, mode="append")

            self.logger.info(f"타켓 저장 Chunk : {targetInfo} / elipsed={datetime.datetime.now() - start_time}")

            offset += chunk_read_size

        self.logger.info(f"소스 로딩 데이터 수 : {sourceInfo} ({offset})")

        target_df = self.target_tm.loadTable(targetTable)     
        self.logger.info(f"타겟 저장 데이터 수 : {targetInfo} ({target_df.count()})")

        self.logger.info(f"ETL 완료 : [ {targetInfo} / {offset == target_df.count()} / {datetime.datetime.now() - start_time} ]")    
    ...
    def ingest_increment(self, source_objects, target_object, source_inc_query, target_condition) :    
        ...
        source_df = self.source_tm.queryTable(source_inc_query, source_objects)
        source_read_size = source_df.count()
        self.logger.info(f"소스 로딩 : {sourceInfo} / source_size={source_read_size} / elipsed={datetime.datetime.now() - start_time}")
        ...
        self.logger.info(f"타켓 저장 : {targetInfo} / delsert_size={after_count - before_count + del_count} (before={before_count}, after={after_count}, del={del_count}) / elipsed={datetime.datetime.now() - start_time}")
...
# source_df, target_df = em.ingest_increment(source_objects, target_object, source_inc_query, target_condition)
    ...
    def run_order_full_load(self, target_type, target_topic, from_date="1900-01-01", to_date="2999-12-31", frequency="Day", target_object=None):
        ...
        self.logger.info(f"Order Full Load 시작: {target_type} {target_topic} ({bdf.count()})")
        ...
        for row in grouped_df.collect():
            self.logger.info(f"Order Group 시작 : source_type={row.source_type}, source_topic={row.source_topic}, target_type={row.target_type}, target_topic={row.target_topic}")
            self.init_rsm(row.source_type, row.source_topic, row.target_type, row.target_topic, 500000)    

            
            for data_row in row.data_list :
                self.logger.info(f"Order Object 시작: source_object={data_row.source_object}, target_object={data_row.target_object}")
                if len(data_row["source_object"]) > 1  : # 2개 이상 테이블인 경우 Incremental Load 로 처리 (1900-01-01 ~ 2999-12-31)
                    #self.run_order_inc_load(target_type, target_topic, from_date, to_date, frequency, target_object)
                ...
                self.logger.info(f"Order Object 완료: source_object={data_row.source_object}, target_object={data_row.target_object}")
                    
            self.logger.info(f"Order Group 완료 : source_type={row.source_type}, source_topic={row.source_topic}, target_type={row.target_type}, target_topic={row.target_topic}")
            
        self.logger.info(f"Order Full Load 완료: {target_type} {target_topic} ({bdf.count()})")
    ...
    def run_order_inc_load(self, target_type, target_topic, from_date, to_date, frequency="Day", target_object=None):
        ...
        self.logger.info(f"Order Incremental Load 시작: {target_type} {target_topic} ({bdf.count()})")
        ...
        for row in grouped_df.collect():
            ...
            for data_row in row.data_list :
                if data_row.incremental is True :
                    self.logger.info(f"Order Object 시작: source_object={data_row.source_object}, target_object={data_row.target_object}, del_cond={data_row.target_delete_condition} , inc_cond={data_row.source_incremental_condition}")
                    self.ingest_increment(data_row.source_object, data_row.target_object, data_row.source_incremental_condition, data_row.target_delete_condition)              
                    
            self.logger.info(f"ELTManager Group 완료 : source_type={row.source_type}, source_topic={row.source_topic}, target_type={row.target_type}, target_topic={row.target_topic}")
