...
class EltManager:
    ...
    def ingest_full(self, source_objects, target_object) :    
        ...
        self.logger.info(f"ETL Done : [ {targetInfo} / {offset == target_df.count()} ({offset}, {target_df.count()}) / {datetime.datetime.now() - start_time} ]")    
    ...
    def ingest_increment(self, source_objects, target_object, source_inc_query, target_condition) :    
        ...
        self.logger.info(f"ETL Done : [ {targetInfo} / {source_read_size == target_read_size} ({source_read_size},{target_read_size}) / {datetime.datetime.now() - start_time} ]")    
