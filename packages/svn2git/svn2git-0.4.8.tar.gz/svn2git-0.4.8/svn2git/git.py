...
class GitMigrationHelper:
    ...
    def fix_tags(self, branches: GitBranches, tags_base: str = "tags") -> None:
        ...
        possible_user_name = run_command("git config user.name", exit_on_error=False, return_stdout=True).splitlines()
        possible_user_email = run_command("git config user.email", exit_on_error=False, return_stdout=True).splitlines()
