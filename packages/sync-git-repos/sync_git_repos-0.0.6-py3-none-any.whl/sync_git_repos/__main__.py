...
def _load_repos(settings: dict) -> Dict[str, utils.SyncSetting]:
    ...
    for key, repo in repos.items():
        ...
        result[key] = utils.SyncSetting(key=key,
                                        ...
                                        hooks={
                                            'before_push': _deserialize_list_of_hooks(
                                                hooks_cfg.get('before_push')) if hooks_cfg else [],
                                            'after': _deserialize_list_of_hooks(
                                                hooks_cfg.get('after')) if hooks_cfg else [],
                                            'before': _deserialize_list_of_hooks(
                                                hooks_cfg.get('before')) if hooks_cfg else [],
                                        })

    return result


def _deserialize_list_of_hooks(hooks_cfg: Optional[List[Union[str, dict]]] = None) -> list:
    if not hooks_cfg:
        return []
