...
from syndb_constants.table import DATASET_ID_COLUMN_NAME, ID_COLUMN_NAME, SyndbTable, syndb_table_to_table_name
...
INSERT_RAW_F_STRING = "INSERT INTO {syndb_table_name} ({df_columns}) VALUES ({df_column_len_qms})"
...
async def _read_datasets(
    ...
    if isinstance(dataset_id, UUID):
        df_scanner = await scylla.execute(
            READ_RAW_F_STRING.format(columns=column_str, syndb_table_name=syndb_table_to_table_name[syndb_table]),
...
async def insert_dataset_into_neuro_table(
    ...
    modifiable_dataset_ids: Optional[set[UUID]] = None,
) -> None:
    """
    ...
    :param modifiable_dataset_ids: Optionally provided set of dataset (UU)IDs, if the dataset IDs in the
        data frames are abscent from this set, will raise DatasetNotModifiableError
    :return: None
    """
    if modifiable_dataset_ids:
        all_dataset_ids = set()
        for df in table_to_df.values():
            all_dataset_ids.update(df[DATASET_ID_COLUMN_NAME].values)

        if not_modifiable := {duuid.bytes for duuid in modifiable_dataset_ids}.difference(all_dataset_ids):
            raise DatasetNotModifiableError(f"The following dataset are not modifiable: {not_modifiable}")
    ...
    for syndb_table, df in table_to_df.items():
        ...
        assert (
            DATASET_ID_COLUMN_NAME in df.columns
        ), f"{DATASET_ID_COLUMN_NAME} must be defined in DataFrame to define the belonging_dataset"
        ...
        for cdf in _df_chunker(df):
            ...
            await scylla.batch(batch, tuple(tuple(_scylla_cast(e) for e in row[1]) for row in cdf.iterrows()))
