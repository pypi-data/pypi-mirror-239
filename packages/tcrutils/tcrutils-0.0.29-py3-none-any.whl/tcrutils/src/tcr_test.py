...
class Timeit:
  ...
  def stop(self, printhook=print, color=True, pattern="%(c_White)s%(name)s%(c_Gold)s took %(c_White)s%(time)s%(c_Gold)s seconds to execute.%(c_reset)s"):
    if self.t is None:
      msg = 'You cannot stop the timer if it was not started'
      raise RuntimeError(msg)
    diff = time.perf_counter() - self.t
    self._print_time(printhook, pattern, self.tname, diff, color)
