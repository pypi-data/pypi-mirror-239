...
class ChallengeResolveMixin:
    ...
    def challenge_resolve(self, last_json: Dict) -> bool:
        ...
        return self.challenge_resolve_simple(challenge_url, code)
    ...
    def challenge_resolve_simple(self, challenge_url: str, code: Optional[str] = "") -> bool:
        """
        ...
        if step_name == "delta_login_review" or step_name == "scraping_warning":
            ...
            #     code = self.challenge_code_handler(self.username, ChallengeChoice.EMAIL, code)
            #     if code:
            #         break
            #     time.sleep(wait_seconds)
            print(
                f'Code entered "{code}" for {self.username} (attempts by {wait_seconds} seconds)'
            )
            print(self.last_json, ' : self.last_json')
            self._send_private_request(challenge_url, {"security_code": self.last_json["code"]})
