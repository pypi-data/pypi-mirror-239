...
def requires_compilation(model_type: SourceModelType) -> bool:
    return (
        model_type == SourceModelType.TORCHSCRIPT or model_type == SourceModelType.ONNX
    )
...
class CompilationJob(Job):
    ...
    target_shapes : InputSpecs
        The input specs for the compiled model to run (profiled or validated).
    options: str
        Options passed during the job submission.
    """
...
class Client:
    ...
    def _make_job(
        ...
        if job_pb.WhichOneof("job") == "compilation_job":
            ...
            target_shapes = dict(
                api.utils.tensor_type_list_pb_to_list_shapes(
                    compilation_job_pb.target_tensor_type_list
                )
            )
    ...
    def submit_compilation_job(
        ...
        if single_compilation and len(devices) > 1:
            ...
            if self.verbose:
                msg = (
                    f"Scheduled compilation job ({job.job_id}) successfully. To see "
    ...
    def submit_validation_job(
        ...
        if requires_compilation(model_type):
            raise UserError(
                "TorchScript and ONNX models cannot be used for validation."
            )
        ...
        for dev in devices:
            ...
            if self.verbose:
                ...
                msg = (
                    f"Scheduled validation job ({job.job_id}) successfully. To see "
                    "the status and results:\n"
                    f"    {base_url}\n"
                )
                print(msg)
    ...
    def submit_profile_job(
        ...
        model_type = _determine_model_type(model)
        if not requires_compilation(model_type):
            return self.submit_profiling_job(model, device, name, options)
        else:
            compilation_jobs = self.submit_compilation_job(
                model, device, name, input_shapes, options, single_compilation=False
            )
            jobs: List[Job] = []
            if isinstance(compilation_jobs, Job):
            ...
            for job in compilation_jobs:
                ...
                if model is not None:
                    job_name = name if name else job.name
                    profile_job = self.submit_profiling_job(
                        model, job.device, job_name, options
                    )
                    assert isinstance(profile_job, ProfileJob)
                    profile_job.shapes = job.shapes
                    jobs.append(profile_job)
                else:
                    jobs.append(job)
            # jobs list can have ProfileJob and CompilationJob instances
            return jobs[0] if len(jobs) == 1 else jobs
