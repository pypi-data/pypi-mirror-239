...
class HuggingFaceWavLMBasePlus(torch.nn.Module):
    ...
    @staticmethod
    def from_pretrained(
        weights_path: str | None = None, apply_apple_npu_opt: bool = False
    ) -> HuggingFaceWavLMBasePlus:
        """Load WavLM from a weightfile created by the source HUggingFaceWavLM repository."""

        model = WavLMModel.from_pretrained(
            "patrickvonplaten/wavlm-libri-clean-100h-base-plus", torchscript=True
        )

        return HuggingFaceWavLMBasePlus(model, apply_apple_npu_opt)
