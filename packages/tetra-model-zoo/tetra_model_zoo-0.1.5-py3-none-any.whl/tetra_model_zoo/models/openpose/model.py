...
DEFAULT_WEIGHTS_BODY_FILE_ID = "1EULkcH_hhSU28qVc1jSJpCh2hGOrzpjK"
DEFAULT_WEIGHTS_BODY = "body_pose_model.pth"
...
class OpenPose(torch.nn.Module):
    ...
    @staticmethod
    def from_pretrained(weights_path: str | None = None) -> OpenPose:
        ...
        return OpenPose(openpose_model)
...
def _load_openpose_source_model_from_weights(
    ...
    with SourceAsRoot(OPENPOSE_PROXY_REPOSITORY, OPENPOSE_PROXY_REPO_COMMIT, MODEL_ID):
        # download the weights file
        if not weights_path_body:
            weights_path_body = download_google_drive(
                DEFAULT_WEIGHTS_BODY_FILE_ID, MODEL_ID, DEFAULT_WEIGHTS_BODY
            )
