...
class SAMTetraWrapper:
    ...
    def get_sam_decoder(
        self, orig_img_size: Tuple(int, int), single_mask_mode=True
    ) -> Callable:
    ...
    @staticmethod
    def from_pretrained(model_type: str = DEFAULT_MODEL_TYPE) -> SAMTetraWrapper:
        (
            sam_model_registry,
            SamOnnxModel,
            ResizeLongestSide,
        ...
        return SAMTetraWrapper(
            sam, sam_encoder, SamOnnxModel, ResizeLongestSide, SamPredictor
        )

    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        raise NotImplementedError("Cannot call SAMTetraWrapper directly")
...
def _patch_sam_with_tetra_modules():
    ...
    sam_repo_path = maybe_clone_git_repo(
        SAM_SOURCE_REPO, SAM_SOURCE_REPO_COMMIT, MODEL_ID
    )
    cwd = os.getcwd()
    try:
