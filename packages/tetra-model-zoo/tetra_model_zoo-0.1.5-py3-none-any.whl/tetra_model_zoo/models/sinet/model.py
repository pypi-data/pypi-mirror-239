...
class SINet(torch.nn.Module):
    ...
    @staticmethod
    def from_pretrained(weights: str = DEFAULT_WEIGHTS) -> SINet:
        sinet_model = _load_sinet_source_model_from_weights(weights)

        return SINet(sinet_model.eval())
...
def _get_weightsfile_from_name(weights_name: str = DEFAULT_WEIGHTS):
    ...
    if weights_name == DEFAULT_WEIGHTS:
        return "https://github.com/clovaai/ext_portrait_segmentation/raw/master/result/SINet/SINet.pth"
    else:
...
def _load_sinet_source_model_from_weights(
    ...
    with SourceAsRoot(SINET_SOURCE_REPOSITORY, SINET_SOURCE_REPO_COMMIT, MODEL_ID):
        if os.path.exists(os.path.expanduser(weights_name_or_path)):
            ...
            if not os.path.exists(weights_name_or_path):
                ...
                weights_path = weights_name_or_path
        ...
        sinet_model.load_state_dict(pretrained_dict, strict=True)
