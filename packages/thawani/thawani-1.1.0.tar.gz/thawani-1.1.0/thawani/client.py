...
class Client:
    ...
    def __init__(self, session=None, secret_key=None,publishable_key=None, **options):
        ...
        self.base_url = self._set_base_url(**options) if data.get('use_sandbox')==1 else  URL.PROD_URL  
        self.app_details = []
    ...
    def generate_payment_link(self,data):
        if data.get('success')==True:
            url = "{}{}/{}?key={}".format(URL.ENDPOINT, URL.PAYMENT_LINK,data['data']['session_id'],self.publishable_key)
            return url
        else:
            return data
