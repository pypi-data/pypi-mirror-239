...
def load_plugin() -> None:
    ...
    # 判断是否可以进行自动补全的函数
    def can_complete():
        runner = get_runner()  # 获取当前运行器（Runner）实例
        return runner and not runner.is_running()  # 判断是否存在运行器且未在运行中

    # 添加到下拉菜单edit
    get_workbench().add_command(
        "autocomplete",  # 命令的唯一标识符
        "edit",  # 命令所属的菜单或工具栏组
        tr("Auto-complete"),  # 命令的显示名称
        completer.request_completions,  # 执行命令时调用的函数
        default_sequence="<Control-space>",  # 命令的默认快捷键序列
        tester=can_complete,  # 判断是否可以执行该命令的函数
    )

    get_workbench().set_default(
        "edit.tab_request_completions_in_editors", False
    )  # 设置在编辑器中按Tab键是否请求自动补全，默认为False
    get_workbench().set_default(
        "edit.tab_request_completions_in_shell", True
    )  # 设置在Shell中按Tab键是否请求自动补全，默认为True
    get_workbench().set_default("edit.automatic_completions", False)  # 设置是否启用自动补全，默认为False
    get_workbench().set_default("edit.automatic_completion_details", True)  # 设置是否显示自动补全详细信息，默认为True

    CodeViewText.perform_midline_tab = (
        completer.patched_perform_midline_tab
    )  # 替换CodeViewText类中的perform_midline_tab方法
    ShellText.perform_midline_tab = (
        completer.patched_perform_midline_tab
    )  # 替换ShellText类中的perform_midline_tab方法
