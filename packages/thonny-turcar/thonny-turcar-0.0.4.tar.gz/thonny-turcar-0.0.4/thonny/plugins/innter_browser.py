...
class InnterBrowser(ttk.Frame):
    def __init__(self, master):
        ...
        ttk.Button(self, text="点击", command=self.openbrowser).grid(row=0, column=1, padx=10, pady=10)
    ...
    def openbrowser(self):
        text_command = self.text_command.get("1.0", tk.END)
        if text_command:
            print(self.browser_widget.browser)
            if self.browser_widget.browser:
                text_command = text_command.replace("\n", "")
                print(text_command)
                self.browser_widget.browser.LoadUrl(text_command)
            else:
                rect = [0, 50, 1200, 1000]
                self.thread = threading.Thread(target=self.embed_browser_thread, args=(self, rect, text_command))
                self.thread.start()
...
def load_plugin() -> None:
    get_workbench().add_view(InnterBrowser, tr("浏览器"), "se")
