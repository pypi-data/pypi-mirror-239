...
"""
TweakableText 类实现了在 Tcl 级别拦截和控制文本命令的能力，以便对其进行自定义处理和扩展。
"""
...
class TweakableText(tk.Text):
    ...
    """
    该方法用于注册 Tk 命令的代理函数，并返回一个原始函数用于调用原始的 Tk 命令。
    """
    ...
    """
    分发 Tk 命令，并使用代理函数或原始的 Tk 命令来处理它们
    """

    def _dispatch_tk_operation(self, operation, *args):
        # _tk_proxies 字典中获取与 operation 相对应的代理函数 f
