...
def mls_iterative(
    ...
    while avg_point_movement > movement_threshold:
        ...
        xyz, avg_point_movement = mls_single(
            xyz=xyz, voxel_size=voxel_size, k_nearest=k_nearest
        )
        cnt += 1
...
def mls_single(
    xyz: np.ndarray, voxel_size: float, k_nearest: int
    ...
    for i, voxel_set in enumerate(neighboring_voxels):
        ...
        mls_approx[i, :] = (
            line.evaluate_at(xyz[i, :]) if line is not None else xyz[i, :]
        )
...
def cached_line_approximator(
    voxelizer: Voxelizer, voxel_set: FrozenSet[str]
) -> Union[Line3D, None]:
    """
