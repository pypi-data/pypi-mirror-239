import inspect
from typing import Any, Iterator, Union, TypeVar, Tuple, Callable, List
...
class Cfg:
    ...
    def _is_config_key_(cls, key: Any) -> bool:
        """判断是否是合法的配置类的 key"""
        return isinstance(key, str) and key.isidentifier() and key not in vars(Cfg)
    ...
    @classmethod
    def _get_(cls, key: Union[str, int], return_kv: bool = False) -> Union[Any, Tuple[str, Any]]:
        """获取静态变量中的内容
        ...
        if key is None:  # 设置 cls 本身
            ...
            for k, v in iter_value:
                ...
                cls._set_(k, v, create_new=create_new, cover_old=cover_old, old_define=old_define,
                          ancestors_has_cfg=ancestors_has_cfg)
            return
        try:
            ...
            if cls._is_config_class_(value):  # 配置类直接复制, 都是新变量无需递归
                setattr(cls, key, value.model_deepcopy(key))
    ...
    def model_dump_code(cls, 
        ...
        for name, value in cls._ikv_:
            ...
            if cls._is_config_class_(value):  # 嵌套类
                nested_class_code = value.model_dump_code(indent=indent+1, tab=tab)
                lines += nested_class_code
            else:  # 静态变量
                lines.append(f"{base_indent}{tab}{name} = {value!r}")
    ...
    def model_deepcopy(cls: T, new_class_name: str) -> T:
        ...
        for k, v in new_class._ikv_:
            if cls._is_config_class_(v):
                setattr(new_class, k, v.model_deepcopy(k))
            else:
                setattr(new_class, k, deepcopy(v))
        return new_class
...
if __name__ == '__main__':
    class MyClass(Cfg):
        ...
        class NestedClass(Cfg, object):
            nested_var = datetime.now()

            class NestedClass(Cfg):
                nested_var = 10*2

            class NestedClass2(Cfg):
                nested_var = '20'
