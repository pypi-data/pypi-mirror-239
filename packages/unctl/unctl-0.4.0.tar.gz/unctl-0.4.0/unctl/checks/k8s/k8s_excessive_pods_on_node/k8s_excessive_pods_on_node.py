...
class k8s_excessive_pods_on_node(Check):
    def _execute(self, node, pod_count, report) -> bool:
        ...
        if pod_count > max_cpu_pods * 0.8:
            report.status_extended = f"Node has too many pods ({pod_count}/{max_cpu_pods}) by CPU {cpu_capacity}"
            return False

        if pod_count > max_mem_pods * 0.8:
            report.status_extended = f"Node has too many pods ({pod_count}/{max_mem_pods}) by memory {mem_capacity}"
            return False
    ...
    def execute(self, data) -> list[Check_Report_K8S]:
        ...
        for node in data.get_nodes():
            ...
            if self._execute(node, node_pod_count[node.metadata.name], report) == False:
                report.status = "FAIL"
