...
class k8s_pod_crashloopbackoff(Check):
    ...
    def _execute(self, pod, report) -> bool:
        ...
        for container_status in container_statuses:
            # Check if the container's last state is terminated and the reason is 'CrashLoopBackOff'
            if (
                container_status.state.waiting
                and container_status.state.waiting.reason == "CrashLoopBackOff"
            ):
                # Set the resource_container attribute to the current container's name
                report.resource_container = container_status.name
                report.status_extended = f"Container {container_status.name} in pod {pod.metadata.name} is in CrashLoopBackOff state."
                return False
    ...
    def execute(self, data) -> list[Check_Report_K8S]:
        ...
        for pod in data.get_pods():
            ...
            if not self._execute(pod, report):
                report.status = "FAIL"
                report.status_extended = f"Pod {pod.metadata.name} in namespace {pod.metadata.namespace} has a container in CrashLoopBackOff state."
