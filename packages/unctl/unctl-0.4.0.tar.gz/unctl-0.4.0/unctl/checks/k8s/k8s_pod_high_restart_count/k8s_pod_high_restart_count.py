...
class k8s_pod_high_restart_count(Check):
    ...
    #                 namespace=pod.metadata.namespace, field_selector=field_selector).items
    #             # Check if there are recent restart events
    #             recent_restart_events = [
    #                 event for event in events if "Restarted" in event.reason and event.last_timestamp >= time_window]
    #             # Report if recent restart events are found
    #             if recent_restart_events:
    #                 print(
    #                     f"Pod {pod.metadata.name} in namespace {pod.metadata.namespace} has restarted more than 10 times in the last 30 minutes!")
    ...
    def execute(self, data) -> list[Check_Report_K8S]:
        ...
        for pod in data.get_pods():
            ...
            report.resource_id = pod.metadata.name
            ...
            if high_restart_containers:
                report.status = "FAIL"
                report.status_extended = f"Containers {', '.join(high_restart_containers)} in the pod have restarted more than 10 times."
                # Assuming multiple containers can have high restarts, store them all.
                report.resource_container = ", ".join(high_restart_containers)
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Pod's containers are within acceptable restart limits."
                )
