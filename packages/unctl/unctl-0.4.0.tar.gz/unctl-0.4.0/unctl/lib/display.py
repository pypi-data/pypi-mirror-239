...
class Display:
    ...
    def display_sort_by_check_table(check_details, llm_summary=False):
        ...
        headers = ["Resource Namespace", "Resource ID", "Status", "Severity"]
        ...
        for detail in check_details:
            if detail.status == "FAIL":
                ...
                resource_id_wrapped = textwrap.fill(
                    detail.resource_id, width=table._max_width.get("Resource ID", 0)
                )
                resource_id_wrapped = Display.center_content(
                    resource_id_wrapped, table._max_width.get("Resource ID", 0)
                )
    ...
    @staticmethod
    def display_list_checks_table(checks: List[Check]):
        print()
        ...
        for detail in checks:
            ...
            descr_width = Display.term_width - 80
            if descr_width < 30:
                descr_width = 30

            table.add_row(
                [
                    fill(detail.CheckTitle, width=30),
                    fill(detail.ServiceName, width=20),
        ...
        if sort_by == "object":
            ...
            for resource_namespace, resources in organized_results.items():
                for resource_id, check_details in resources.items():
                    for detail in check_details:
                        # Skip the detail if -f is enabled and the status is "PASS".
                        ...
                        wrapped_status_extended = textwrap.fill(
                            status_extended, width=60
                        )
            ...
            table_output = (
                ...
                .replace(
                    "Low", Fore.LIGHTGREEN_EX + Style.BRIGHT + "Low" + Style.RESET_ALL
                )
            )
            print(table_output)
        elif sort_by == "check":
