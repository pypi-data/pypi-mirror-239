...
def process_rule(rule_cfg, inventory):
    ...
    for item in inventory:
        ...
        if rule_cfg.get("pattern_is_shell", False) == False:
...
def main():
    ...
    # Cache inventories to avoid running the same inventory command multiple times
    inventories = {}
    ...
    for rule in data.get("rules", []):
        ...
        if inventory_id not in inventories:
            ...
            print(
                f"Inventory processed for {inventory_id} using {inventory_cfg.get('command')} and acquired {len(inventories[inventory_id])} items."
            )
