...
class SecretStoreApi():
    ...
    def get_handle(self, credentials_str: str, load_default: bool = False):
        ...
        elif connectorObject.credential_type == "CONNECTOR_TYPE_KEYCLOAK":
            from unskript.connectors.keycloak import KeycloakConnector
            handle = KeycloakConnector().get_handle(data=secretValue)
        elif connectorObject.credential_type == "CONNECTOR_TYPE_VAULT":
            from unskript.connectors.vault import VaultConnector
            handle = VaultConnector().get_handle(data=secretValue)
        else:
