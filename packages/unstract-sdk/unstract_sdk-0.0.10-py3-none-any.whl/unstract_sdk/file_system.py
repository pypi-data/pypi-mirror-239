...
from unstract_sdk.platform import UnstractPlatformBase
...
class UnstractToolFileSystem(UnstractPlatformBase):
    ...
    def get_fsspec_fs(
        ...
        fileType: str,
        utils: UnstractToolUtils = UnstractToolUtils(),
    ) -> AbstractFileSystem:
        ...
        Returns:
            Any: engine
        """
        platform_host = utils.get_env_or_die(PlatformServiceKeys.PLATFORM_HOST)
        platform_port = utils.get_env_or_die(PlatformServiceKeys.PLATFORM_PORT)

        utils.stream_log("Connecting to DB and getting table metadata")
