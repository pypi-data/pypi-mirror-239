...
class DeltaPositionMonitor:
    ...
    def update_position_greeks(self) -> None:
        if np.isnan(self.call_greeks.delta) and np.isnan(self.put_greeks.delta):
            pass
        elif np.isnan(self.call_greeks.delta):
            self.call_greeks.delta = self.put_greeks.delta + 1
        elif np.isnan(self.put_greeks.delta):
            self.put_greeks.delta = 1 - self.call_greeks.delta

        if np.isnan(self.call_greeks.gamma) and np.isnan(self.put_greeks.gamma):
            pass
        elif np.isnan(self.call_greeks.gamma):
            self.call_greeks.gamma = self.put_greeks.gamma * 1.2
        elif np.isnan(self.put_greeks.gamma):
            self.put_greeks.gamma = self.call_greeks.gamma * 1.2
...
def most_even_delta_strangle(
    ...
    # Filter out the NaN values before finding the minimum
    unevenness = {
        strangle: unevenness[strangle]
        for strangle in unevenness
        if not np.isnan(unevenness[strangle])
    }
    logger.info(
