...
def configure_http_backend(backend_factory: BACKEND_FACTORY_T = _default_backend_factory) -> None:
    """
    Configure the HTTP backend by providing a `backend_factory`. Any HTTP calls made by `wow_ai_hub` will use a
    Session object instantiated by this factory. This can be useful if you are running your scripts in a specific
    environment requiring custom configuration (e.g. custom proxy or certifications).

    Use [`get_session`] to get a configured Session. Since `requests.Session` is not guaranteed to be thread-safe,
    `wow_ai_hub` creates 1 Session instance per thread. They are all instantiated using the same `backend_factory`
    ...
    ```py
    import requests
    from wow_ai_hub import configure_http_backend, get_session
    ...
    # In practice, this is mostly done internally in `wow_ai_hub`
    session = get_session()
    ```
    """
...
def http_backoff(
    ...
    **kwargs,
    ...
    Example:
    ```
    >>> from wow_ai_hub.utils import http_backoff
    ...
    issue on [Github](https://github.com/huggingface/wow_ai_hub).
