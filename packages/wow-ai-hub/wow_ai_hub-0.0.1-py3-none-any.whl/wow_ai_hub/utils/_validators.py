...
"""Contains utilities to validate argument values in `wow_ai_hub`."""
import inspect
import re
import warnings
...
class HFValidationError(ValueError):
    """Generic exception thrown by `wow_ai_hub` validators.
...
def validate_hf_hub_args(fn: CallableT) -> CallableT:
    """Validate values received as argument for any public method of `wow_ai_hub`.
    ...
    Validators:
        ...
        - [`~utils.smoothly_deprecate_use_auth_token`]: Use `token` instead of
          ...
          function - in practice, always the case in `wow_ai_hub`).

    Example:
    ```py
    >>> from wow_ai_hub.utils import validate_hf_hub_args
    ...
    wow_ai_hub.utils._validators.HFValidationError: Cannot have -- or .. in repo_id: 'other..repo..id'.
    ...
    # True in `wow_ai_hub`. Might not be the case in a downstream library.
...
def validate_repo_id(repo_id: str) -> None:
    ...
    Example:
    ```py
    >>> from wow_ai_hub.utils import validate_repo_id
    >>> validate_repo_id(repo_id="valid_repo_id")
    >>> validate_repo_id(repo_id="other..repo..id")
    wow_ai_hub.utils._validators.HFValidationError: Cannot have -- or .. in repo_id: 'other..repo..id'.
    ```

    Discussed in https://github.com/huggingface/wow_ai_hub/issues/1008.
...
def smoothly_deprecate_use_auth_token(fn_name: str, has_token: bool, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    """Smoothly deprecate `use_auth_token` in the `wow_ai_hub` codebase.
    ...
    3. Step 3: After a transitional period (6 months e.g. until April 2023?), we update
       `wow_ai_hub` to throw a warning on `use_auth_token`. Hopefully, very few
       users will be impacted as it would have already been fixed.
       In addition, unit tests in `wow_ai_hub` must be adapted to expect warnings
    ...
    4. Step 4: After a normal deprecation cycle (3 releases ?), remove this validator.
       ...
       In addition, we update unit tests in `wow_ai_hub` to use `token` everywhere.

    This has been discussed in:
    - https://github.com/huggingface/wow_ai_hub/issues/1094.
    - https://github.com/huggingface/wow_ai_hub/pull/928
    - (related) https://github.com/huggingface/wow_ai_hub/pull/1064
    """
