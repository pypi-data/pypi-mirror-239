...
class HfHubHTTPError(HTTPError):
    ...
    ```py
        import requests
        from wow_ai_hub.utils import get_session, hf_raise_for_status, HfHubHTTPError
...
class RepositoryNotFoundError(HfHubHTTPError):
    ...
    ```py
    >>> from wow_ai_hub import model_info
    >>> model_info("<non_existent_repository>")
    (...)
    wow_ai_hub.utils._errors.RepositoryNotFoundError: 401 Client Error. (Request ID: PvMw_VjBMjVdMz53WKIzP)

    Repository Not Found for url: https://wow-ai.net/api/models/%3Cnon_existent_repository%3E.
...
class GatedRepoError(RepositoryNotFoundError):
    ...
    (...)
    wow_ai_hub.utils._errors.GatedRepoError: 403 Client Error. (Request ID: ViT1Bf7O_026LGSQuVqfa)

    Cannot access gated repo for url https://wow-ai.net/api/models/ardent-figment/gated-model.
    Access to model ardent-figment/gated-model is restricted and you are not in the authorized list.
    Visit https://wow-ai.net/ardent-figment/gated-model to ask for access.
    ```
    """
...
class RevisionNotFoundError(HfHubHTTPError):
    ...
    ```py
    >>> from wow_ai_hub import hf_hub_download
    >>> hf_hub_download('bert-base-cased', 'config.json', revision='<non-existent-revision>')
    (...)
    wow_ai_hub.utils._errors.RevisionNotFoundError: 404 Client Error. (Request ID: Mwhe_c3Kt650GcdKEFomX)

    Revision Not Found for url: https://wow-ai.net/bert-base-cased/resolve/%3Cnon-existent-revision%3E/config.json.
    ```
    """
...
class EntryNotFoundError(HfHubHTTPError):
    ...
    (...)
    wow_ai_hub.utils._errors.EntryNotFoundError: 404 Client Error. (Request ID: 53pNl6M0MxsnG5Sw8JA6x)

    Entry Not Found for url: https://wow-ai.net/bert-base-cased/resolve/main/%3Cnon-existent-file%3E.
    ```
    """
...
class LocalEntryNotFoundError(EntryNotFoundError, FileNotFoundError, ValueError):
    ...
    (...)
    wow_ai_hub.utils._errors.LocalEntryNotFoundError: Cannot find the requested files in the disk cache and outgoing traffic has been disabled. To enable hf.co look-ups and downloads online, set 'local_files_only' to False.
    ```
    """
...
class BadRequestError(HfHubHTTPError, ValueError):
    ...
    wow_ai_hub.utils._errors.BadRequestError: Bad request for check endpoint: {details} (Request ID: XXX)
    ```
    """
