...
from ..constants import wow_ai_hub_CACHE
from . import logging
...
class HFCacheInfo:
    ...
    def delete_revisions(self, *revisions: str) -> DeleteCacheStrategy:
        ...
        Examples:
        ```py
        >>> from wow_ai_hub import scan_cache_dir
...
def scan_cache_dir(cache_dir: Optional[Union[str, Path]] = None) -> HFCacheInfo:
    ...
    if not cache_dir.exists():
        raise CacheNotFound(
            ...
            " argument or set `wow_ai_hub_CACHE` environment variable.",
            cache_dir=cache_dir,
        )
    ...
    if cache_dir.is_file():
        raise ValueError(
            ...
            " `cache_dir` argument or set `wow_ai_hub_CACHE` environment"
            " variable."
        )
