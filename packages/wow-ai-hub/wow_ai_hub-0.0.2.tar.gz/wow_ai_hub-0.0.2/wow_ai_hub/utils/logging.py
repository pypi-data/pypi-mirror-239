...
def _get_default_logging_level():
    """
    If wow_ai_hub_VERBOSITY env var is set to one of the valid choices
    return that as the new default level. If it is not - fall back to
    `_default_log_level`
    """
    env_level_str = os.getenv("wow_ai_hub_VERBOSITY", None)
    if env_level_str:
        if env_level_str in log_levels:
            return log_levels[env_level_str]
        else:
            logging.getLogger().warning(
                f"Unknown option wow_ai_hub_VERBOSITY={env_level_str}, "
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    ```python
    >>> from wow_ai_hub import get_logger
...
def get_verbosity() -> int:
    ...
    Returns:
        Logging level, e.g., `wow_ai_hub.logging.DEBUG` and
        `wow_ai_hub.logging.INFO`.

    <Tip>

    HuggingFace Hub has following logging levels:

    - `wow_ai_hub.logging.CRITICAL`, `wow_ai_hub.logging.FATAL`
    - `wow_ai_hub.logging.ERROR`
    - `wow_ai_hub.logging.WARNING`, `wow_ai_hub.logging.WARN`
    - `wow_ai_hub.logging.INFO`
    - `wow_ai_hub.logging.DEBUG`
