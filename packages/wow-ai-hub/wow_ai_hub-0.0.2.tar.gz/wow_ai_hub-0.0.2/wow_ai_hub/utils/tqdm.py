...
"""Utility helpers to handle progress bars in `wow_ai_hub`.

Example:
    1. Use `wow_ai_hub.utils.tqdm` as you would use `tqdm.tqdm` or `tqdm.auto.tqdm`.
...
def disable_progress_bars() -> None:
    """
    Disable globally progress bars used in `wow_ai_hub` except if `HF_HUB_DISABLE_PROGRESS_BARS` environment
...
def enable_progress_bars() -> None:
    """
    Enable globally progress bars used in `wow_ai_hub` except if `HF_HUB_DISABLE_PROGRESS_BARS` environment
    variable has been set.
...
def are_progress_bars_disabled() -> bool:
    ...
    Progress bars used in `wow_ai_hub` can be enable or disabled globally using [`~utils.enable_progress_bars`]
...
class tqdm(old_tqdm):
    ...
    def __delattr__(self, attr: str) -> None:
        """Fix for https://github.com/huggingface/wow_ai_hub/issues/1603"""
        try:
...
def tqdm_stream_file(path: Union[Path, str]) -> Iterator[io.BufferedReader]:
    ...
    First implemented in `transformers` in 2019 but removed when switched to git-lfs. Used in `wow_ai_hub` to show
