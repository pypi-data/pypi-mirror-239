...
class Template:
    ...
    def move_directory(self, source, destination, boxno):
        if os.path.exists(destination+boxno):
            shutil.rmtree(destination)  # Remove the destination directory if it exists
        #os.makedirs(destination)  # Ensure the destination directory exists
        shutil.move(source, destination)  # Move the entire directory
