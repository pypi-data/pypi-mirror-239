...
window.vm = new Vue({
    el: '#app', data: {
        ...
        canvasStyle: {
            opacity: 0.5, width: 'inherit', height: 'inherit'
        },
        lastScreenSize: {
            screen: {}, canvas: {
                width: 1, height: 1
        ...
        mapAttrCount: {},
        pyshell: {
            running: false, restarting: false, consoleData: [], wsOpen: false, ws: null, lineno: {
                offset: 0, current: -1,
            }
        }
    }, watch: {
        ...
        // 记得在组件销毁时移除事件监听，以防止内存泄漏
        window.removeEventListener('beforeunload', this.closeWDA);
    }, mounted: function () {
        const queryString = window.location.search;
        const param = new URLSearchParams(queryString);
        this.imgUrl = param.get('imgUrl');
        this.xmlUrl = param.get('xmlUrl');
        ...
        if (this.isUiParse) {
            this.tabActiveName = "hierarchy"
            this.getImage()
            this.getDumpHierarchy()
        } else {
            window.addEventListener('beforeunload', this.closeWDA);
        ...
        startWda: function () {
            this.starting = true
            $.ajax({
                url: LOCAL_URL + "api/v1/startWDA", method: 'POST', data: {
            ...
            this.getting = true
            $.ajax({
                url: LOCAL_URL + "api/v1/devices/list", type: "GET",
            })
                .done((ret) => {
            ...
            $.ajax({
                url: LOCAL_URL + "api/v1/version", type: "GET",
            })
                .done((ret) => {
            ...
            ws.onmessage = (message) => {
                ...
                switch (data.method) {
                    ...
                    case "restarted":
                        ...
                        this.resetEditor()
                        this.$notify.success({
                            title: "重启内核", message: "成功", duration: 800, offset: 100,
                        })
            ...
            const code = ["# coding: utf-8", "#", "import uiautomator2 as u2", "", "d = u2.connect()", "", "",].join("\n");
            ...
            this.deviceId = '';
            return $.ajax({
                url: LOCAL_URL + "api/v1/connect", method: 'POST', data: {
                    platform: this.platform, deviceUrl: this.deviceUrl,
                },
            })
                .then((ret) => {
            ...
            if (this.starting) {
                ...
                this.deviceId = '';
                return $.ajax({
                    url: LOCAL_URL + "api/v1/close/wda", method: 'POST', data: {
                        serial: this.current_device.split('|')[1]
                    },
                })
                    .then((ret) => {
            ...
            editor.commands.addCommands([{
                ...
                name: "build-inline", bindKey: {
                    win: "Ctrl-Shift-Enter", mac: "Command-Shift-Enter",
                }, exec: function (editor) {
                    self.codeRunSelected()
                }
            }]);

            const AndroidCompletions = [{name: "应用安装", value: "d.app_install"}, {
                name: "启动应用", value: "d.app_start"
            }, {name: "清空应用", value: "d.app_clear"}, {name: "停止应用", value: "d.app_stop"}, {
                name: "当前应用", value: "d.app_current()"
            }, {name: "获取应用信息", value: "d.app_info"}, {
                name: "等待应用运行", value: "d.app_wait"
            }, {name: "窗口大小", value: "d.window_size()"}, {name: "截图", value: "d.screenshot()"}, {
            ...
            }, {name: "坐标点击 x,y", value: "d.click"}, {name: "获取图层", value: "d.dump_hierarchy()"}, {
            ...
            const iOSCompletions = [{name: "状态信息", value: "d.status()"}, {
                name: "等待就绪", value: "d.wait_ready(timeout=300)"
            }, {name: "HOME", value: "d.home()"}, {name: "截图", value: "d.screenshot()"}, {
            ...
            }, {name: "窗口UI大小", value: "d.window_size()"}, {name: "点击", value: "d.click"}, {
            ...
            const xpathCompletions = [{name: "点击", value: "click()"}, {
                name: "存在时点击", value: "click_exists()"
            }, {name: "等待元素出现", value: "wait()"}, {name: "等待元素消失", value: "wait_gone()"}, {
                name: "是否存在", value: "exists"
            }, {name: "控件截图", value: "screenshot()"}, {
                name: "控件上滑", value: 'swipe("up")'
            }, {name: "获取控件中心点坐标", value: "center()"}, {name: "信息", value: "info"}, {
                name: "获取Element", value: "get()"
            }, {name: "返回所有匹配", value: "all()"}, {name: "获取XpathElement", value: "get(timeout=10)"},]

            const isAndroid = (this.platform === "Android")

            let keywordCompleter = {
                identifierRegexps: [/\./], getCompletions: (editor, session, pos, prefix, callback) => {
                    ...
                    if (prefix === "." && /\w+\.xpath\([^)]+\)\./.test(line)) { // match: d.xpath("settings").
                        callback(null, xpathCompletions.map(v => {
                            return {
                                score: 1, meta: v.name, value: prefix + v.value,
                            }
                        }))
                    } else if (prefix === "d") {
            ...
            if (img) {
                if (this.lastScreenSize.canvas.width == img.width && this.lastScreenSize.canvas.height == img.height) {
                    return;
                }
            } else {
            ...
            xhr.onload = () => {
                if (xhr.status === 200) {
                    ...
                    reader.onload = () => {
                        ...
                        // 在这里可以使用base64Data，例如显示在页面上或者发送给服务器
                        // 去除文件头信息
                        const startIndex = base64Data.indexOf(',') + 1;
                        base64Data = base64Data.substring(startIndex);
                        // console.log(base64Data);
                        var newBlob = b64toBlob(base64Data, imageType);
                        this.drawBlobImageToScreen(newBlob);
                        this.loading = false
                    };
                    // 读取Blob数据并触发onload事件
            ...
            var dtd = $.Deferred();
            var bgcanvas = this.canvas.bg, fgcanvas = this.canvas.fg, ctx = bgcanvas.getContext('2d'), self = this,
            ...
            let codeLines;
            if (m[1] == "ios") {
                codeLines = [// "print('Set environment for iOS device\\nInit c = d = wda.Client()')",
                    "import os", "import wda", `os.environ['DEVICE_URL'] = "${deviceUrl}"`, `c = d = wda.Client()`,]
            } else if (m[1] == "android") {
                codeLines = [// "print('Set environment and prepare d = u2.connect()')",
                    "import os", "import uiautomator2 as u2", `os.environ['ANDROID_DEVICE_IP'] = "${deviceUrl}"`, `d = u2.connect()`,]
            } else {
                console.error("Unsupported deviceId", this.deviceId)
                codeLines = [`print("Unsupported deviceId: ${this.deviceId}")`]
            }
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    windowSize: localStorage.windowSize.split(",").map(v => {
            ...
            var x = node.rect.x, y = node.rect.y, w = node.rect.width, h = node.rect.height;
            color = color || 'black';
            ...
            function isInside(node, x, y) {
                ...
                var lx = node.rect.x, ly = node.rect.y, rx = node.rect.width + lx, ry = node.rect.height + ly;
            ...
            function coord(event) {
                ...
                return {
                    px: px, py: py, x: Math.floor(px * element.width), y: Math.floor(py * element.height),
                }
            }
