...
# import os
class DeviceNode():
    def __init__(self, tabname,idname,id,section):
        self.tabname = tabname
        self.idname = idname
        self.id = id
        self.section = section
    def read(self):
        device=Mysql(self.tabname)
        s,c=device.databycheckdatas({self.idname:str(self.id)})
        device.close()
        return s[0][self.section]
    def write(self,value):
        device = Mysql(self.tabname)
        device.modify(self.section,self.idname,[str(value),str(self.id)] )
        device.close()
class Mysql():
...
if __name__ == '__main__':
   ...
   d1 = DeviceNode("device","DeviceID","M1","DeviceType")
   print(d1.read())
